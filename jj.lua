...
local function nt(n)
    ...
    for i = #g, 1, -1 do
        ...
        if v ~= 0 then
            ...
            if i == 4 then txt = (v == 1 and "um bilhão" or txt .. " bilhões") end
            if i == 5 then txt = (v == 1 and "um trilhão" or txt .. " trilhões") end
            table.insert(x, txt)
        end
    end
...
jjs.Start = function()
    ...
    task.spawn(function()
        ...
        local s, e, dir = startVal, endVal, 1
        
        if c.ReverseEnabled then 
            s, e, dir = endVal, startVal, -1 
        else
            if s > e then dir = -1 end
        end

            ...
            if s and content then
                ...
                if clean:sub(1, 12) ~= "michigun.xyz" then
                    ...
                    if d and d.Frames then
                        local name = path:match("([^/\\]+)%.json$") or "migrated_file"
                        local rj = hs:JSONEncode({ Version = 1, Frames = d.Frames })
...
tas.GetSaved = function()
    local out = {}
    if listfiles then 
        for _, f in ipairs(listfiles(fDir)) do 
            if f:sub(-5) == ".json" then out[#out + 1] = f:match("([^/]+)%.json$") end 
        end 
    end
    return out
end
        ...
        for i, n in ipairs(tas.Selection) do
            if not tas.Loaded[n] and n ~= "" then
                ...
                if isfile(p) then
                    ...
                    local clean = stripHeader(raw)
                    local data = nil
                    
                    if clean:sub(1, 12) == "michigun.xyz" then
                        pcall(function() data = hs:JSONDecode(s_e(S_KEY, dh(clean:sub(13)))) end)
                    else
                        pcall(function() data = hs:JSONDecode(raw) end)
                    end
                    
                    if data and data.Frames then
                        tas.Loaded[n] = { Frames = data.Frames, Viewports = {}, PathParts = {}, Waiting = false, Playing = false }
                    end
                end
            end

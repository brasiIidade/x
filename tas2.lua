...
-- VirtualInputManager removido para evitar sumiço do TouchGui
...
local function stopMovementInput()
    -- Não usamos mais VirtualInputManager aqui
    
    local hum = getHumanoid()
    if hum then hum.AutoRotate = true end

    -- Garante que o TouchGui esteja visível
    local pg = LocalPlayer:FindFirstChild("PlayerGui")
    if pg then
        local tg = pg:FindFirstChild("TouchGui")
        if tg then
            tg.Enabled = false -- Reseta o estado
            task.spawn(function()
                for i = 1, 10 do -- Loop de persistência para forçar a UI voltar
...
local function applyFrame(f)
    ...
    -- Aplica a velocidade gravada (Isso garante que o pulo físico aconteça)
    if f.vel then hrp.AssemblyLinearVelocity = Vector3.new(f.vel[1], f.vel[2], f.vel[3]) end
    
    -- Câmera livre (comentado para não travar a visão)
    -- if f.cam then CurrentCamera.CFrame = CFrame.new(unpack(f.cam)) end 

    -- Lógica de Pulo NOVA (Sem VirtualInputManager e sem propriedade .Jump)
    if f.jump ~= _G.TAS.LastJumpInput then
        if f.jump then
            -- Força o estado para Jumping (não é input de teclado, então não esconde UI)
    ...
    if recordedState then
        ...
        -- Evita spammar o estado Jumping se já estiver pulando

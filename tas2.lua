...
local function stopMovementInput()
    ...
    -- Retorna AutoRotate ao normal ao parar
...
local function captureFrame()
    ...
    -- Mudança: Usamos AssemblyLinearVelocity para maior precisão física
    return {
        ...
        jump = hum.Jump,
        -- Opcional: Gravar o estado pode ajudar, mas calcular na hora é melhor para fluidez
...
local function applyFrame(f)
    ...
    -- 1. Desativar AutoRotate para não conflitar com o CFrame
    hum.AutoRotate = false

    -- 2. Aplicar CFrame (Teleporte)
    if f.cf then 
        hrp.CFrame = CFrame.new(unpack(f.cf)) 
    end

    -- 3. Aplicar Velocidade
    -- Isso é crucial para o script 'Animate' saber a velocidade de mistura da animação
    local velocity = Vector3.zero
    if f.vel then 
        velocity = Vector3.new(f.vel[1], f.vel[2], f.vel[3])
        hrp.AssemblyLinearVelocity = velocity 
    end

    -- 4. CORREÇÃO DOS BRAÇOS (State Management)
    -- Se estivermos nos movendo verticalmente rápido, assumimos pulo/queda.
    -- Se não, forçamos "Running" para evitar que o braço levante (animação de equilíbrio).
    local yVel = velocity.Y
    
    if f.jump then
        hum:ChangeState(Enum.HumanoidStateType.Jumping)
    elseif yVel < -10 then -- Caindo rápido
        hum:ChangeState(Enum.HumanoidStateType.Freefall)
    else
        -- Força o estado de corrida mesmo que esteja um pouco no ar (devido ao CFrame)
        -- Isso estabiliza os braços
        hum:ChangeState(Enum.HumanoidStateType.Running)
    end

    if f.cam then CurrentCamera.CFrame = CFrame.new(unpack(f.cam)) end
    
    -- Nota: Não aplicamos hum.Jump direto aqui pois o ChangeState cuida disso melhor
end
...
local function activateTAS(name)
    ...
    data.MarkerConn = RunService.Heartbeat:Connect(function()
        ...
        if flatDist <= actRadius and math.abs(delta.Y) <= actHeight and dot >= math.cos(math.rad(actAngle)) then
            ...
            -- Mudamos para Heartbeat para garantir que a atualização visual ocorra junto com o frame renderizado
...
_G.TAS.StartRecording = function()
    ...
    _G.TAS.RecordConn = RunService.Heartbeat:Connect(function(dt)
        local hrp = getHRP()
        if hrp then
            -- Usar AssemblyLinearVelocity é mais preciso que Velocity (que é deprecated)

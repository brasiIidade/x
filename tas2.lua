...
-- === NOVO: VERIFICAÇÃO DE CHÃO (RAYCAST) ===
-- Isso impede que o personagem fique com animação de "Caindo" enquanto está visualmente no chão.
local function isGrounded(hrp)
    local params = RaycastParams.new()
    params.FilterDescendantsInstances = {LocalPlayer.Character}
    params.FilterType = Enum.RaycastFilterType.Exclude
    -- Lança um raio de 3.5 studs para baixo (altura da perna + margem)
...
local function applyFrame(f)
    ...
    -- 1. Aplicar CFrame e Velocidade primeiro
    ...
    -- Se a gravação diz que estamos caindo ou pulando, mas o raio detecta chão
    -- E a velocidade vertical é baixa (não estamos explodindo pra cima num pulo)
    -- ENTÃO forçamos "Running" para evitar o bug do braço e da queda no chão.
    
    if onGround and math.abs(vY) < 5 then
        -- Estamos no chão ou pousando.
        -- Se o estado gravado for Freefall ou Jumping, isso é um erro visual de 1 frame.
        -- Forçamos Running para suavizar a aterrissagem.
        if recordedState == Enum.HumanoidStateType.Freefall or recordedState == Enum.HumanoidStateType.Jumping then
             recordedState = Enum.HumanoidStateType.Running
        end
    end

    -- Se o estado for escalada (Climbing), respeitamos sempre, pois o Raycast pode falhar em escadas
    if f.state == Enum.HumanoidStateType.Climbing.Value then
        recordedState = Enum.HumanoidStateType.Climbing
    end

    -- Aplica o estado corrigido
    if recordedState then
        if hum:GetState() ~= recordedState then

...
pcall(function() ScanHierarchy(game:GetService("CoreGui")) end)
SetupChatSpoof()
...
local function UpdateAnonVisuals(reverting)
    local function ProcessNode(node)
        pcall(function()
            if (node:IsA("TextLabel") or node:IsA("TextButton") or node:IsA("TextBox")) and node.Text ~= "" then
                if reverting then
                    local safeAnon = S.FakeName:gsub("([%^%$%(%)%%%.%[%]%*%+%-%?])", "%%%1")
                    node.Text = node.Text:gsub(safeAnon, S.RealDisplay)
                else
                    local txt = node.Text:gsub(S.SafeName, S.FakeName)
                    txt = txt:gsub(S.SafeDisplay, S.FakeDisplay)
                    if node.Text ~= txt then node.Text = txt end
                end
            end
        end)
    end
    ...
    pcall(function() 
        for _, v in ipairs(game:GetService("CoreGui"):GetDescendants()) do ProcessNode(v) end 
    end)
end

...
local function sc(m)
    ...
    -- 1. Envia a mensagem no chat nativo do Roblox
    if tcs.ChatVersion == Enum.ChatVersion.TextChatService then
        local tgt = tcs.ChatInputBarConfiguration.TargetTextChannel
        if tgt then
            pcall(function() tgt:SendAsync(s) end)
        else
            local c = tcs:FindFirstChild("TextChannels")
            if c then
                local gen = c:FindFirstChild("RBXGeneral")
                if gen then
                    pcall(function() gen:SendAsync(s) end)
                end
            end
        end
    else
    ...
    -- 2. Checa se o sistema customizado do mapa (ServerCommands) existe e o dispara
    local assets = rep:FindFirstChild("Assets")
    if assets then
        local remotes = assets:FindFirstChild("Remotes")
        if remotes then
            local serverCommands = remotes:FindFirstChild("ServerCommands")
            if serverCommands then
                pcall(function()
                    serverCommands:FireServer(s)
                end)
            end
        end
    end

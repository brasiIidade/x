...
local function buildPath(frames)
...
local function clearMarkers()
    ...
    for _, v in ipairs(ActiveViewports) do
...
local function createStartMarker(name, frames)
    local cf = CFrame.new(unpack(frames[1].cf))

    local folder = Instance.new("Folder", Workspace)
    folder.Name = "Start_" .. name

    local function mk(shape, size, cframe, color)
    ...
    mk(Enum.PartType.Cylinder, Vector3.new(0.25,4,4), cf * CFrame.Angles(0,0,math.rad(90)), BASE_COLOR_POS)
    mk(Enum.PartType.Block, Vector3.new(0.5,0.1,1.5), cf * CFrame.new(0,1,-0.75), BASE_COLOR_LOOK)
    mk(Enum.PartType.Block, Vector3.new(1,0.1,0.5), cf * CFrame.new(0,1,-1.75), BASE_COLOR_LOOK)

    buildPath(frames)

    StartMarkers[#StartMarkers+1] = {
        Frames = frames,
        CFrame = cf,
        Folder = folder
    }
...
local function playAll()
    ...
    if #SelectedTAS == 0 then return end

    clearMarkers()
    WaitingForStart = true

    for _, name in ipairs(SelectedTAS) do
        local frames = FramesByName[name]
        if frames then
            createStartMarker(name, frames)
        end
    end
    ...
    MarkerConn = RunService.Heartbeat:Connect(function()
        ...
        for _, marker in ipairs(StartMarkers) do
            local delta = hrp.Position - marker.CFrame.Position
            if Vector3.new(delta.X,0,delta.Z).Magnitude <= 2 then
                clearMarkers()
                Playing = true

                local i = 1
                PlayConn = RunService.Heartbeat:Connect(function()
                    if i > #marker.Frames then
                        PlayConn:Disconnect()
                        Playing = false
                        RequestedPlay = false
                        TAS:ToggleSet(false)
...
local function saveCurrentTAS()
    if not CurrentName or #FramesByName[CurrentName] == 0 then return end
    writefile(TAS_FOLDER .. "/" .. CurrentName .. ".json",
        HttpService:JSONEncode({ Frames = FramesByName[CurrentName] })
    )
    TASDropdown:Refresh(getSavedTAS())
end

local function loadMultiple(list)
    clearMarkers()
    SelectedTAS = list
    FramesByName = {}
    ...
    for _, name in ipairs(list) do
        ...
        if isfile(path) then
            FramesByName[name] = HttpService:JSONDecode(readfile(path)).Frames or {}
        end
    end
end

...
_G.TAS.ActivationRadius = 1.5
_G.TAS.ActivationHeight = 1.5
_G.TAS.ActivationAngle = 14
...
local function createViewportMarker(partToTrack)
    ...
    clone.Color = Color3.fromRGB(255, 204, 153)
...
local function activateTAS(name)
    ...
    dummyModel.Name = "HumanoidVisual"
    dummyModel.Parent = container

    local humanoid = Instance.new("Humanoid")
    humanoid.DisplayDistanceType = Enum.HumanoidDisplayDistanceType.None
    humanoid.Parent = dummyModel

    local function makeBodyPart(pName, pSize, pCFrame, pColor)
    ...
    local head = makeBodyPart("Head", Vector3.new(2, 1, 1), cf * CFrame.new(0, 1.5, 0), skinColor)
    ...
    local torso = makeBodyPart("Torso", Vector3.new(2, 2, 1), cf * CFrame.new(0, 0, 0), skinColor)
    local leftArm = makeBodyPart("Left Arm", Vector3.new(1, 2, 1), cf * CFrame.new(-1.5, 0, 0), skinColor)
    local rightArm = makeBodyPart("Right Arm", Vector3.new(1, 2, 1), cf * CFrame.new(1.5, 0, 0), skinColor)
    local leftLeg = makeBodyPart("Left Leg", Vector3.new(1, 2, 1), cf * CFrame.new(-0.5, -2, 0), skinColor)
    local rightLeg = makeBodyPart("Right Leg", Vector3.new(1, 2, 1), cf * CFrame.new(0.5, -2, 0), skinColor)

    local shirt = Instance.new("Shirt", dummyModel)
    shirt.ShirtTemplate = "http://www.roblox.com/asset/?id=607785314"

    local pants = Instance.new("Pants", dummyModel)
    pants.PantsTemplate = "http://www.roblox.com/asset/?id=129458426"

    local basePad = Instance.new("Part")
    basePad.Name = "BasePad"
    basePad.Shape = Enum.PartType.Cylinder
    basePad.Size = Vector3.new(0.2, 6, 6)
    basePad.CFrame = cf * CFrame.new(0, -2.8, 0) * CFrame.Angles(0,0,math.rad(90))
    basePad.Anchored = true
    basePad.CanCollide = false
    basePad.Material = Enum.Material.Neon
    basePad.Color = BASE_COLOR_POS
    basePad.Transparency = 0.5
    basePad.Parent = container
...
_G.TAS.StopRecording = function()
    ...
    safeNotify("michigun.xyz", string.format("Gravação parada (%.2f segundos)", duration))
...
_G.TAS.DeleteSelected = function()
    if #_G.TAS.Selection == 0 then
        safeNotify("michigun.xyz", "Nenhuma gravação selecionada")
        return
    end

...
--// SPAWN TAS (LÓGICA ORIGINAL ISOLADA)
local function spawnTAS(frames)
	local ctx = {}
	ctx.PathParts = buildPathLine(frames)
	...
	local start = Instance.new("Part", container)
	start.Shape = Enum.PartType.Cylinder
	start.Size = Vector3.new(0.25,4,4)
	start.CFrame = cf * CFrame.Angles(0,0,math.rad(90))
	start.Anchored = true
	start.CanCollide = false
	start.Material = MATERIAL_INDICATOR
	start.Color = BASE_COLOR_POS
	start.Transparency = TRANSPARENCY_INDICATOR

	createViewportMarker(start)

	ctx.MarkerConn = RunService.Heartbeat:Connect(function()
		if Paused then return end
		local hrp = getHRP()
		if not hrp then return end
		if (hrp.Position - cf.Position).Magnitude <= 2 then
			ctx.MarkerConn:Disconnect()
			local i = 1
			ctx.PlayConn = RunService.Heartbeat:Connect(function()
				if Paused or i > #frames then
					ctx.PlayConn:Disconnect()
					return
				end
				applyFrame(frames[i])
				i += 1
...
local function saveCurrentTAS()
	if not CurrentName or #Frames == 0 then return end
	writefile(TAS_FOLDER.."/"..CurrentName..".json",
		HttpService:JSONEncode({ Frames = Frames }))
end
...
local TASDropdown = TAS:Dropdown({
	...
	Callback = function(list)
		for _, name in ipairs(list) do
			local data = HttpService:JSONDecode(readfile(TAS_FOLDER.."/"..name..".json"))
			spawnTAS(data.Frames)
		end
	end
})

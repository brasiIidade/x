...
-- Novas variáveis para o filtro de times
local TeamFilterEnabled = false
local SelectedTeams = {}

-- Função auxiliar para pegar nomes dos times
...
local function ApplyHitbox(plr)
    -- Checagem padrão: Se hitbox off, ou é o player local, ou é do mesmo time
    ...
    -- Checagem do Filtro de Times (NOVO)
        ...
        if flatDist <= 2 and math.abs(delta.Y) <= 1 and dot >= math.cos(math.rad(10)) then
            ...
            PlayConn = RunService.Heartbeat:Connect(function()
                if i > #Frames then
                    ...
                    Playing = false
                    RequestedPlay = false
                    if TAS and TAS.ToggleSet then TAS:ToggleSet(false) end
                    return
                end
                applyFrame(Frames[i])
                i += 1
...
local function playTAS()
    ...
    if not Frames or #Frames == 0 then
        Notify("TAS", "Selecione uma gravação primeiro.")
        return
    end
...
TAS:Button({ Title = "Deletar gravação", Desc = "Deleta a gravação selecionada", Callback = deleteTAS })
TAS:Space()

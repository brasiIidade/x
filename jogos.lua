...
elseif MapaAtual == "Apex" then
        -- Mantém a tabela se ela já existir (caso a UI carregue antes)
        getgenv().ApexLogic = getgenv().ApexLogic or {}
        
        -- Separamos as conexões para o CharacterAdded não ser deletado por engano
        getgenv()._ApexSpamMasterConns = getgenv()._ApexSpamMasterConns or {}
        getgenv()._ApexSpamCharConns = getgenv()._ApexSpamCharConns or {}

        local function limparMasterConns()
            for _, conn in ipairs(getgenv()._ApexSpamMasterConns) do
                if typeof(conn) == "RBXScriptConnection" and conn.Connected then 
                    conn:Disconnect() 
                end
            end
            table.clear(getgenv()._ApexSpamMasterConns)
        end

        local function limparCharConns()
            for _, conn in ipairs(getgenv()._ApexSpamCharConns) do
                if typeof(conn) == "RBXScriptConnection" and conn.Connected then 
                    conn:Disconnect() 
                end
            end
            table.clear(getgenv()._ApexSpamCharConns)
        end
        ...
        getgenv().ApexLogic.ToggleSound = function(ativar)
            ...
            if ativar then
                -- Prevenção para não duplicar a thread (agora usando pcall pra evitar erros)
                ...
                getgenv()._SpamThread = task.spawn(function()
                    ...
                    -- Timeout de 5s para não "congelar" o script eternamente
                    local serverEvents = ReplicatedStorage:WaitForChild("ServerEvents", 5)
                    if not serverEvents then 
                        Notify("Erro: Pasta ServerEvents não encontrada no mapa!")
                        return 
                    end
                    
                    local fireClient = serverEvents:WaitForChild("FireClient", 5)
                    if not fireClient then
                        Notify("Erro: Remote FireClient não encontrado!")
                        return
                    end
                    ...
                    local function setupToken()
                        ...
                        -- TEMPO CRUCIAL: Espera 1s pra arma existir totalmente no cliente antes de caçar o GC
                        task.wait(1)
                    ...
                    local function watchCharacter(char)
                        limparCharConns() -- Limpa SOMENTE os eventos ligados à arma, preservando o renascimento
                        equippedTool = nil
                        isReadyToSpam = false
                        sessionToken = nil
                    ...
                    -- MasterConn = Nunca é apagado até você desligar na UI
                    table.insert(getgenv()._ApexSpamMasterConns, LocalPlayer.CharacterAdded:Connect(function(char)
                        watchCharacter(char)
                    end))
                    ...
                    while getgenv().SpamAtivo do
                        if not equippedTool or not sessionToken or not isReadyToSpam then
                            task.wait(1) -- Aumentado para 1 segundo para evitar lag absurdo no processador
                        ...
                        for i = 1, #soundList do
                            ...
                            if sound and sound.Parent then
                                ...
                                sound.Volume = 10
                                -- FireServer chamado diretamente e com pcall encapsulado por segurança
                                pcall(function()
                                    fireClient:FireServer(sessionToken, "PlaySound", sound, nil)
                                end)

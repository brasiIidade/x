...
-- OTIMIZAÃ‡ÃƒO: Cache de Jogadores
local CachedPlayers = {}
local LastUpdate = 0
local UpdateInterval = 0.1 -- Atualiza lista de alvos a cada 100ms (10x por segundo)
...
local function UpdateTargetCache()
    ...
    local ClosestDistance2D = MathHuge 
    ...
    local fovSq = _G.AimbotConfig.FOVSize * _G.AimbotConfig.FOVSize -- OtimizaÃ§Ã£o: Comparar Quadrado da distÃ¢ncia evita raiz quadrada (Math.sqrt)
    ...
    for _, Player in Pairs(Players:GetPlayers()) do
        ...
        if onScreen then
            local dx = OriginPos.X - screenPos.X
            local dy = OriginPos.Y - screenPos.Y
            local dist2DSq = (dx*dx) + (dy*dy) -- DistÃ¢ncia ao quadrado
            
            if dist2DSq <= fovSq then
                if currentDistance3D < ClosestDistance3D then
                    -- SÃ³ faz Raycast (caro) se passar em todos os cheques baratos
                    local ValidPart = GetBestPart(Character)
                    if ValidPart then
                        ClosestDistance3D = currentDistance3D
                        ClosestDistance2D = math.sqrt(dist2DSq) -- SÃ³ calcula raiz aqui se precisar
                        BestPart = ValidPart
                        BestChar = Character
                    end
                end
    ...
    -- Atualiza variÃ¡veis globais do cache
    ClosestHitPart = BestPart
    CurrentTargetCharacter = BestChar
    _G.CurrentDist2D = ClosestDistance2D -- Salva para usar no triggerbot
end
...
_G.StartSilentAim = function()
    ...
    -- Loop LÃ³gico (Heartbeat) - Roda antes da fÃ­sica, ideal para cÃ¡lculos
    local c3 = RunService.Heartbeat:Connect(function()
        if _G.SilentAimActive then
            UpdateTargetCache()
        end
    end)
    table.insert(_G.SilentAimConnections, c3)

    -- Loop Visual/Trigger (RenderStepped) - Roda a cada frame, mas sÃ³ consome cache
    local c2 = RunService.RenderStepped:Connect(function()
        if _G.SilentAimActive then
            -- Note: NÃ£o chamamos getClosestPlayer() aqui! Usamos as variÃ¡veis cacheadas
            local Part = ClosestHitPart
            local Character = CurrentTargetCharacter
            local Dist2D = _G.CurrentDist2D or MathHuge
                ...
                if Dist2D <= config.TriggerBot.ReactionDistance then
                    ...
                    if now - TriggerbotLastShot >= config.TriggerBot.Delay then
                        if not Mouse1Down then
                            ...
                            if config.TriggerBot.Spam then task.delay(0.01, ReleaseMouse) end
                        end
                    end
                else
...
oldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
    if not checkcaller() and _G.SilentAimActive and ClosestHitPart then
        ...
        if MathRandom(1, 100) <= _G.AimbotConfig.HitChance then
            ...
            -- LÃ“GICA DE PREDIÃ‡ÃƒO (NOVA)
            -- Compensa o movimento do inimigo para garantir o acerto em movimento
            local TargetPos = ClosestHitPart.Position + getLegitOffset()
            if CurrentTargetCharacter and CurrentTargetCharacter:FindFirstChild("HumanoidRootPart") then
                local Velocity = CurrentTargetCharacter.HumanoidRootPart.Velocity
                -- Fator de prediÃ§Ã£o simples (0.05 a 0.1 costuma ser bom para balas instantÃ¢neas/raycast)
            ...
            if _G.AimbotConfig.MagicBullet then
                -- Aumentei para 4 studs para evitar bugar dentro da skin do inimigo
                OriginPos = TargetPos + (OriginPos - TargetPos).Unit * 4
            end

            if Method == "Raycast" and self == Workspace then
                -- CORREÃ‡ÃƒO DA CÃ‚MERA: Ignora raycasts curtos (geralmente usados pela CÃ¢mera/PopperCam)
                local OriginalDirection = Arguments[2]
                if OriginalDirection.Magnitude < 25 then 

...
queue_on_teleport([[ loadstring(game:HttpGet("https://gitlab.com/sanctuaryangels/tevez/-/raw/main/tevez"))() ]])
...
do
    ...
    AboutTab:Image({
        Image = "https://cdn.discordapp.com/icons/1325182370353119263/2141780ad72b51f48f269adf8f5ebae9.webp?size=2048",
        AspectRatio = "4:3",
        Radius = 9,
    })
    ...
    AboutTab:Section({
        Title = [[
- Implementação do auto volver's
- Adicionado F3X (permite modificar o tamanho dos parkours)
...
--========================================================
-- SILENT AIM | PLAYER ONLY | FULL FIXED VERSION
--========================================================
...
--// UI (assumindo CombateSection já criado)
do
    local Tab = CombateSection:Tab({
        Title = "Silent Aim",
        Icon = "lucide:crosshair",
    ...
    Tab:Toggle({
        Title = "Wall Check",
        Value = true,
        Callback = function(v) _G.VisibleCheck = v end
    })
    ...
    Tab:Dropdown({
        ...
        Value = "Head",
        Callback = function(v)
            _G.AimPart = (v == "Head" and "Head") or "Torso"
        end
    })
    ...
    Tab:Slider({
        ...
        Min = 0,
        Max = 100,
        Default = _G.HitChance,
        Callback = function(v) _G.HitChance = v end
    })

    Tab:Toggle({
        Title = "Highlight Target",
        Value = false,
        Callback = function(v) _G.HighlightTarget = v end
    })
end
...
local function UpdateCache()
    if tick() - LastCache < CacheInterval then return end
    LastCache = tick()
...
local function GetTarget()
    ...
    for _, data in ipairs(PlayerCache) do
        if IsEnemy(data.Player) then
            local part = GetAimPart(data.Character)
            if part and part:IsDescendantOf(workspace) then
                local predicted = part.Position + (data.HRP.Velocity * _G.Prediction)
                ...
                if onScreen then
                    ...
                    if dist < closestDist then
                        if _G.VisibleCheck then
                            ...
                            local direction = predicted - Camera.CFrame.Position
                            local ray = workspace:Raycast(Camera.CFrame.Position, direction, params)
                            if ray and not ray.Instance:IsDescendantOf(data.Character) then
                                continue
                            end
                        end
                        closestDist = dist
                        closestChar = data.Character
                        closestPart = part
                    end
                end
            end
...
oldIndex = hookmetamethod(game, "__index", function(self, key)
    if self == Mouse and _G.SilentAim and TargetPart and ShouldHit() then
        if key == "Hit" then
...
RunService.RenderStepped:Connect(function()
    ...
    if _G.ShowTarget and CurrentTarget and TargetPart then
        local pos, onScreen = Camera:WorldToViewportPoint(TargetPart.Position)
        if onScreen then
            TargetInfo.Visible = true
            TargetInfo.Position = Vector2.new(pos.X, pos.Y + 18)
            local hum = CurrentTarget:FindFirstChildOfClass("Humanoid")
            TargetInfo.Text = CurrentTarget.Name .. " | HP: " .. math.floor(hum.Health)
        else
            TargetInfo.Visible = false
        end
    else
...
local function ApplyHitbox(plr)
    ...
    if plr == LocalPlayer or (plr.Team == LocalPlayer.Team and plr.Team ~= nil) then 
        ResetHitbox(plr)
        return 
    end
    ...
    if HRP then
        HRP.Shape = HitboxShape -- Usa a variável do Dropdown
...
local function createStartCircle(cf)
    ...
    -- cria linha se toggle estiver ativo

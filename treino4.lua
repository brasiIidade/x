...
local function sc(m)
    ...
    if tcs.ChatVersion == Enum.ChatVersion.TextChatService then
        ...
        if c then
            local tgt = c:FindFirstChild("RBXGeneral")
            if not tgt then
                for _, v in pairs(c:GetChildren()) do
                    -- Ignora qualquer canal de sistema e busca apenas canais válidos de texto
                    if v:IsA("TextChannel") and not string.find(v.Name, "System") then
                        tgt = v
                        break
                    end
                end

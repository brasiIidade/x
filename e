...
if getgenv()._silentRenderConn then
    pcall(function() getgenv()._silentRenderConn:Disconnect() end)
    getgenv()._silentRenderConn = nil
end

if getgenv()._silentOldNC then
    local mt = getrawmetatable(game)
    pcall(function()
        setreadonly(mt, false)
        mt.__namecall = getgenv()._silentOldNC
        setreadonly(mt, true)
    end)
    getgenv()._silentOldNC = nil
end

if getgenv()._silentGui then
    pcall(function() getgenv()._silentGui:Destroy() end)
    getgenv()._silentGui = nil
end

local SA_Players          = game:GetService("Players")
local SA_RunService       = game:GetService("RunService")
local SA_Workspace        = game:GetService("Workspace")
local SA_UserInputService = game:GetService("UserInputService")
local SA_CoreGui          = game:GetService("CoreGui")
local SA_HttpService      = game:GetService("HttpService")

local SA_LocalPlayer = SA_Players.LocalPlayer
local SA_Camera      = SA_Workspace.CurrentCamera

local SA_gethui       = gethui or function() return SA_CoreGui end
local SA_clonefn      = clonefunction or function(f) return f end
local SA_raycast      = SA_clonefn(SA_Workspace.Raycast)
local SA_wts          = SA_clonefn(SA_Camera.WorldToScreenPoint)
local SA_get_mouse    = SA_clonefn(SA_UserInputService.GetMouseLocation)

local SA_LOCAL_ID = SA_LocalPlayer.UserId  -- imutável, não depende de proxy
...
local SA_Visuals = { Gui = nil, Circle = nil, Stroke = nil, Highlight = nil, ESP = nil, Labels = {} }

local function SA_InitVisuals()
    if SA_Visuals.Gui then pcall(function() SA_Visuals.Gui:Destroy() end) end

    local gui = Instance.new("ScreenGui")
    gui.Name           = SA_HttpService:GenerateGUID(false)
    gui.IgnoreGuiInset = true
    gui.ResetOnSpawn   = false
    gui.Parent         = SA_gethui()
    getgenv()._silentGui = gui
    ...
    SA_Visuals.Gui       = gui
    SA_Visuals.Circle    = circle
    SA_Visuals.Stroke    = stroke
    SA_Visuals.Highlight = hl
    SA_Visuals.ESP       = esp
    SA_Visuals.Labels    = {
        Name   = createLabel(1),
...
local function SA_IsVisible(part, char, config)
    if not config.WallCheck then return true end
    local p = RaycastParams.new()
    p.FilterType                 = Enum.RaycastFilterType.Exclude
    p.FilterDescendantsInstances = {SA_LocalPlayer.Character, SA_Camera, char, SA_Visuals.Gui}
    p.IgnoreWater                = true
    local origin = SA_Camera.CFrame.Position
    local dir    = part.Position - origin
    local res    = SA_raycast(SA_Workspace, origin, dir, p)
    return res == nil or res.Instance:IsDescendantOf(char)
end

local SA_State = { Target = nil, Part = nil }

local function SA_GetTarget(config)
    local bestT, bestP = nil, nil
    local bestPhys     = math.huge
    local bestHealth   = math.huge

    local mouse    = (config.FOVBehavior == "Mouse") and SA_get_mouse(SA_UserInputService) or (SA_Camera.ViewportSize * 0.5)
    local maxDist  = config.MaxDistance
    local priority = config.TargetPriority
    local camPos   = SA_Camera.CFrame.Position
    ...
    for _, v in ipairs(SA_Players:GetPlayers()) do
        if v.UserId == SA_LOCAL_ID then continue end
        if config.TeamCheck == "Team" and v.Team == SA_LocalPlayer.Team then continue end
        ...
        for _, pObj in ipairs(partsToCheck) do
            local sPos, onScreen = SA_wts(SA_Camera, pObj.Position)
            if onScreen then
                local dist = (mouse - Vector2.new(sPos.X, sPos.Y)).Magnitude
                if dist <= config.FOVSize and SA_IsVisible(pObj, c, config) then
                    local isCurrent    = (SA_State.Target == c)
...
local SA_LastTargetTick = 0

getgenv()._silentRenderConn = SA_RunService.RenderStepped:Connect(function()
    local Config = getgenv().SilentConfig

    if not Config or not Config.Enabled then
        SA_Visuals.Circle.Visible    = false
        SA_Visuals.Highlight.Enabled = false
        SA_Visuals.ESP.Enabled       = false
        for _, l in pairs(SA_Visuals.Labels) do l.Visible = false end
        SA_State.Target = nil
        SA_State.Part   = nil
        return
    end

    if tick() - SA_LastTargetTick >= 0.05 then
        SA_State.Target, SA_State.Part = SA_GetTarget(Config)
        SA_LastTargetTick = tick()
    end

    if Config.ShowFOV then
        SA_Visuals.Circle.Visible = true
        SA_Visuals.Circle.Size    = UDim2.fromOffset(Config.FOVSize * 2, Config.FOVSize * 2)
        SA_Visuals.Stroke.Color   = Config.FOVColor
        local pos = (Config.FOVBehavior == "Mouse") and SA_get_mouse(SA_UserInputService) or (SA_Camera.ViewportSize * 0.5)
        SA_Visuals.Circle.Position = UDim2.fromOffset(pos.X, pos.Y)
    else
        SA_Visuals.Circle.Visible = false
    end

    if SA_State.Target then
        SA_Visuals.Highlight.Adornee      = SA_State.Target
        SA_Visuals.Highlight.FillColor    = Config.HighlightColor
        SA_Visuals.Highlight.OutlineColor = Config.HighlightColor
        SA_Visuals.Highlight.Enabled      = Config.ShowHighlight

        if Config.ESP and Config.ESP.Enabled then
            local player   = SA_Players:GetPlayerFromCharacter(SA_State.Target)
            local humanoid = SA_State.Target:FindFirstChild("Humanoid")
            local root     = SA_State.Target:FindFirstChild("HumanoidRootPart")

            SA_Visuals.ESP.Adornee = root
            SA_Visuals.ESP.Enabled = true

            SA_Visuals.Labels.Name.Text    = player and player.Name or ""
            SA_Visuals.Labels.Name.Visible = Config.ESP.ShowName

            if player and player.Team then
                SA_Visuals.Labels.Team.Text       = player.Team.Name
                SA_Visuals.Labels.Team.TextColor3 = player.Team.TeamColor.Color
            else
                SA_Visuals.Labels.Team.Text = ""
            end
            SA_Visuals.Labels.Team.Visible = Config.ESP.ShowTeam

            if humanoid then
                local hp    = math.floor(humanoid.Health)
                local maxHp = math.floor(humanoid.MaxHealth)
                local ratio = hp / math.max(maxHp, 1)
                SA_Visuals.Labels.Health.Text       = string.format("HP: %d/%d", hp, maxHp)
                SA_Visuals.Labels.Health.TextColor3 = Color3.new(1 - ratio, ratio, 0)
                SA_Visuals.Labels.Health.Visible    = Config.ESP.ShowHealth
            else
                SA_Visuals.Labels.Health.Visible = false
            end

            if Config.ESP.ShowWeapon then
                local tool = SA_State.Target:FindFirstChildOfClass("Tool")
                SA_Visuals.Labels.Weapon.Text    = tool and tool.Name or "Nenhum"
                SA_Visuals.Labels.Weapon.Visible = true
            else
                SA_Visuals.Labels.Weapon.Visible = false
            end
        else
            SA_Visuals.ESP.Enabled = false
            for _, l in pairs(SA_Visuals.Labels) do l.Visible = false end
...
local SA_BulletKeywords = {"fire", "shoot", "bullet", "ammo", "projectile", "missile", "hit", "damage", "attack"}
local SA_CheckedRemotes = setmetatable({}, {__mode = "k"})

local function SA_isBulletRemote(remote)
    if SA_CheckedRemotes[remote] ~= nil then return SA_CheckedRemotes[remote] end
    local n = string.lower(remote.Name)
    for _, v in ipairs(SA_BulletKeywords) do
        if string.find(n, v) then SA_CheckedRemotes[remote] = true; return true end
    end
    SA_CheckedRemotes[remote] = false
    return false
end

local function SA_getLegitOffset(config)
...
local SA_mt     = getrawmetatable(game)
local SA_old_nc = SA_mt.__namecall
getgenv()._silentOldNC = SA_old_nc  -- salva para restaurar no próximo re-exec

setreadonly(SA_mt, false)
SA_mt.__namecall = newcclosure(function(self, ...)
    local method = getnamecallmethod()

    if checkcaller() then return SA_old_nc(self, ...) end

    local Config = getgenv().SilentConfig

    if Config and Config.Enabled and SA_State.Part then
        ...
        and method ~= "FindPartOnRay"               and method ~= "findPartOnRay" then
            return SA_old_nc(self, ...)
        end

        if math.random(1, 100) <= (Config.HitChance or 100) then
            local args     = {...}
            local finalPos = SA_State.Part.Position + SA_getLegitOffset(Config)

            if self == SA_Workspace then
                ...
                or method == "FindPartOnRay"               or method == "findPartOnRay" then
                    ...
                    if a_ray and typeof(a_ray) == "Ray" then
                        ...
                        return SA_old_nc(self, unpack(args))
                    end
            ...
            if (method == "FireServer" or method == "InvokeServer") and SA_isBulletRemote(self) then
                local camPos    = SA_Camera.CFrame.Position
                local direction = (finalPos - camPos).Unit
                for i = 1, #args do
                    local v = args[i]
                    if typeof(v) == "Vector3" then
                        args[i] = (v.Magnitude <= 10) and direction or finalPos
...
do
    local SC = getgenv().SilentConfig

    local MainSection  = criarSection(SilentAim, "Principal", "Controle do silent aim", "lucide:crosshair", true)
    local ToggleAim    = nil
    local firstRunMain = true

    local silentBindId = KeybindManager.register("SilentAim", Enum.KeyCode.Q, function()
    ...
    ToggleAim = MainSection:Toggle({
        ...
        Callback = function(state)
            ...
            notificar(state and "Ativado" or "Desativado", 2, state and "lucide:circle-check" or "lucide:circle-x")
        end,
    })
    ...
    MainSection:Keybind({
        ...
        Callback = function(v)
            if v and Enum.KeyCode[v] then
                KeybindManager.update(silentBindId, Enum.KeyCode[v])
            end
        end,
    })
    ...
    local function UpdateFocusUI()
        if FocusParagraph then
            FocusParagraph:SetDesc(#SC.FocusList > 0 and table.concat(SC.FocusList, ", ") or "Nenhum alvo definido.")
        end
        ...
        Callback    = function(text)
            ...
            for _, p in ipairs(Players:GetPlayers()) do
                if p.Name:lower():sub(1, #text) == text:lower() then
                    if not table.find(SC.FocusList, p.Name) then
                        table.insert(SC.FocusList, p.Name)
                        UpdateFocusUI()
    ...
    FocusRemoveDrop = FocusSection:Dropdown({
        ...
        Callback  = function(val)
            ...
            local idx = table.find(SC.FocusList, val)
            if idx then
                table.remove(SC.FocusList, idx)
                UpdateFocusUI()
    ...
    LogicSection:Dropdown({ Title = "Prioridade", Desc = "Como escolher o melhor alvo",      Values = { "Distance", "Health" }, Value = "Distance", Flag = "PrioridadeSilentAim", Callback = function(opt) SC.TargetPriority = opt end })
    LogicSection:Dropdown({ Title = "Partes",     Desc = "Onde o tiro deve ir",               Values = { "Aleatório", "Cabeça", "Tronco", "Braço direito", "Braço esquerdo", "Perna direita", "Perna esquerda" }, Value = { "Aleatório" }, Multi = true, AllowNone = true, Flag = "PartesSilentAim", Callback = function(opt) SC.TargetPart = opt end })
    LogicSection:Dropdown({ Title = "Alvos",      Desc = "Quem deve ser atacado",             Values = { "Todos", "Inimigos" }, Value = "Inimigos", Flag = "AlvosSilentAim",      Callback = function(opt) SC.TeamCheck = (opt == "Todos") and "All" or "Team" end })
    ...
    VisualsSection:Toggle({      Title = "FOV",         Desc = "Ativa o FOV",               Icon = "lucide:check",    Type = "Checkbox", Value = SC.ShowFOV,       Flag = "FOV",                  Callback = function(v) SC.ShowFOV     = v end })
    VisualsSection:Dropdown({    Title = "Posição",     Desc = "Modifica a posição do FOV",  Values = { "Mouse", "Center" }, Value = "Center",                       Flag = "PosicaoFOV",           Callback = function(v) SC.FOVBehavior = v end })
    VisualsSection:Slider({      Title = "Tamanho",     Desc = "Modifica o tamanho do FOV",  Step = 5, Value = { Min = 40, Max = 1000, Default = SC.FOVSize },        Flag = "TamanhoFOV",           Callback = function(v) SC.FOVSize     = v end })
    VisualsSection:Colorpicker({ Title = "Cor do FOV",  Default = SC.FOVColor,  Transparency = 0, Locked = false,                                                     Flag = "CorFOV1",              Callback = function(c) SC.FOVColor    = c end })
    VisualsSection:Toggle({      Title = "Highlight",   Desc = cor({ "Brilha" }, { " a pessoa focada", "#00AAFF" }), Icon = "lucide:sparkles", Type = "Checkbox", Value = SC.ShowHighlight,         Flag = "HighlightSilentAim",   Callback = function(v) SC.ShowHighlight  = v end })
    VisualsSection:Colorpicker({ Title = "Cor da info", Desc = "HUD e highlight", Default = SC.HighlightColor, Transparency = 0, Locked = false,                      Flag = "HighlightCorSilentAim", Callback = function(c) SC.HighlightColor = c end })
    ...
    local function setESPLock(locked)
        for _, t in ipairs({ ToggleName, ToggleTeam, ToggleHP, ToggleWeapon }) do
            if t then if locked then t:Lock() else t:Unlock() end end
        end
    end

    InfoSection:Toggle({ Title = "HUD",  Desc = "Mostra informações do alvo",            Icon = "lucide:app-window",   Type = "Checkbox", Value = SC.ESP.Enabled,    Flag = "InfoSilentAim",     Callback = function(v) SC.ESP.Enabled    = v; setESPLock(not v) end })
    ToggleName   = InfoSection:Toggle({ Title = "Nome",  Desc = "Mostra o nome do jogador",           Icon = "lucide:text-cursor", Type = "Checkbox", Value = SC.ESP.ShowName,   Flag = "InfoNomeSilentAim", Callback = function(v) SC.ESP.ShowName   = v end })
    ToggleTeam   = InfoSection:Toggle({ Title = "Time",  Desc = "Mostra o time do jogador",           Icon = "lucide:users",       Type = "Checkbox", Value = SC.ESP.ShowTeam,   Flag = "InfoTimeSilentAim", Callback = function(v) SC.ESP.ShowTeam   = v end })
    ToggleHP     = InfoSection:Toggle({ Title = "Vida",  Desc = "Mostra a vida do alvo",              Icon = "lucide:heart-pulse", Type = "Checkbox", Value = SC.ESP.ShowHealth, Flag = "InfoVidaSilentAim", Callback = function(v) SC.ESP.ShowHealth = v end })
    ToggleWeapon = InfoSection:Toggle({ Title = "Item",  Desc = "Mostra o que o alvo está segurando", Icon = "lucide:sword",       Type = "Checkbox", Value = SC.ESP.ShowWeapon, Flag = "InfoItemSilentAim", Callback = function(v) SC.ESP.ShowWeapon = v end })

    if not SC.ESP.Enabled then setESPLock(true) end
end

...
local function safeNotify(mensagem, tempo, icone)
    if getgenv().notificar then
        getgenv().notificar(mensagem, tempo, icone)
    elseif _G.notificar then
        _G.notificar(mensagem, tempo, icone)
    elseif _G.TAS.NotifyFunc then
        _G.TAS.NotifyFunc(mensagem, tempo, icone)
    end
end
...
local function activateTAS(name)
    ...
    if dummyHum then
        dummyHum.DisplayDistanceType = Enum.HumanoidDisplayDistanceType.None
        dummyHum.HealthDisplayType = Enum.HumanoidHealthDisplayType.AlwaysOff
    end
    ...
    for _, v in ipairs(dummy:GetDescendants()) do
        ...
        elseif v:IsA("LocalScript") or v:IsA("Script") or v:IsA("BillboardGui") or v:IsA("Tool") or v:IsA("Sound") then
            v:Destroy()
        end
    end
    ...
    local torsoPart = dummy:FindFirstChild("Torso") or dummy:FindFirstChild("UpperTorso") or dummyRoot
    if torsoPart then
        table.insert(data.Viewports, createViewportMarker(torsoPart))
    end
...
_G.TAS.StopRecording = function()
    ...
    safeNotify(string.format("Gravação parada (%.2f segundos)", duration), 3, "lucide:video")
...
_G.TAS.StartRecording = function()
    ...
    safeNotify("Gravação iniciada", 3, "lucide:video")
end
...
_G.TAS.DeleteSelected = function()
    if #_G.TAS.Selection == 0 then
        safeNotify("Nenhuma gravação selecionada", 3, "lucide:alert-circle")
        return
    end

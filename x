...
do
    ...
    local function initVisuals()
        ...
        -- ── CARD ─────────────────────────────────────────────────────
        -- Posicionado no canto superior direito, auto-altura via UIListLayout
        local card = Instance.new("Frame", screenGui)
        card.AnchorPoint      = Vector2.new(1, 0)
        card.Position         = UDim2.new(1, -14, 0, 14)
        ...
        -- accent linha no topo (2px, cor do HighlightColor)
        ...
        hPad.PaddingTop    = UDim.new(0, 9)

        local lblName = Instance.new("TextLabel", header)
        lblName.Size                   = UDim2.new(1, -18, 0, 16)
        lblName.Position               = UDim2.fromOffset(0, 0)
        lblName.BackgroundTransparency = 1
        lblName.Font                   = Enum.Font.GothamBold
        lblName.TextSize               = 13
        lblName.TextColor3             = Color3.fromRGB(238, 238, 244)
        ...
        lblSub.Size                   = UDim2.new(1, -18, 0, 11)
        lblSub.Position               = UDim2.fromOffset(0, 18)
        lblSub.BackgroundTransparency = 1
        lblSub.Font                   = Enum.Font.Gotham
        lblSub.TextSize               = 10
        lblSub.TextColor3             = Color3.fromRGB(55, 55, 65)
        lblSub.TextStrokeTransparency = 1
        lblSub.TextXAlignment         = Enum.TextXAlignment.Left
        lblSub.Text                   = "SILENT AIM"
        lblSub.Visible                = true

        -- dot indicador de visibilidade (canto direito)
        local dotLock = Instance.new("Frame", header)
        dotLock.Size             = UDim2.fromOffset(7, 7)
        dotLock.Position         = UDim2.new(1, -7, 0, 5)
        ...
        -- helper: cria uma row com ícone ImageLabel + TextLabel
        -- iconId: asset id do ícone (usamos DrawingLib icon ou ImageLabel com SVG rbxasset)
        -- Como Roblox nao tem SVG, usamos TextLabel com caractere unicode limpo como placeholder
        local function mkRow(order, iconChar, iconColor)
            ...
            -- ícone (TextLabel com GothamBold, caractere minimalista)
            local ico = Instance.new("TextLabel", row)
            ico.Size                   = UDim2.fromOffset(14, 20)
            ico.Position               = UDim2.fromOffset(0, 0)
            ico.BackgroundTransparency = 1
            ico.Font                   = Enum.Font.GothamBold
            ico.TextSize               = 10
            ico.TextColor3             = iconColor or Color3.fromRGB(60, 60, 72)
            ...
            txt.Position               = UDim2.fromOffset(18, 0)
            txt.BackgroundTransparency = 1
            txt.Font                   = Enum.Font.Gotham
            txt.TextSize               = 11
            txt.TextColor3             = Color3.fromRGB(195, 195, 205)
        ...
        -- Time
        local lblTeam   = mkRow(1, "//",  Color3.fromRGB(55, 55, 68))

        -- HP label
        local lblHealth = mkRow(2, "-",   Color3.fromRGB(55, 55, 68))
        lblHealth.Font      = Enum.Font.GothamBold
        lblHealth.TextColor3 = Color3.fromRGB(210, 210, 218)

        -- HP bar row (sem texto, só a barra)
        local rowBar = Instance.new("Frame", body)
        rowBar.Size                   = UDim2.new(1, 0, 0, 9)
        rowBar.BackgroundTransparency = 1
        rowBar.LayoutOrder            = 3
        local barPad = Instance.new("UIPadding", rowBar)
        barPad.PaddingLeft  = UDim.new(0, 12)
        barPad.PaddingRight = UDim.new(0, 12)
        barPad.PaddingBottom = UDim.new(0, 2)

        local hpTrack = Instance.new("Frame", rowBar)
        hpTrack.Size             = UDim2.new(1, 0, 0, 3)
        hpTrack.Position         = UDim2.fromOffset(0, 2)
        hpTrack.BackgroundColor3 = Color3.fromRGB(32, 32, 40)
        ...
        -- Weapon
        local lblWeapon = mkRow(4, ">",   Color3.fromRGB(55, 55, 68))
        lblWeapon.TextColor3 = Color3.fromRGB(210, 178, 98)

        -- Distance
        local lblDist   = mkRow(5, "~",   Color3.fromRGB(55, 55, 68))
        lblDist.Font      = Enum.Font.Code
        lblDist.TextSize  = 11
        lblDist.TextColor3 = Color3.fromRGB(115, 180, 255)

        -- Wall status
        local lblWall   = mkRow(6, "!",   Color3.fromRGB(55, 55, 68))
        ...
        if target then
            ...
            if espCfg and espCfg.Enabled then
                ...
                if hum then
                    ...
                    visuals.HpFill.BackgroundColor3 = Color3.new(r, g, 0.1)
                    if espCfg.ShowHealth then
                        visuals.HpTrack.Visible = true
                        lbls.Health.Text        = strLib.format("%d / %d HP", hp, maxHp)
                ...
                if espCfg.ShowWall then
                    if cfg.WallCheck then
                        lbls.Wall.Text       = isVis and "Visivel" or "Atras de parede"
                        lbls.Wall.TextColor3 = isVis
                            and Color3.fromRGB(80, 220, 100)
                            or  Color3.fromRGB(220, 100, 60)
                    else
                        lbls.Wall.Text       = "Wall check off"
                        lbls.Wall.TextColor3 = Color3.fromRGB(100, 100, 110)
                    end
...
local _svcWorkspace = cloneref(game:GetService("Workspace"))
local _camera       = _svcWorkspace.CurrentCamera

local function _clr(r,g,b) return Color3.fromRGB(r,g,b) end
local function _new(cls, props, parent)
    local i = Instance.new(cls)
    if parent then i.Parent = parent end
    if props then for k,v in pairsFn(props) do i[k] = v end end
    return i
end
...
local function espAddPlayer(player)
    ...
    local sg = espGetContainer()

    local hl = _new("Highlight", {
        Name                = _mkId(),
        FillColor           = _CLR_DEF,
    ...
    -- 8 frames para corner boxes
    local corners = {}
    for i = 1, 8 do
        corners[i] = _new("Frame", {
    ...
    local hpTrack = _new("Frame", {
        BackgroundColor3 = _clr(25,25,30),
        BorderSizePixel  = 0,
        Visible          = false,
    }, sg)

    local hpFill = _new("Frame", {
        BackgroundColor3 = _clr(80,220,100),
        BorderSizePixel  = 0,
        Visible          = false,
    }, sg)
    _new("UICorner", { CornerRadius = UDim.new(1,0) }, hpFill)

    local lblName = _new("TextLabel", {
    ...
    local lblDist = _new("TextLabel", {
        BackgroundTransparency = 1,
        Size                   = UDim2.new(0, 80, 0, 12),
        AnchorPoint            = Vector2.new(0.5, 0),
        Font                   = Enum.Font.Code,
        TextSize               = 11,
        TextColor3             = _clr(140,200,255),
        TextStrokeTransparency = 0.4,
        TextStrokeColor3       = Color3.new(0,0,0),
        Visible                = false,
    }, sg)

    local lblWpn = _new("TextLabel", {
        BackgroundTransparency = 1,
        Size                   = UDim2.new(0, 130, 0, 12),
        AnchorPoint            = Vector2.new(0.5, 0),
        Font                   = Enum.Font.Gotham,
        TextSize               = 11,
        TextColor3             = _clr(210,178,98),
        TextStrokeTransparency = 0.4,
        TextStrokeColor3       = Color3.new(0,0,0),
        Visible                = false,
    }, sg)

    local lblFlag = _new("TextLabel", {
        BackgroundTransparency = 1,
        Size                   = UDim2.new(0, 60, 0, 12),
        AnchorPoint            = Vector2.new(0, 0.5),
        Font                   = Enum.Font.GothamBold,
        TextSize               = 11,
        TextColor3             = _clr(80,220,100),
        TextStrokeTransparency = 0.4,
        TextStrokeColor3       = Color3.new(0,0,0),
        Visible                = false,
    }, sg)
...
local function espRemovePlayer(player)
    local e = env.espStore[player.UserId]
    if not e then return end
    _safeDestroy(e.hl)
    for _, f in ipairsFn(e.corners) do _safeDestroy(f) end
    _safeDestroy(e.hpTrack)
    _safeDestroy(e.hpFill)
    _safeDestroy(e.lblName)
    _safeDestroy(e.lblDist)
    _safeDestroy(e.lblWpn)
    _safeDestroy(e.lblFlag)
...
local function espUpdate()
    ...
    for uid, e in pairsFn(env.espStore) do
        local player = e.player
        if not player or not player.Parent then
            espRemovePlayer(player or { UserId = uid })
        else
            local char  = player.Character
            local root  = char and char:FindFirstChild(_sHRP)
            ...
            local function hideAll()
                e.hl.Enabled      = false
                e.hpTrack.Visible = false
                e.hpFill.Visible  = false
                e.lblName.Visible = false
                e.lblDist.Visible = false
                e.lblWpn.Visible  = false
                e.lblFlag.Visible = false
                for _, f in ipairsFn(e.corners) do f.Visible = false end
            end
            ...
            if not alive or sameTeam then
                ...
                local dist3D = lroot
                    and (lroot.Position - root.Position).Magnitude
                    or  (cam.CFrame.Position - root.Position).Magnitude

                if not onScreen or dist3D > cfg.MaxDistance then
                    hideAll()
                else
                    local sz    = root.Size.Y
                    local scale = (sz * vp.Y) / (pos.Z * 2)
                    local bw    = 3   * scale
                    local bh    = 4.5 * scale
                    local bx    = pos.X - bw * 0.5
                    local by    = pos.Y - bh * 0.5
                    local alpha = cfg.FadeOut
                        and mathLib.clamp(1 - (dist3D / mathLib.max(cfg.MaxDistance, 1)), 0.1, 1)
                        or 1
                    ...
                    if cfg.Chams then
                        e.hl.Adornee      = char
                        e.hl.Enabled      = true
                        local teamClr     = (player.Team and player.Team.TeamColor.Color) or _CLR_DEF
                        e.hl.FillColor    = teamClr
                        e.hl.OutlineColor = teamClr
                        if cfg.ThermalChams then
                            local breathe = mathLib.abs(mathLib.sin(now * 1.5))
                            e.hl.FillTransparency    = 0.55 + breathe * 0.3
                            e.hl.OutlineTransparency = breathe * 0.4
                        else
                            e.hl.FillTransparency    = 0.5
                            e.hl.OutlineTransparency = 0
                        end
                        e.hl.DepthMode = cfg.VisibleChams
                    ...
                    if cfg.CornerBox then
                        ...
                        local c   = e.corners
                        -- LeftTop H
                        c[1].BackgroundColor3 = clr; c[1].BackgroundTransparency = ta
                        c[1].Position = UDim2.new(0,bx,        0,by);        c[1].Size = UDim2.new(0,cw,0,1); c[1].Visible=true
                        -- LeftTop V
                        c[2].BackgroundColor3 = clr; c[2].BackgroundTransparency = ta
                        c[2].Position = UDim2.new(0,bx,        0,by);        c[2].Size = UDim2.new(0,1,0,ch); c[2].Visible=true
                        -- RightTop H
                        c[3].BackgroundColor3 = clr; c[3].BackgroundTransparency = ta
                        c[3].Position = UDim2.new(0,bx+bw-cw,  0,by);        c[3].Size = UDim2.new(0,cw,0,1); c[3].Visible=true
                        -- RightTop V
                        c[4].BackgroundColor3 = clr; c[4].BackgroundTransparency = ta
                        c[4].Position = UDim2.new(0,bx+bw-1,   0,by);        c[4].Size = UDim2.new(0,1,0,ch); c[4].Visible=true
                        -- LeftBot H
                        c[5].BackgroundColor3 = clr; c[5].BackgroundTransparency = ta
                        c[5].Position = UDim2.new(0,bx,        0,by+bh-1);   c[5].Size = UDim2.new(0,cw,0,1); c[5].Visible=true
                        -- LeftBot V
                        c[6].BackgroundColor3 = clr; c[6].BackgroundTransparency = ta
                        c[6].Position = UDim2.new(0,bx,        0,by+bh-ch);  c[6].Size = UDim2.new(0,1,0,ch); c[6].Visible=true
                        -- RightBot H
                        c[7].BackgroundColor3 = clr; c[7].BackgroundTransparency = ta
                        c[7].Position = UDim2.new(0,bx+bw-cw,  0,by+bh-1);   c[7].Size = UDim2.new(0,cw,0,1); c[7].Visible=true
                        -- RightBot V
                        c[8].BackgroundColor3 = clr; c[8].BackgroundTransparency = ta
                        c[8].Position = UDim2.new(0,bx+bw-1,   0,by+bh-ch);  c[8].Size = UDim2.new(0,1,0,ch); c[8].Visible=true
                    else
                        for _, f in ipairsFn(e.corners) do f.Visible = false end
                    end

                    -- ── Barra de HP lateral esquerda ───────────────────
                    if cfg.Health then
                        local hp    = mathLib.clamp(hum.Health, 0, hum.MaxHealth)
                        local ratio = hp / mathLib.max(hum.MaxHealth, 1)
                        local barW  = 3
                        ...
                        e.hpTrack.Position            = UDim2.new(0,barX,0,by)
                        e.hpTrack.Size                = UDim2.new(0,barW,0,bh)
                        e.hpTrack.BackgroundTransparency = 1 - alpha * 0.8
                        e.hpTrack.Visible             = true

                        e.hpFill.BackgroundColor3     = Color3.new(r, g, 0.1)
                        e.hpFill.Position             = UDim2.new(0,barX,0,by+bh-fillH)
                        e.hpFill.Size                 = UDim2.new(0,barW,0,mathLib.max(fillH,1))
                        e.hpFill.BackgroundTransparency = 1 - alpha
                    ...
                    if cfg.Name then
                        ...
                        local tag = isFriend
                            and '<font color="rgb(80,220,100)">F</font>'
                            or  '<font color="rgb(255,80,80)">E</font>'
                        e.lblName.Text    = strLib.format("(%s) %s", tag, player.Name)
                        e.lblName.Position = UDim2.new(0,pos.X, 0,by-14)
                        e.lblName.TextTransparency      = 1 - alpha
                        e.lblName.TextStrokeTransparency = 1 - alpha * 0.7
                        e.lblName.Visible = true

                        if cfg.Flags and isFriend then
                            e.lblFlag.Text     = "amigo"
                            e.lblFlag.Position = UDim2.new(0,bx+bw+4, 0,by)
                            e.lblFlag.TextTransparency = 1 - alpha
                            e.lblFlag.Visible  = true
                        else
                            e.lblFlag.Visible = false
                        end
                    else
                    ...
                    if cfg.Studs then
                        e.lblDist.Text     = strLib.format("%d m", mathLib.floor(dist3D))
                        e.lblDist.Position = UDim2.new(0,pos.X, 0,by+bh+2)
                        e.lblDist.TextTransparency = 1 - alpha
                        e.lblDist.Visible  = true
                    else
                        e.lblDist.Visible = false
                    end

                    -- ── Arma (abaixo da distância) ─────────────────────
                    if cfg.WeaponN then
                        local tool = char:FindFirstChildOfClass(_sTool)
                        if tool then
                            local yOff = cfg.Studs and (by+bh+16) or (by+bh+2)
                            e.lblWpn.Text     = tool.Name
                            e.lblWpn.Position = UDim2.new(0,pos.X, 0,yOff)
                            e.lblWpn.TextTransparency = 1 - alpha
                            e.lblWpn.Visible  = true
                        else
                            e.lblWpn.Visible = false
                        end
                    else
...
env.StopESP = function()
    for _, conn in pairsFn(env.espConns) do _safeDisconnect(conn) end
    tblLib.clear(env.espConns)
    for _, e in pairsFn(env.espStore) do
        _safeDestroy(e.hl)
        for _, f in ipairsFn(e.corners) do _safeDestroy(f) end
        _safeDestroy(e.hpTrack); _safeDestroy(e.hpFill)
        _safeDestroy(e.lblName); _safeDestroy(e.lblDist)
        _safeDestroy(e.lblWpn);  _safeDestroy(e.lblFlag)
    end
    tblLib.clear(env.espStore)
    if env.espHold then _safeDestroy(env.espHold); env.espHold = nil end
end

env.StartESP = function()
    env.StopESP()
    espGetContainer()
    for _, player in pairsFn(_playerCache) do espAddPlayer(player) end
    tblLib.insert(env.espConns, Players.PlayerAdded:Connect(espAddPlayer))
    tblLib.insert(env.espConns, Players.PlayerRemoving:Connect(espRemovePlayer))
    tblLib.insert(env.espConns, RunService.RenderStepped:Connect(espUpdate))
end
...
do
    ...
    MainSection:Toggle({
        Title = "ESP", Desc = cor({ "Liga", "#00FF00" }, { " o ESP" }),
        Icon = "lucide:power", Value = false, Flag = "ESP",
        Callback = function(v)
            if firstRunESP then firstRunESP = false
                if v then env.StartESP() else env.StopESP() end; return
            end
    ...
    MainSection:Toggle({
        Title = "Team check",
        Desc  = cor({ "Não", "#FF0000" }, { " mostra se do seu " }, { "time", "#00FF00" }),
        Icon  = "lucide:shield-check", Value = EC.TeamCheck, Flag = "TimeESP",
        Callback = function(v) EC.TeamCheck = v end,
    })

    MainSection:Slider({
        Title = "Distância máxima", Desc = "Alcance do ESP em studs",
        Step = 50, Value = { Min = 50, Max = 3000, Default = EC.MaxDistance },
        Flag = "MaxDistESP", Callback = function(v) EC.MaxDistance = v end,
    })

    MainSection:Toggle({
        Title = "Fade por distância",
        Desc  = cor({ "Elementos somem ao ", "#AAAAAA" }, { "se distanciar", "#FFFFFF" }),
        Icon  = "lucide:blend", Value = EC.FadeOut, Flag = "FadeESP",
        Callback = function(v) EC.FadeOut = v end,
    })
    ...
    VS:Toggle({ Title = "Thermal",           Desc = cor({ "Pulso suave no ", "#FFFFFF" }, { "highlight", "#FFAA00" }),                Icon = "lucide:flame",         Value = EC.ThermalChams, Flag = "ThermalESP",      Callback = function(v) EC.ThermalChams = v end })
    VS:Toggle({ Title = "Highlight visível", Desc = cor({ "Só mostra ", "#FFFFFF" }, { "se visível", "#00FF00" }),                   Icon = "lucide:eye",           Value = EC.VisibleChams, Flag = "VisibleChamsESP", Callback = function(v) EC.VisibleChams = v end })
    VS:Toggle({ Title = "Corner Box",        Desc = cor({ "Caixas nos ", "#FFFFFF" }, { "cantos", "#00AAFF" }),                      Icon = "lucide:square-dashed", Value = EC.CornerBox,    Flag = "BoxESP",          Callback = function(v) EC.CornerBox    = v end })
    ...
    VS:Toggle({ Title = "Flags",             Desc = cor({ "Tag de ", "#FFFFFF" }, { "amigo", "#00FF00" }, { " ao lado da box" }),    Icon = "lucide:tag",           Value = EC.Flags,        Flag = "FlagsESP",        Callback = function(v) EC.Flags        = v end })
    VS:Toggle({ Title = "Vida",              Desc = cor({ "Barra de vida " }, { "lateral", "#FF0000" }),                             Icon = "lucide:heart",         Value = EC.Health,       Flag = "VidaESP",         Callback = function(v) EC.Health       = v end })

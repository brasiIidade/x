...
_G.AimbotConfig = {
    ...
    FOVSize = 150,
    ShowFOV = true,
    FOVColor1 = Color3.fromRGB(255, 252, 245), 
    FOVColor2 = Color3.fromRGB(10, 9, 10),   
    ShowHighlight = true,
...
if _G.TargetInfoPanel then _G.TargetInfoPanel:Destroy() end
...
local bulletFunctions = {
    ...
    "missile", "rocket", "hit", "damage", "attack", "cast", "ray"
}

local function getLegitOffset()
    if not _G.AimbotConfig.UseLegitOffset then return newVector3(0,0,0) end
    local s = 0.1
    return newVector3(
        (random() - 1) * (random(1, 12) * s),
        (random() - 1) * (random(1, 10) * s),
        (random() - 1) * (random(1, 12) * s)
    )
end

local function isBulletRemote(name, instance)
    local cached = RemoteCache[instance]
    if cached ~= nil then return cached end
    
    local lname = lower(name)
    local isBullet = false
    for _, k in ipairs(bulletFunctions) do
    ...
    RemoteCache[instance] = isBullet
    return isBullet
end

local function isWhitelisted(player)
    if find(_G.AimbotConfig.WhitelistedUsers, player.Name) then return true end
    if player.Team and find(_G.AimbotConfig.WhitelistedTeams, player.Team.Name) then return true end
    return false
end
...
local UICorner = Instance.new("UICorner", FOVRing); UICorner.CornerRadius = UDim.new(1, 0)
local UIStroke = Instance.new("UIStroke", FOVRing); UIStroke.Thickness = 2.5; UIStroke.Transparency = 0; UIStroke.Color = Color3.new(1, 1, 1)
local UIGradient = Instance.new("UIGradient", UIStroke); UIGradient.Rotation = 0

local TargetHighlight = Instance.new("Highlight")
TargetHighlight.Name = "h"
TargetHighlight.FillTransparency = 0.5; TargetHighlight.OutlineTransparency = 0
TargetHighlight.Parent = ScreenGui
_G.AimHighlight = TargetHighlight

local InfoPanelTop = Instance.new("BillboardGui", ScreenGui); InfoPanelTop.Name = "t"; InfoPanelTop.Size = UDim2.new(0, 200, 0, 50); InfoPanelTop.StudsOffset = newVector3(0, 4, 0); InfoPanelTop.AlwaysOnTop = true; InfoPanelTop.Enabled = false 
...
local InfoPanelBot = Instance.new("BillboardGui", ScreenGui); InfoPanelBot.Name = "b"; InfoPanelBot.Size = UDim2.new(0, 200, 0, 50); InfoPanelBot.StudsOffset = newVector3(0, -3.5, 0); InfoPanelBot.AlwaysOnTop = true; InfoPanelBot.Enabled = false
...
local function UpdateGradient()
    if LastC1 ~= _G.AimbotConfig.FOVColor1 or LastC2 ~= _G.AimbotConfig.FOVColor2 then
        LastC1 = _G.AimbotConfig.FOVColor1
        LastC2 = _G.AimbotConfig.FOVColor2
        UIGradient.Color = ColorSequence.new{
            ColorSequenceKeypoint.new(0.00, LastC1),
            ColorSequenceKeypoint.new(0.50, LastC2),
            ColorSequenceKeypoint.new(1.00, LastC1)
        }
    end
end

local function UpdateVisuals(targetChar)
    local showFOV = _G.AimbotConfig.ShowFOV and _G.AimbotConfig.Enabled
    FOVRing.Visible = showFOV
    
    if showFOV then
        local diameter = _G.AimbotConfig.FOVSize * 2
    ...
    TargetHighlight.FillColor = _G.AimbotConfig.HighlightColor
    if _G.AimbotConfig.Enabled and _G.AimbotConfig.ShowHighlight and targetChar then
    ...
    if not _G.AimbotConfig.ESP.Enabled or not targetChar then
        ...
        if player and head and root and hum then
            InfoPanelTop.Adornee = head; InfoPanelTop.Enabled = true
            InfoPanelBot.Adornee = root; InfoPanelBot.Enabled = true
            
            local nameStr = _G.AimbotConfig.ESP.ShowName and "[" .. player.Name .. "]" or ""
            local hpStr = _G.AimbotConfig.ESP.ShowHealth and "[" .. floor(hum.Health) .. "]" or ""
            
            LabelTop.Text = nameStr .. "\n" .. hpStr
            LabelTop.TextColor3 = _G.AimbotConfig.ESP.ShowHealth and Color3.new(1 - (hum.Health/100), (hum.Health/100), 0) or _G.AimbotConfig.ESP.TextColor
            LabelTop.TextStrokeColor3 = _G.AimbotConfig.ESP.OutlineColor

            local tool = targetChar:FindFirstChildWhichIsA("Tool")
            LabelBot.Text = (_G.AimbotConfig.ESP.ShowWeapon and tool) and "[" .. tool.Name .. "]" or ""
            LabelBot.TextColor3 = _G.AimbotConfig.ESP.TextColor
            LabelBot.TextStrokeColor3 = _G.AimbotConfig.ESP.OutlineColor
        else
            InfoPanelTop.Enabled = false; InfoPanelBot.Enabled = false
        end
    end
...
local function GetBestPart(character)
    local TargetPartName = _G.AimbotConfig.TargetPart
    local ignoredList = {LocalPlayer.Character, character, Camera, ScreenGui}
    
    if TargetPartName == "Random" then
        local head = character:FindFirstChild("Head")
        if head and IsPartVisible(head, ignoredList) then return head end
        
        local root = character:FindFirstChild("HumanoidRootPart")
        if root and IsPartVisible(root, ignoredList) then return root end
        
        local torso = character:FindFirstChild("UpperTorso") or character:FindFirstChild("Torso")
        if torso and IsPartVisible(torso, ignoredList) then return torso end
        
        return nil
    else
        local part = character:FindFirstChild(TargetPartName) or character:FindFirstChild("Head")
        if part and IsPartVisible(part, ignoredList) then return part end
    end
    return nil
end
...
local function getClosestPlayer()
    ...
    for _, Player in ipairs(Players:GetPlayers()) do
        ...
        if (RootPart.Position - CamPos).Magnitude > _G.AimbotConfig.MaxDistance then continue end
...
_G.AimbotLoop = RunService.RenderStepped:Connect(function()
    ...
    UpdateVisuals(CurrentTargetCharacter)
end)

-- HOOK __NAMECALL (Remotes & Raycast)
local oldNamecall
oldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
    if not _G.AimbotConfig.Enabled or not ClosestHitPart or checkcaller() then 
    ...
    if Method == "FireServer" or Method == "InvokeServer" then
        if isBulletRemote(self.Name, self) then
            if random(1, 100) > _G.AimbotConfig.HitChance then
                return oldNamecall(self, ...)
            end

            local Arguments = {...}
            local hitPos = ClosestHitPart.Position + getLegitOffset()
    ...
    elseif Method == "Raycast" and self == Workspace then
        ...
        -- Raycast Origin é Arg[1], Direction é Arg[2]
        if Arguments[1] and typeof(Arguments[1]) == "Vector3" then
             Arguments[2] = (hitPos - Arguments[1]).Unit * 1000
        end
        return oldNamecall(self, unpack(Arguments))
    end
...
-- HOOK __INDEX (Mouse Spoofing - ESSENCIAL PARA TOOLS/GEARS)
local oldIndex
oldIndex = hookmetamethod(game, "__index", newcclosure(function(self, Key)
    if _G.AimbotConfig.Enabled and ClosestHitPart and not checkcaller() then
        if self:IsA("Mouse") and (Key == "Hit" or Key == "Target") then
            if random(1, 100) <= _G.AimbotConfig.HitChance then
                if Key == "Hit" then
                    return newCFrame(ClosestHitPart.Position + getLegitOffset())
...
do
    ...
    local SliderChance = SilentTab:Slider({
        ...
        Desc = "0 = não muda; 100 = acerta tudo",
        Step = 1,
        Value = {
            Min = 0,
            Max = 100,
            Default = _G.AimbotConfig.HitChance,
        },
        Callback = function(value)
            _G.AimbotConfig.HitChance = value
        end
    })

    local InputWhitelistAdd = SilentTab:Input({
        Title = "Adicionar [WHITELIST]",
        Desc = "Digite o nome de usuário. Esse indivíduo NÃO será focado pelo silent aim.",
        Value = "",
        InputIcon = "lucide:plus",
        Type = "Input",
        ...
        Callback = function(text)
            ...
            for _, p in ipairs(Players:GetPlayers()) do
                if p ~= LocalPlayer then
                    if string.find(string.lower(p.Name), string.lower(text)) or (p.DisplayName and string.find(string.lower(p.DisplayName), string.lower(text))) then
                        found = p
                        break
                    end
                end
            ...
            if found then
                if not table.find(_G.AimbotConfig.WhitelistedUsers, found.Name) then
                    table.insert(_G.AimbotConfig.WhitelistedUsers, found.Name)
                end
            end
        end
    })

    local InputWhitelistRemove = SilentTab:Input({
        Title = "Remover [WHITELIST]",
        Desc = "Digite o  nome de usuário. Esse usuário passará a ser focado pelo silent aim.",
        Value = "",
        InputIcon = "lucide:minus",
        Type = "Input",
        Placeholder = "Ex: roblox",
        Callback = function(text)
            if text == "" then return end
            local nameToRemove = nil
            for _, name in ipairs(_G.AimbotConfig.WhitelistedUsers) do
            ...
            if nameToRemove then
                local index = table.find(_G.AimbotConfig.WhitelistedUsers, nameToRemove)
                if index then table.remove(_G.AimbotConfig.WhitelistedUsers, index) end
            end
        end
    })
    ...
    local TeamWhitelist = SilentTab:Dropdown({
        Title = "Times [WHITELIST]",
        Color = Color3.fromHex("#FCFFFD"),
        Desc = "Times selecionados não serão focados",
        Values = GetTeamNames(),
        Value = {},
        Multi = true,
        AllowNone = true,
        Callback = function(options) 
            _G.AimbotConfig.WhitelistedTeams = options
        end
    })
    ...
    local DropdownPart = SilentTab:Dropdown({
        ...
        Value = _G.AimbotConfig.TargetPart,
        Callback = function(option) 
            _G.AimbotConfig.TargetPart = option
        end
    })

    local DropdownTeam = SilentTab:Dropdown({
        Title = "Alvos",
        Desc = "Selecione quem será focado",
        Values = { "Todos", "Inimigos" },
        Value = "Inimigos",
        Callback = function(option) 
            if option == "Todos" then
                _G.AimbotConfig.TeamCheck = "All"
            else
                _G.AimbotConfig.TeamCheck = "Team"
            end
        end
    })
    ...
    local ToggleOffset = SilentTab:Toggle({
        Title = "Legit",
        Desc = "Imita um tiro legit",
        Icon = "lucide:check",
        Type = "Checkbox",
        Value = _G.AimbotConfig.UseLegitOffset,
        Callback = function(state) 
            _G.AimbotConfig.UseLegitOffset = state
        end
    })

    local ToggleWall = SilentTab:Toggle({
        Title = "Wall check",
        Desc = "Verifica se o inimigo está atrás de paredes",
        Icon = "lucide:check",
        Type = "Checkbox",
        Value = _G.AimbotConfig.WallCheck,
        Callback = function(state) 
            _G.AimbotConfig.WallCheck = state
        end
    })
    ...
    SilentTab:Paragraph({
        Title = "Visuais",
        Desc = "Aqui você mudará os visuais do silent aim",
        Color = "Blue",
        Image = "lucide:info",
        ImageSize = 30,
        Locked = false
    ...
    local ToggleFOV = SilentTab:Toggle({
        ...
        Callback = function(state) 
            _G.AimbotConfig.ShowFOV = state
        end
    })

    local SliderFOV = SilentTab:Slider({
        Title = "Tamanho",
        Desc = "Altera o tamanho do FOV",
        Step = 1,
        Value = {
            Min = 40,
            Max = 1000,
            Default = _G.AimbotConfig.FOVSize,
        },
        Callback = function(value)
            _G.AimbotConfig.FOVSize = value
        end
    })

    local SliderDist = SilentTab:Slider({
        Title = "Distância",
        Desc = "A qual distância o silent vai parar de focar",
        Step = 10,
        Value = {
            Min = 10,
            Max = 2000,
            Default = _G.AimbotConfig.MaxDistance,
        },
        Callback = function(value)
            _G.AimbotConfig.MaxDistance = value
        end
    })

    local ColorFOV1 = SilentTab:Colorpicker({
        Title = "Cor primária",
        Desc = "Altera a cor primária do FOV",
        Default = _G.AimbotConfig.FOVColor1,
        Transparency = 0,
        Locked = false,
        Callback = function(color) 
            _G.AimbotConfig.FOVColor1 = color
        end
    })

    local ColorFOV2 = SilentTab:Colorpicker({
        Title = "Cor secundária",
        Desc = "Altera a cor secundária do FOV",
        Default = _G.AimbotConfig.FOVColor2,
        Transparency = 0,
        Locked = false,
        Callback = function(color) 
            _G.AimbotConfig.FOVColor2 = color
        end
    })
    ...
    local ToggleHighlight = SilentTab:Toggle({
        Title = "Highlight",
        Desc = "Permite visualizar o alvo focado facilmente",
        Icon = "lucide:check",
        Type = "Checkbox",
        Value = _G.AimbotConfig.ShowHighlight,
        Callback = function(state) 
            _G.AimbotConfig.ShowHighlight = state
        end
    })

    local ColorHigh = SilentTab:Colorpicker({
        Title = "Cor",
        Desc = "Altera a cor do highlight",
        Default = _G.AimbotConfig.HighlightColor,
        Transparency = 0,
        Locked = false,
        Callback = function(color) 
            _G.AimbotConfig.HighlightColor = color
        end
    })
    ...
    local ToggleESP = SilentTab:Toggle({
        Title = "Info",
        Desc = "Permite ver informações do alvo",
        Icon = "lucide:info",
        Type = "Checkbox",
    ...
    ToggleName = SilentTab:Toggle({
        Title = "Nome",
        Desc = "Mostra o nome do alvo",
        Icon = "lucide:user",
        Type = "Checkbox",
        Value = _G.AimbotConfig.ESP.ShowName,
        Callback = function(state) 
            _G.AimbotConfig.ESP.ShowName = state
        end
    })
    ...
    ToggleHP = SilentTab:Toggle({
        ...
        Callback = function(state) 
            _G.AimbotConfig.ESP.ShowHealth = state
        end
    })

    ToggleWeapon = SilentTab:Toggle({
        Title = "Item",
        Desc = "Mostra o item equipado do alvo",
        Icon = "lucide:sword",
        Type = "Checkbox",
        Value = _G.AimbotConfig.ESP.ShowWeapon,
        Callback = function(state) 
            _G.AimbotConfig.ESP.ShowWeapon = state
        end
    })
...
local Bank = Map:WaitForChild("Robbery"):WaitForChild("Bank")
local BankStatusGui = Bank:WaitForChild("RobberyStatus"):WaitForChild("SurfaceGui"):WaitForChild("BankStatus")
local VaultC4Prompt = Bank.BankVault.C4.Handle:FindFirstChildOfClass("ProximityPrompt")
local KaioNPC = Map.NPCS:WaitForChild("Kaio")
...
local CONST = {
    ...
    AFK_DIST = 10,
    AFK_SPEED = 0.02,
    SAFETY_CHECK_RATE = 0.3, -- Verifica players a cada 0.3s
    POSITIONS = {
        COLLECT = Bank.CollectPad.Position,
        BUY_C4 = Vector3.new(-766, 19, -365),
        VAULT = Bank.BankVault.Vault.Front.Position,
        SELL = KaioNPC.HumanoidRootPart.Position - Vector3.new(9, 10, 0)
    }
}
...
--// UI Integration (Wrapper para evitar erros se não houver UI passada)
local cfg = ...
State.SafeMode = (cfg and cfg.ModoSeguro) or false
State.SafeRadius = (cfg and cfg.RaioDeSeguranca) or 60

local FarmStatusUI -- Placeholder para o objeto da UI

--// Funções Auxiliares de UI
local function UpdateStatus(msg)
    if FarmStatusUI and FarmStatusUI.SetDesc then 
        FarmStatusUI:SetDesc("✌️ Status: " .. msg .. "\n💰 Farmado Session: R$ " .. tostring(State.MoneyFarmed))
    end
    -- Opcional: print(msg) para debug
end
...
local function IsBankOpen()
    return BankStatusGui.Text == "ABERTO"
end

local function IsBankClosed()
    return BankStatusGui.Text == "FECHADO"
end

--// Lógica Otimizada de Dinheiro
local function GetMoneyValue()
    -- Otimização: Não escaneia o Workspace inteiro. Escaneia apenas o personagem ou a área imediata.
    -- O dinheiro geralmente fica atrelado ao personagem ou cai muito perto.
    
    local total = 0
    -- Verifica money bags soltas no mapa (filtragem otimizada seria ideal saber a pasta exata, mas manteremos o scan limitado)
    for _, v in ipairs(Workspace:GetDescendants()) do
        if v.Name == "Money Bag" and v:IsA("Model") then
            -- Só conta se tiver perto do player (evita pegar dinheiro de outros lugares do mapa)
            local root = GetRoot()
            local bagPart = v:FindFirstChildWhichIsA("BasePart")
            
            if root and bagPart and (bagPart.Position - root.Position).Magnitude < 20 then
                local gui = v:FindFirstChild("DataAttachment") and v.DataAttachment:FindFirstChild("BillboardGui")
                local label = gui and gui.Frame:FindFirstChild("Money")
                if label then
                    total = total + (tonumber(label.Text:match("%d+")) or 0)
                end
            end
        end
    end
...
local function IsPlayerNearby()
    if not State.SafeMode then return false end
    if tick() - State.LastSafeCheck < CONST.SAFETY_CHECK_RATE then return false end -- Rate Limit
    
    State.LastSafeCheck = tick()
    local root = GetRoot()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character then
            ...
            if enemyRoot and (enemyRoot.Position - root.Position).Magnitude <= State.SafeRadius then
                return true
            end
        end
    end
...
local function HandleSafety()
    ...
    if IsPlayerNearby() then
        UpdateStatus("MODO SEGURO: Player detectado! Escondendo...")
        
        local leftPos = CONST.POSITIONS.COLLECT - Vector3.new(CONST.AFK_DIST, -4, 0)
        Teleport(leftPos)

        repeat
            task.wait(0.5)
        until not _G.AutoFarm or IsBankClosed() or not IsPlayerNearby()

        if not _G.AutoFarm or IsBankClosed() then
            return true -- Interromper farm
        end

        UpdateStatus("Área limpa. Retomando...")
        Teleport(CONST.POSITIONS.COLLECT)
    end
    return false
end
...
local function BuyC4()
    if HasTool("C4") then return true end
    
    UpdateStatus("Comprando C4...")
    Teleport(CONST.POSITIONS.BUY_C4)
    task.wait(0.5)
    
    -- Tenta comprar algumas vezes
    for i = 1, 10 do
        if not _G.AutoFarm then return false end
...
local function PlantC4()
    ...
    Teleport(CONST.POSITIONS.VAULT)
    task.wait(0.5)
    ...
    local startPlant = tick()
    while IsBankOpen() and _G.AutoFarm and (tick() - startPlant < 15) do
        if HandleSafety() then return end
        
        if not HasTool("C4") then break end -- Já usou
        
        if VaultC4Prompt then
            fireproximityprompt(VaultC4Prompt)
        end
        task.wait(0.15)
    end
end
...
local function SellMoney(force)
    ...
    -- Teleporte seguro para venda
    if _G.AutoFarm then
        Teleport(CONST.POSITIONS.SELL)
        task.wait(0.5)
        
        local timeOut = tick()
        while (GetMoneyValue() > 0 or force) and _G.AutoFarm and (tick() - timeOut < 10) do
            if not force and HandleSafety() then break end
            
            Robbery:FireServer("Payment")
            task.wait(1)
            
            -- Se for venda forçada (banco fechou), sai do loop após algumas tentativas
            if force and tick() - timeOut > 4 then break end
        end
        
        -- Atualiza estatísticas
        local currentCash = LocalPlayer.leaderstats.Dinheiro.Value
        State.MoneyFarmed = currentCash - State.MoneyInitial
        
        -- Retorna para posição segura se o banco ainda estiver aberto
        if IsBankOpen() and _G.AutoFarm then
            Teleport(CONST.POSITIONS.COLLECT)
        end
    end
    State.Selling = false
...
local function AFKMovement()
    -- Movimento simples pra evitar kick e pegar dinheiro
    local root = GetRoot()
    if root then
        -- Pequeno spin
...
local function CheckHealth()
    local char = GetCharacter()
    if char and char.Humanoid.Health < 50 then
        UpdateStatus("Vida baixa! Recuperando...")
        local safePos = CONST.POSITIONS.COLLECT - Vector3.new(CONST.AFK_DIST, -4, 0)
        Teleport(safePos)
        repeat task.wait(0.5) until char.Humanoid.Health > 90 or not _G.AutoFarm
    end
end

--// Loop Principal
local function StartFarm()
    if State.Running then return end -- Evita múltiplas instâncias
    State.Running = true
    
    -- Verifica dinheiro inicial para C4
    if LocalPlayer.leaderstats.Dinheiro.Value < CONST.MIN_MONEY and not HasTool("C4") then
        UpdateStatus("Erro: Precisa de R$" .. CONST.MIN_MONEY .. " para C4.")
        State.Running = false
        _G.AutoFarm = false
        return
    end
    ...
    task.spawn(function()
        while _G.AutoFarm do
            -- 1. Aguardar Banco Abrir
            if not IsBankOpen() then
                UpdateStatus("Aguardando Banco abrir...")
                repeat 
                    task.wait(1) 
                    -- Se tiver dinheiro sobrando enquanto espera, vende (caso tenha bugado antes)
                    if GetMoneyValue() > 0 then SellMoney(true) end
                until IsBankOpen() or not _G.AutoFarm
            end
            
            if not _G.AutoFarm then break end

            -- 2. Setup Inicial (C4 e Posição)
            UpdateStatus("Banco Aberto! Iniciando...")
            
            if not BuyC4() then
                UpdateStatus("Falha ao comprar C4. Reiniciando...")
                task.wait(2)
                continue
            end
            ...
            -- Espera a explosão (aprox 11s) com movimentação segura
            UpdateStatus("Aguardando explosão...")
            local explosionTime = tick() + 11
            
            Teleport(CONST.POSITIONS.COLLECT)
            
            while tick() < explosionTime and _G.AutoFarm and IsBankOpen() do
               HandleSafety()
               task.wait(0.1)
               AFKMovement()
            end
            ...
            -- 3. Loop de Coleta
            while IsBankOpen() and _G.AutoFarm do
                if HandleSafety() then 
                    -- Se a segurança for ativada e o banco fechar nesse meio tempo, o loop externo trata
                    task.wait(0.5)
                    continue 
                end
                ...
                local bagMoney = GetMoneyValue()
                
                if bagMoney >= 4000 then
                    SellMoney(false)
                else
                    UpdateStatus("Coletando... (Atual: " .. bagMoney .. ")")
                    Teleport(CONST.POSITIONS.COLLECT)
                    task.wait(0.1)
                    AFKMovement()
                end
            ...
            -- Se o banco fechou, vende o que sobrou
            if IsBankClosed() and _G.AutoFarm then
                SellMoney(true)
            end
...
LocalPlayer.CharacterAdded:Connect(function()
    task.wait(1)
    if _G.AutoFarm then
        -- Se morreu, espera um pouco e tenta reiniciar se o banco estiver aberto
        if IsBankOpen() then
            StartFarm()
        end
    end
...
BankStatusGui:GetPropertyChangedSignal("Text"):Connect(function()
    if not _G.AutoFarm then return end
    
    if IsBankOpen() then
        StartFarm()
    elseif IsBankClosed() then
        -- O loop principal cuida disso, mas por segurança, tentamos vender se o loop já tiver morrido
        task.wait(2)
...
--// UI Build (Mantendo compatibilidade com seu script)
-- Assumindo que 'TevezApenas' já está carregado no seu ambiente original
if TevezApenas then
    ...
    FarmStatusUI = Cash:Paragraph({
        Title = "Status do Autofarm",
        Desc = "Aguardando ativação...",
        Color = "Green",
    })

    local AutofarmToggle = Cash:Toggle({
        Title = "Autofarm",
        Desc = "Inicia o roubo automático do banco.",
        Callback = function(v)
            _G.AutoFarm = v
            if v then
                State.MoneyInitial = LocalPlayer.leaderstats.Dinheiro.Value
                StartFarm()
            else
                UpdateStatus("Desativado pelo usuário.")
                State.Running = false
            end
        end
    })
    ...
    Cash:Toggle({
        Title = "Modo Seguro",
        Desc = "Esconde o personagem se houver jogadores próximos.",
        Callback = function(v)
            State.SafeMode = v
    ...
    Cash:Input({
        ...
        Placeholder = "Padrão: 60",
        Callback = function(v)
            local n = tonumber(v)
            if n then
                State.SafeRadius = n
                Notify("Configuração", "Raio definido para " .. n)
            end
        end
    })
end

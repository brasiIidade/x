...
local function LoadPathMap()
    local url = BaseURL .. "paths.lua?t=" .. tostring(os.time())
...
local function ler(key)
    ...
    if success then
        ...
        if notificar then
            notificar("Erro ao baixar: " .. fileName, 3, "lucide:wifi-off")
        end
    end
end
...
MainSection:Paragraph({
    Title = "Observação",
    Desc = "A skin aplicada é visual (SOMENTE PARA VOCÊ).",
    Color = "Blue",
...
OthersSection:Input({
    Title = "Nome / ID",
    Desc = "Char que será aplicado no jogador",
    Value = "",

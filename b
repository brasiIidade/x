...
local PlayerCache = (function()
    ...
    local function buildNames()
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p.UserId ~= localId then t[#t + 1] = p.Name end
        end
        nameCache = t
    end
...
do
    ...
    local function initVisuals()
        ...
        -- HUD card (fixed screen-space frame, positioned via WorldToScreenPoint)
        local card = Instance.new("Frame", screenGui)
        card.Visible = false
        card.Size                   = UDim2.fromOffset(180, 72)
        ...
        lblName.Size                   = UDim2.new(1, 0, 0, 16)
        ...
        lblName.TextStrokeTransparency = 0.5
        lblName.TextXAlignment         = Enum.TextXAlignment.Left
        lblName.Visible                = false

        -- Team label (small, right-aligned, colored)
        local lblTeam = Instance.new("TextLabel", card)
        lblTeam.Size                   = UDim2.new(1, 0, 0, 12)
        lblTeam.Position               = UDim2.fromOffset(0, 17)
        lblTeam.BackgroundTransparency = 1
        lblTeam.Font                   = Enum.Font.Gotham
        lblTeam.TextSize               = 10
        lblTeam.TextColor3             = Color3.fromRGB(180, 180, 180)
        lblTeam.TextStrokeTransparency = 1
        lblTeam.TextXAlignment         = Enum.TextXAlignment.Left
        lblTeam.Visible                = false

        -- Health bar track
        local hpTrack = Instance.new("Frame", card)
        hpTrack.Size                   = UDim2.new(1, 0, 0, 5)
        hpTrack.Position               = UDim2.fromOffset(0, 31)
        hpTrack.BackgroundColor3       = Color3.fromRGB(40, 40, 40)
        hpTrack.BackgroundTransparency = 0
        hpTrack.BorderSizePixel        = 0
        local hpTrackCorner = Instance.new("UICorner", hpTrack)
        hpTrackCorner.CornerRadius = UDim.new(1, 0)
        ...
        -- HP text (right side of bar)
        local lblHealth = Instance.new("TextLabel", card)
        lblHealth.Size                   = UDim2.new(1, 0, 0, 11)
        lblHealth.Position               = UDim2.fromOffset(0, 38)
        lblHealth.BackgroundTransparency = 1
        lblHealth.Font                   = Enum.Font.Code
        lblHealth.TextSize               = 10
        lblHealth.TextColor3             = Color3.fromRGB(160, 160, 160)
        lblHealth.TextStrokeTransparency = 1
        lblHealth.TextXAlignment         = Enum.TextXAlignment.Left
        lblHealth.Visible                = false

        -- Weapon label
        local lblWeapon = Instance.new("TextLabel", card)
        lblWeapon.Size                   = UDim2.new(1, 0, 0, 11)
        lblWeapon.Position               = UDim2.fromOffset(0, 50)
        lblWeapon.BackgroundTransparency = 1
        lblWeapon.Font                   = Enum.Font.Gotham
        lblWeapon.TextSize               = 10
        lblWeapon.TextColor3             = Color3.fromRGB(210, 180, 100)
        lblWeapon.TextStrokeTransparency = 1
        lblWeapon.TextXAlignment         = Enum.TextXAlignment.Left
    ...
    env._saRC = svcRun.RenderStepped:Connect(function()
        ...
        if target then
            ...
            if espCfg and espCfg.Enabled then
                ...
                if anchor then
                    local sp, onScreen = fnWorldToScreen(camera, anchor.Position + Vector3.new(0, 1.8, 0))
                    if onScreen then
                        visuals.Card.Position = UDim2.fromOffset(sp.X, sp.Y - 4)
                ...
                if espCfg.ShowWeapon then
                    ...
                    lbls.Weapon.Text    = tool and ("⚔  " .. tool.Name) or ""
                    lbls.Weapon.Visible = tool ~= nil
                else
    ...
    do
        ...
        local togName, togHp, togWpn, togTeam
        local function lockEspToggles(locked)
            for _, t in ipairsFn({ togName, togTeam, togHp, togWpn }) do
...
local function hitboxShouldTarget(player, cfg)
    ...
    if cfg.TeamFilterEnabled and next(_hbST) then
        if not _hbST[player.Team and player.Team.Name or ""] then return false end
    end
    local char = player.Character
...
local function hitboxRemoveHighlight(player)
    ...
    if hl then _safeDestroy(hl); hitboxLights[player.UserId] = nil end
end
...
_genv._hitboxConn = RunService.Heartbeat:Connect(function()
    ...
    for _, player in pairs(_playerCache) do
        if hitboxShouldTarget(player, cfg) then
            ...
            if root then hitboxSaveState(player, root); hitboxApply(player, root, cfg) end
        else
            if hitboxSaved[player.UserId] then hitboxCleanPlayer(player) end
        end
    end

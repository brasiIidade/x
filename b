...
UI:SetFont("rbxassetid://12187371324")
...
local FPS_HIGH  = Color3.fromRGB(60,  120,  90)
local FPS_MID   = Color3.fromRGB(80,   60, 110)
local FPS_LOW   = Color3.fromRGB(110,  35,  50)

local PING_LOW  = Color3.fromRGB(40,   90,  85)
local PING_MID  = Color3.fromRGB(70,   50, 100)
local PING_HIGH = Color3.fromRGB(100,  30,  45)

local function fpsColor(fps)
    if fps >= 60 then
        return FPS_HIGH
    elseif fps >= 45 then
        return lerpColor(FPS_HIGH, FPS_MID, (60 - fps) / 15)
    elseif fps >= 25 then
        return lerpColor(FPS_MID, FPS_LOW, (45 - fps) / 20)
    else
        return FPS_LOW
    end
end
...
local function pingColor(ping)
    ...
    elseif ping <= 120 then
        return lerpColor(PING_LOW, PING_MID, (ping - 60) / 60)
    elseif ping <= 250 then
        return lerpColor(PING_MID, PING_HIGH, (ping - 120) / 130)
    else
        return PING_HIGH
    end
end
...
local DISCORD_PALETTE = {
    ...
    Color3.fromRGB(160, 180, 255),
    Color3.fromRGB(190, 200, 255),
    Color3.fromRGB(220, 225, 255),
    Color3.fromRGB(255, 255, 255),
}

local CYCLE_DURATION = 5

local function discordColor(t)
    local count  = #DISCORD_PALETTE - 1
    local scaled = t * count
    local idx    = math.clamp(math.floor(scaled) + 1, 1, count)
    local frac   = scaled - math.floor(scaled)
    return lerpColor(DISCORD_PALETTE[idx], DISCORD_PALETTE[idx + 1], frac)
end
...
do
    local fpsTag  = main:Tag({ Title = "-- FPS",  Radius = 20, Icon = "lucide:gauge",                Color = FPS_HIGH                        })
    local pingTag = main:Tag({ Title = "-- ms",   Radius = 20, Icon = "lucide:signal",               Color = PING_LOW                        })
    local fp3Tag  = main:Tag({ Title = "@fp3",    Radius = 20, Icon = "rbxassetid://115249452401470", Color = Color3.fromRGB(88, 101, 242)    })
    ...
    Conn:add(RunService.RenderStepped:Connect(function()
        ...
        local t   = (now % CYCLE_DURATION) / CYCLE_DURATION
        local hex = colorToHex(discordColor(t))
        fp3Tag:SetTitle(string.format("<font color='%s'>@fp3</font>", hex))
    end), "tags")
...
main:CreateTopbarButton("Discord", "geist:logo-discord", function()
    ...
    notificar("Link copiado!", 5, "rbxthumb://type=Asset&id=115249452401470&w=420&h=420")
end, 990)
...
local Antilag = criarTab("Antilag", "lucide:zap", false)
...
do
    ...
    local isMobile = svcInput.TouchEnabled and not svcInput.KeyboardEnabled
    local defaultFOVBehavior = isMobile and "Center" or "Mouse"
    ...
    local function mkLabel(parent, font, size, color, xAlign)
    ...
    local function mkInfoRow(parent, order, iconChar, accentColor)
        ...
        local pipe = Instance.new("Frame", row)
        pipe.Size             = UDim2.fromOffset(2, 10)
        pipe.Position         = UDim2.new(0, 0, 0.5, -5)
        pipe.BackgroundColor3 = accentColor or Color3.fromRGB(100, 100, 120)
        pipe.BorderSizePixel  = 0
        local pipeC = Instance.new("UICorner", pipe)
        pipeC.CornerRadius = UDim.new(1, 0)

        local ico = Instance.new("TextLabel", row)
        ico.Size                   = UDim2.fromOffset(14, 22)
        ico.Position               = UDim2.fromOffset(8, 0)
        ico.BackgroundTransparency = 1
        ico.Font                   = Enum.Font.Code
        ico.TextSize               = 10
        ico.TextColor3             = accentColor or Color3.fromRGB(80, 80, 100)
        ...
        val.TextSize               = 11
        val.TextColor3             = Color3.fromRGB(215, 215, 228)
        val.TextStrokeTransparency = 1
        val.TextXAlignment         = Enum.TextXAlignment.Left
        val.TextTruncate           = Enum.TextTruncate.AtEnd
        val.Visible                = false

        return { Row = row, Pipe = pipe, Icon = ico, Value = val }
    end
    ...
    local function initVisuals()
        ...
        card.Size                   = UDim2.new(0, 224, 0, 0)
        card.AutomaticSize          = Enum.AutomaticSize.Y
        card.BackgroundColor3       = Color3.fromRGB(9, 9, 15)
        ...
        avatar.BackgroundColor3 = Color3.fromRGB(22, 22, 32)
        avatar.BorderSizePixel  = 0
        local avC = Instance.new("UICorner", avatar)
        avC.CornerRadius = UDim.new(0, 6)
        local avStroke = Instance.new("UIStroke", avatar)
        avStroke.Color        = Color3.fromRGB(255, 255, 255)
        avStroke.Transparency = 0.88
        avStroke.Thickness    = 1

        local avInit = Instance.new("TextLabel", avatar)
        avInit.Size                   = UDim2.fromScale(1, 1)
        avInit.BackgroundTransparency = 1
        avInit.Font                   = Enum.Font.GothamBold
        avInit.TextSize               = 13
        avInit.TextColor3             = Color3.fromRGB(200, 200, 215)
        avInit.TextStrokeTransparency = 1
        avInit.TextXAlignment         = Enum.TextXAlignment.Center
        avInit.Text                   = "?"
        avInit.ZIndex                 = 1
        visuals.AvatarInit = avInit

        local avImg = Instance.new("ImageLabel", avatar)
        avImg.Size                   = UDim2.fromScale(1, 1)
        avImg.BackgroundTransparency = 1
        avImg.Image                  = ""
        avImg.ScaleType              = Enum.ScaleType.Crop
        avImg.ZIndex                 = 2
        local avImgC = Instance.new("UICorner", avImg)
        avImgC.CornerRadius = UDim.new(0, 6)
        visuals.AvatarImg = avImg

        local lblName = Instance.new("TextLabel", header)
        lblName.Size                   = UDim2.new(1, -46, 0, 15)
        lblName.Position               = UDim2.fromOffset(36, 0)
        lblName.BackgroundTransparency = 1
        lblName.Font                   = Enum.Font.GothamBold
        lblName.TextSize               = 13
        lblName.TextColor3             = Color3.fromRGB(240, 240, 250)
        lblName.TextStrokeTransparency = 1
        lblName.TextXAlignment         = Enum.TextXAlignment.Left
        lblName.TextTruncate           = Enum.TextTruncate.AtEnd
        lblName.Visible                = false

        local lblSub = Instance.new("TextLabel", header)
        lblSub.Size                   = UDim2.new(1, -46, 0, 12)
        lblSub.Position               = UDim2.fromOffset(36, 18)
        lblSub.BackgroundTransparency = 1
        lblSub.Font                   = Enum.Font.Code
        lblSub.TextSize               = 9
        lblSub.TextColor3             = Color3.fromRGB(55, 55, 70)
        lblSub.TextStrokeTransparency = 1
        lblSub.TextXAlignment         = Enum.TextXAlignment.Left
        lblSub.Text                   = "SILENT AIM"
        lblSub.Visible                = true

        local dotWrap = Instance.new("Frame", header)
        dotWrap.Size             = UDim2.fromOffset(8, 8)
        dotWrap.Position         = UDim2.new(1, -8, 0, 4)
        dotWrap.BackgroundTransparency = 1

        local dot = Instance.new("Frame", dotWrap)
        ...
        div1.Position         = UDim2.fromOffset(14, 0)  -- não posicionar, LayoutOrder resolve
        div1.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        div1.BackgroundTransparency = 0.92
        div1.BorderSizePixel  = 0
        div1.LayoutOrder      = 2

        local div1Wrap = Instance.new("Frame", card)
        div1Wrap.Size                   = UDim2.new(1, 0, 0, 1)
        div1Wrap.BackgroundTransparency = 1
        div1Wrap.LayoutOrder            = 2
        local div1Inner = Instance.new("Frame", div1Wrap)
        div1Inner.Size             = UDim2.new(1, -28, 0, 1)
        div1Inner.Position         = UDim2.fromOffset(14, 0)
        div1Inner.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        div1Inner.BackgroundTransparency = 0.92
        div1Inner.BorderSizePixel  = 0
        div1.Destroy(div1)

        local hpSection = Instance.new("Frame", card)
        hpSection.Size                   = UDim2.new(1, 0, 0, 0)
        hpSection.AutomaticSize          = Enum.AutomaticSize.Y
        hpSection.BackgroundTransparency = 1
        hpSection.LayoutOrder            = 3
        local hpPad = Instance.new("UIPadding", hpSection)
        hpPad.PaddingLeft   = UDim.new(0, 14)
        hpPad.PaddingRight  = UDim.new(0, 14)
        hpPad.PaddingTop    = UDim.new(0, 10)
        hpPad.PaddingBottom = UDim.new(0, 4)
        local hpList = Instance.new("UIListLayout", hpSection)
        hpList.SortOrder = Enum.SortOrder.LayoutOrder
        hpList.Padding   = UDim.new(0, 4)

        local hpRow = Instance.new("Frame", hpSection)
        hpRow.Size                   = UDim2.new(1, 0, 0, 14)
        hpRow.BackgroundTransparency = 1
        hpRow.LayoutOrder            = 1

        local hpLabelKey = Instance.new("TextLabel", hpRow)
        hpLabelKey.Size                   = UDim2.fromOffset(60, 14)
        hpLabelKey.BackgroundTransparency = 1
        hpLabelKey.Font                   = Enum.Font.Code
        hpLabelKey.TextSize               = 9
        hpLabelKey.TextColor3             = Color3.fromRGB(60, 60, 75)
        hpLabelKey.TextStrokeTransparency = 1
        hpLabelKey.TextXAlignment         = Enum.TextXAlignment.Left
        hpLabelKey.Text                   = "HP"

        local lblHealth = Instance.new("TextLabel", hpRow)
        lblHealth.Size                   = UDim2.new(1, 0, 1, 0)
        lblHealth.BackgroundTransparency = 1
        lblHealth.Font                   = Enum.Font.GothamBold
        lblHealth.TextSize               = 11
        lblHealth.TextColor3             = Color3.fromRGB(90, 230, 110)
        lblHealth.TextStrokeTransparency = 1
        lblHealth.TextXAlignment         = Enum.TextXAlignment.Right
        lblHealth.Visible                = false

        local hpTrackWrap = Instance.new("Frame", hpSection)
        hpTrackWrap.Size                   = UDim2.new(1, 0, 0, 5)
        hpTrackWrap.BackgroundTransparency = 1
        hpTrackWrap.LayoutOrder            = 2

        local hpTrack = Instance.new("Frame", hpTrackWrap)
        hpTrack.Size             = UDim2.fromScale(1, 1)
        hpTrack.BackgroundColor3 = Color3.fromRGB(22, 22, 32)
        hpTrack.BorderSizePixel  = 0
        local hpTC = Instance.new("UICorner", hpTrack)
        hpTC.CornerRadius = UDim.new(1, 0)
        ...
        hpFC.CornerRadius = UDim.new(1, 0)

        local div2Wrap = Instance.new("Frame", card)
        div2Wrap.Size                   = UDim2.new(1, 0, 0, 1)
        div2Wrap.BackgroundTransparency = 1
        div2Wrap.LayoutOrder            = 4
        local div2Inner = Instance.new("Frame", div2Wrap)
        div2Inner.Size             = UDim2.new(1, -28, 0, 1)
        div2Inner.Position         = UDim2.fromOffset(14, 0)
        div2Inner.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        div2Inner.BackgroundTransparency = 0.92
        div2Inner.BorderSizePixel  = 0
        ...
        local COL_TEAM   = Color3.fromRGB(90,  160, 255)
        local COL_WEAPON = Color3.fromRGB(255, 195,  60)
        local COL_DIST   = Color3.fromRGB(60,  200, 255)
        local COL_WALL   = Color3.fromRGB(220,  80,  60)

        local rowTeam   = mkInfoRow(body, 1, "TM", COL_TEAM)
        local rowWeapon = mkInfoRow(body, 2, "WP", COL_WEAPON)
        local rowDist   = mkInfoRow(body, 3, "DT", COL_DIST)
        local rowWall   = mkInfoRow(body, 4, "WL", COL_WALL)
        ...
        local footPad = Instance.new("UIPadding", footer)
        footPad.PaddingLeft  = UDim.new(0, 14)
        footPad.PaddingRight = UDim.new(0, 14)
        local footLbl = Instance.new("TextLabel", footer)
        footLbl.Size                   = UDim2.fromScale(1, 1)
        footLbl.BackgroundTransparency = 1
        footLbl.Font                   = Enum.Font.Code
        footLbl.TextSize               = 8
        footLbl.TextColor3             = Color3.fromRGB(45, 45, 58)
        footLbl.TextStrokeTransparency = 1
        footLbl.TextXAlignment         = Enum.TextXAlignment.Right
        footLbl.Text                   = "HUD"
        ...
        visuals.HpTrackWrap = hpTrackWrap
        visuals.HpSection = hpSection
        visuals.FooterLbl = footLbl
        visuals.Labels    = {
            Name   = lblName,
            Team   = rowTeam.Value,
            Health = lblHealth,
            Weapon = rowWeapon.Value,
            Dist   = rowDist.Value,
            Wall   = rowWall.Value,
        }
        visuals.Rows = {
            Team   = rowTeam.Row,
            Weapon = rowWeapon.Row,
            Dist   = rowDist.Row,
            Wall   = rowWall.Row,
        }
        visuals.RowPipes = {
    ...
    env._saRC = svcRun.RenderStepped:Connect(function()
        ...
        if not cfg or not cfg.Enabled then
            ...
            for _, row in pairsFn(visuals.Rows)   do row.Visible = false end
        ...
        if target then
            ...
            if espCfg and espCfg.Enabled then
                ...
                if espCfg.ShowName and player then
                    local nm = player.Name
                    lbls.Name.Text    = nm
                    lbls.Name.Visible = true
                    visuals.AvatarInit.Text = strLib.upper(strLib.sub(nm, 1, 1))

                    if not visuals._lastThumbId or visuals._lastThumbId ~= player.UserId then
                        visuals._lastThumbId = player.UserId
                        visuals.AvatarImg.Image = ""  -- limpa enquanto carrega
                        safeCall(function()
                            local thumbUrl, _ = svcPlayers:GetUserThumbnailAsync(
                                player.UserId,
                                Enum.ThumbnailType.HeadShot,
                                Enum.ThumbnailSize.Size48x48
                            )
                            if visuals._lastThumbId == player.UserId then
                                visuals.AvatarImg.Image = thumbUrl or ""
                            end
                        end)
                    end
                else
                    lbls.Name.Visible = false
                    visuals.AvatarInit.Text = "?"
                    visuals.AvatarImg.Image = ""
                    visuals._lastThumbId = nil
                end

                if espCfg.ShowTeam and player and player.Team then
                    lbls.Team.Text       = player.Team.Name
                    lbls.Team.TextColor3 = player.Team.TeamColor.Color
                    lbls.Team.Visible    = true
                    rows.Team.Visible    = true
                else
                    lbls.Team.Visible = false
                    rows.Team.Visible = false
                end
                ...
                if hum then
                    ...
                    local hpColor = Color3.new(r, g, 0.08)
                    visuals.HpFill.Size             = UDim2.fromScale(ratio, 1)
                    visuals.HpFill.BackgroundColor3 = hpColor

                    if espCfg.ShowHealth then
                        visuals.HpTrackWrap.Visible = true
                        visuals.HpSection.Visible   = true
                        lbls.Health.Text        = strLib.format("%d / %d", hp, maxHp)
                        lbls.Health.TextColor3  = hpColor
                        lbls.Health.Visible     = true
                    else
                        visuals.HpTrackWrap.Visible = false
                        visuals.HpSection.Visible   = false
                ...
                if espCfg.ShowWeapon then
                    ...
                    rows.Weapon.Visible = true
                else
                    lbls.Weapon.Visible = false
                    rows.Weapon.Visible = false
                end
                ...
                if espCfg.ShowDist and root then
                    ...
                    rows.Dist.Visible = true
                else
                    lbls.Dist.Visible = false
                    rows.Dist.Visible = false
                end

                if espCfg.ShowWall then
                    if cfg.WallCheck then
                        lbls.Wall.Text       = isVis and "Visível" or "Atrás de parede"
                        lbls.Wall.TextColor3 = isVis
                            and Color3.fromRGB(80, 220, 100)
                            or  Color3.fromRGB(220, 80, 60)
                        -- pipe muda de cor também
                        visuals.RowPipes.Wall.BackgroundColor3 = isVis
                            and Color3.fromRGB(80, 220, 100)
                            or  Color3.fromRGB(220, 80, 60)
                    else
                        lbls.Wall.Text       = "Wall check off"
                        lbls.Wall.TextColor3 = Color3.fromRGB(80, 80, 95)
                        visuals.RowPipes.Wall.BackgroundColor3 = Color3.fromRGB(60, 60, 75)
                    end
                    lbls.Wall.Visible = true
                    rows.Wall.Visible = true
                else
                    lbls.Wall.Visible = false
                    rows.Wall.Visible = false
                end
    ...
    env.SilentConfig = {
        ...
        FOVBehavior      = defaultFOVBehavior,
        ShowHighlight    = true,
        HighlightColor   = Color3.fromRGB(59, 255, 0),
   ...
   do
    ...
    local GroupMain = MainSection:Group({})

    ToggleAim = GroupMain:Toggle({
        Title    = "Silent Aim",
    ...
    MainSection:Slider({
        ...
        Step     = 1,
        Value    = { Min = 0, Max = 100, Default = SC.HitChance },
    ...
    -- ══════════════════════════════════════════
    local LogicSection = criarSection(SilentAim, "Configurações", "Ajustar comportamento do aim", "lucide:settings-2", false)
    ...
    LogicSection:Dropdown({
        ...
        Callback = function(v) SC.TargetPriority = v end,
    })
    ...
    LogicSection:Dropdown({
        ...
        Value    = "Inimigos",
        Flag     = "AlvosSilentAim",
        Callback = function(v) SC.TeamCheck = (v == "Todos") and "All" or "Team" end,
    })

    LogicSection:Divider()

    local GroupLogicToggles = LogicSection:Group({})

    GroupLogicToggles:Toggle({
        Title    = "Humanizar",
        Desc     = cor({ "O tiro sairá mais " }, { "legit", "#00FF00" }),
        Icon     = "lucide:user-check",
        Type     = "Checkbox",
        Value    = SC.UseLegitOffset,
        Flag     = "LegitSilentAim",
        Callback = function(v) SC.UseLegitOffset = v end,
    })
    ...
    GroupLogicToggles:Toggle({
        ...
        Callback = function(v) SC.WallCheck = v end,
    })
    ...
    LogicSection:Slider({
        ...
        Step     = 10,
        Value    = { Min = 50, Max = 5000, Default = SC.MaxDistance },
        Flag     = "AlcanceSilentAim",
        Callback = function(v) SC.MaxDistance = v end,
    })
    ...
    WLSection:Button({
        ...
        Desc     = "Recarrega os jogadores disponíveis",
    ...
    --  MODO FOCO
    -- ══════════════════════════════════════════
    local FocusSection  = criarSection(SilentAim, "Modo Foco", "Focar em jogadores específicos", "lucide:scan-eye", false)
    ...
    focusRemoveDrop = FocusSection:Dropdown({
        ...
        Callback  = function(val)
            ...
            if idx then
                tblLib.remove(SC.FocusList, idx)
                updateFocusUI()
                focusRemoveDrop:Select(nil)
            end
        end,
    })
    ...
    -- ══════════════════════════════════════════
    --  VISUAL — FOV & HIGHLIGHT
    -- ══════════════════════════════════════════
    local VisSection = criarSection(SilentAim, "Visual", "FOV e destaque do alvo", "lucide:palette", false)

    local GroupFOV = VisSection:Group({})

    GroupFOV:Toggle({
        Title    = "FOV",
        Desc     = "Ativa o círculo de FOV",
        Icon     = "lucide:circle-dashed",
        Type     = "Checkbox",
        Value    = SC.ShowFOV,
        Flag     = "FOV",
        Callback = function(v) SC.ShowFOV = v end,
    })

    GroupFOV:Dropdown({
        Title    = "Posição",
        Desc     = "Ponto de referência do FOV",
        Values   = { "Mouse", "Center" },
        Value    = defaultFOVBehavior,
        Flag     = "PosicaoFOV",
        Callback = function(v) SC.FOVBehavior = v end,
    })

    VisSection:Slider({
        Title    = "Tamanho do FOV",
        Desc     = "Raio do círculo em pixels",
        Step     = 5,
        Value    = { Min = 40, Max = 1000, Default = SC.FOVSize },
        Flag     = "TamanhoFOV",
        Callback = function(v) SC.FOVSize = v end,
    })
    ...
    VisSection:Colorpicker({
        ...
        Default  = SC.FOVColor,
        Flag     = "CorFOV1",
        Callback = function(c) SC.FOVColor = c end,
    })

    VisSection:Divider()

    local GroupHighlight = VisSection:Group({})

    GroupHighlight:Toggle({
        Title    = "Highlight",
        Desc     = cor({ "Brilha" }, { " a pessoa focada", "#00AAFF" }),
        Icon     = "lucide:sparkles",
        Type     = "Checkbox",
        Value    = SC.ShowHighlight,
        Flag     = "HighlightSilentAim",
        Callback = function(v) SC.ShowHighlight = v end,
    })

    GroupHighlight:Colorpicker({
        Title    = "Cor do highlight",
        Desc     = "HUD e highlight do alvo",
        Default  = SC.HighlightColor,
        Flag     = "HighlightCorSilentAim",
        Callback = function(c) SC.HighlightColor = c end,
    })

    -- ══════════════════════════════════════════
    --  HUD — INFORMAÇÕES DO ALVO
    -- ══════════════════════════════════════════
    local InfoSection = criarSection(SilentAim, "HUD", "Informações exibidas sobre o alvo", "lucide:layout-template", false)
    ...
    InfoSection:Toggle({
        Title    = "HUD",
        Desc     = "Mostra informações do alvo em tela",
        Icon     = "lucide:app-window",
        Type     = "Checkbox",
        Value    = SC.ESP.Enabled,
        Flag     = "InfoSilentAim",
        Callback = function(v)
            SC.ESP.Enabled = v
            lockEspToggles(not v)
        end,
    })

    InfoSection:Divider()

    local GroupESP1 = InfoSection:Group({})
    local GroupESP2 = InfoSection:Group({})

    togName = GroupESP1:Toggle({
        Title    = "Nome",
        Desc     = "Mostra o nome do jogador",
        Icon     = "lucide:text-cursor",
        Type     = "Checkbox",
        Value    = SC.ESP.ShowName,
        Flag     = "InfoNomeSilentAim",
        Callback = function(v) SC.ESP.ShowName = v end,
    })

    togTeam = GroupESP1:Toggle({
        Title    = "Time",
        Desc     = "Mostra o time do jogador",
        Icon     = "lucide:users",
        Type     = "Checkbox",
        Value    = SC.ESP.ShowTeam,
        Flag     = "InfoTimeSilentAim",
        Callback = function(v) SC.ESP.ShowTeam = v end,
    })

    togHp = GroupESP1:Toggle({
        Title    = "Vida",
        Desc     = "Mostra a vida do alvo",
        Icon     = "lucide:heart-pulse",
        Type     = "Checkbox",
        Value    = SC.ESP.ShowHealth,
        Flag     = "InfoVidaSilentAim",
        Callback = function(v) SC.ESP.ShowHealth = v end,
    })

    togWpn = GroupESP2:Toggle({
        Title    = "Item",
        Desc     = "Mostra o que o alvo está segurando",
        Icon     = "lucide:sword",
        Type     = "Checkbox",
        Value    = SC.ESP.ShowWeapon,
        Flag     = "InfoItemSilentAim",
        Callback = function(v) SC.ESP.ShowWeapon = v end,
    })

    togDist = GroupESP2:Toggle({
        Title    = "Distância",
        Desc     = "Mostra a distância até o alvo em studs",
        Icon     = "lucide:ruler",
        Type     = "Checkbox",
        Value    = SC.ESP.ShowDist,
        Flag     = "InfoDistSilentAim",
        Callback = function(v) SC.ESP.ShowDist = v end,
    })

    togWall = GroupESP2:Toggle({
        Title    = "Parede",
        Desc     = "Mostra se o alvo está atrás de parede",
        Icon     = "lucide:wall",
        Type     = "Checkbox",
        Value    = SC.ESP.ShowWall,
        Flag     = "InfoWallSilentAim",
        Callback = function(v) SC.ESP.ShowWall = v end,
    })
...
if game.PlaceId == 13132367906 then
    local function desativarHitboxAC()
        local char = LocalPlayer.Character
        if char then
            local s = char:FindFirstChild("Hitbox")
        ...
        if sc then
            local s = sc:FindFirstChild("Hitbox")
            if s then pcall(function() s.Disabled = true; s:Destroy() end) end
        end
    end
...
tas.ActH          = 1.5
tas.ActAng        = 10
tas.ColorBot      = Color3.fromRGB(238, 238, 238)
tas.ColorPath     = Color3.fromRGB(255, 188, 0)
tas.VisualOpacity = 0
...
local function buildTASWaypoint(tasName, startPos)
    local guiRef = (gethui and gethui()) or CoreGui
    local cam    = Workspace.CurrentCamera
    ...
    screen.Parent         = guiRef

    -- root do waypoint, ancorado no centro
    ...
    -- ── card superior: nome + distância ──────────────────────────────────────
    local card = Instance.new("Frame")
    card.Size                   = UDim2.new(0, 0, 0, 26)
    card.AutomaticSize          = Enum.AutomaticSize.X
    card.BackgroundColor3       = Color3.fromRGB(6, 6, 10)
    ...
    cardPad.PaddingTop    = UDim.new(0, 0)
    cardPad.PaddingBottom = UDim.new(0, 0)

    local cardRow = Instance.new("UIListLayout", card)
    ...
    distLabel.TextColor3             = Color3.fromRGB(200, 210, 200)
    ...
    stem.Size             = UDim2.new(0, 2, 0, 18)
    stem.AnchorPoint      = Vector2.new(0.5, 0)
    stem.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    stem.BackgroundTransparency = 0.3
    stem.BorderSizePixel  = 0
    stem.LayoutOrder      = 2
    stem.Parent           = root

    local stemCorner = Instance.new("UICorner", stem)
    ...
    local pinCorner = Instance.new("UICorner", pin)
    pinCorner.CornerRadius = UDim.new(1, 0)

    local pinStroke = Instance.new("UIStroke", pin)
    pinStroke.Thickness    = 1.5
    pinStroke.Transparency = 0.2

    -- alpha atual (para fade suave)
    local alpha     = 0
    local tick0     = tick()
    local lastAlpha = -1

    local function applyAlpha(a)
        if math.abs(a - lastAlpha) < 0.005 then return end
        lastAlpha = a
        local hide = a < 0.01

        root.Visible = not hide
        if hide then return end

        card.BackgroundTransparency    = 0.08 + (1 - a) * 0.92
        cardStroke.Transparency        = 0.3  + (1 - a) * 0.7
        stem.BackgroundTransparency    = 0.3  + (1 - a) * 0.7
        pin.BackgroundTransparency     = (1 - a) * 0.1
        pinStroke.Transparency         = 0.2  + (1 - a) * 0.8

        local tAlpha = math.clamp(1 - a, 0, 1)
        nameLabel.TextTransparency     = tAlpha
        nameLabel.TextStrokeTransparency = 0.5 + tAlpha * 0.5
        distLabel.TextTransparency     = tAlpha
        distLabel.TextStrokeTransparency = 0.5 + tAlpha * 0.5
    end
    ...
    local loop = RunService.Heartbeat:Connect(function(dt)
        ...
        -- atualiza cor do accent
        local accent = tas.ColorBot
        cardStroke.Color   = accent
        pinStroke.Color    = accent
        nameLabel.TextColor3 = accent
        pin.BackgroundColor3 = accent
        stem.BackgroundColor3 = accent

        -- projeção na tela
        local worldTgt   = startPos + Vector3.new(0, 3.2, 0)
        local sp, onScr  = cam:WorldToScreenPoint(worldTgt)
        local vp         = cam.ViewportSize
        local inBounds   = sp.X > 10 and sp.X < vp.X - 10
                        and sp.Y > 10 and sp.Y < vp.Y - 10

        local visible = onScr and inBounds
        local target  = visible and 1 or 0
        local speed   = visible and 6 or 10
        alpha = alpha + (target - alpha) * math.min(dt * speed, 1)
        applyAlpha(alpha)

        if alpha < 0.01 then return end

        -- posiciona o root no ponto projetado (ancorPoint = bottom-center)
        root.Position = UDim2.fromOffset(sp.X, sp.Y)

        -- distância
        local dist = math.floor((hrp.Position - startPos).Magnitude)
        local closeFlash = dist < 8 and tick() % 0.5 < 0.25
        distLabel.TextColor3 = closeFlash
            and Color3.fromRGB(255, 90, 90)
            or  Color3.fromRGB(200, 210, 200)
        distLabel.Text = dist .. " m"

        -- pulso suave na bolinha do pino
        local pulse = 1 + math.sin((tick() - tick0) * 4) * 0.12
        local ps = math.floor(10 * pulse)
        pin.Size = UDim2.new(0, ps, 0, ps)
    end)
...
local function clrTas(n, playingOnly)
    ...
    if not playingOnly then
        ...
        if d._WaypointConn then d._WaypointConn:Disconnect(); d._WaypointConn = nil end
        if d._removeWaypoint then d._removeWaypoint(); d._removeWaypoint = nil end
...
local function actTas(n)
    ...
    local startPos = Vector3.new(c[1], c[2], c[3])
    ...
    d._removeWaypoint = buildTASWaypoint(n, startPos)
    ...
    for _, d in pairs(tas.Loaded) do
        if d.Adornments then
            for _, p in ipairs(d.Adornments) do
                if p and p.Parent then p.Color3 = colorBot; p.Transparency = visOpacity end
            end
        end
        if d.PathParts then
            for _, p in ipairs(d.PathParts) do
                if p then p.Color3 = colorPath; p.Transparency = visOpacity end
            end
        end
    end
end
...
do
    ...
    PlaybackSection:Toggle({
        Title = "Iniciar", Icon = "lucide:circle-play",
        Desc  = cor({ "Inicia a " }, { "execução", "#00FF00" }, { " do percurso" }),
        Value = false, Flag = "TAS",
        Callback = function(v)
    ...
    StopButton = PlaybackSection:Button({
        Title = "Parar", Icon = "lucide:circle-stop", Locked = true,
    ...
    TASDropdown = PlaybackSection:Dropdown({
        Title = "Selecionar", Icon = "lucide:list",
        Desc  = cor({ "Escolha os " }, { "TAS para iniciar", "#FFFFFF" }),
        Values = {}, Value = {}, Multi = true, AllowNone = true, Flag = "SelecaoTAS",
        Callback = function(v)
    ...
    PlaybackSection:Colorpicker({
        Title = "Humanoid", Desc = "Muda a cor do personagem e da seta",
        Default = Color3.fromRGB(0, 255, 0), Flag = "TASHumanoidCor",
        Callback = function(c)
            if getgenv().TAS and getgenv().TAS.UpdateVisuals then getgenv().TAS.UpdateVisuals(c, nil, nil) end
        end,
    })
    PlaybackSection:Colorpicker({
        Title = "Trajetória", Desc = "Muda a cor da trajetória",
        Default = Color3.fromRGB(0, 255, 0), Flag = "TASTrajetoriaCor",
        Callback = function(c)
            if getgenv().TAS and getgenv().TAS.UpdateVisuals then getgenv().TAS.UpdateVisuals(nil, c, nil) end
        end,
    })
    PlaybackSection:Slider({
        Title = "Opacidade", Value = { Min = 0, Max = 1, Default = 0 }, Step = 0.05, Flag = "OpacidadeTAS",
        Desc  = cor({ "Visibilidade do  " }, { "TAS", "#FF0000" }),
        Callback = function(v)
            if getgenv().TAS and getgenv().TAS.UpdateVisuals then getgenv().TAS.UpdateVisuals(nil, nil, v) end
        end,
    })
    ...
    T.UpdateButtonState = function(playing)
        if StopButton then if playing then StopButton:Unlock() else StopButton:Lock() end end
    end
    ...
    RecordSection:Paragraph({
        Title = "Observação", Color = Color3.fromHex("#800080"),
        Image = "lucide:message-square", ImageSize = 30,
        Desc  = cor({ "Também é possível usar " }, { "/e gravar", "#00FF00" }, { " e " }, { "/e parar", "#FF0000" }, { " no chat" }),
    })
    RecordSection:Input({
        Title = "Nome", Value = "", InputIcon = "lucide:pencil", Flag = "NomeTAS",
        Desc  = cor({ "Defina o " }, { "nome", "#00AAFF" }, { " do arquivo" }),
        Callback = function(v) if getgenv().TAS then getgenv().TAS.CurrentName = v end end,
    })

    local g = RecordSection:Group()
    g:Button({
        Title = "Gravar", Icon = "lucide:circle-dot",
    ...
    g:Space()
    g:Button({
        Title = "Parar", Icon = "lucide:square",
    ...
    RecordSection:Paragraph({
        Title = "Observação", Color = Color3.fromHex("#800080"),
        Image = "lucide:hard-drive", ImageSize = 30,
        Desc  = cor({ "O arquivo será salvo para a pasta " }, { "michigun.xyz/tas", "#00AAFF" }),
    })
    RecordSection:Button({
        Title = "Salvar", Icon = "lucide:save",
        Desc  = cor({ "Salva", "#FFAA00" }, { " o arquivo" }),
        Callback = function()
            local T2 = getgenv().TAS; if not T2 then return end
            if not T2.CurrentName or T2.CurrentName == "" then
                notificar("Defina um nome antes de salvar", 3, "lucide:alert-circle"); return
            end
...
do
    ...
    local function stopThinking()
        ...
        if thinkingPara then
            pcall(function() thinkingPara:Destroy() end)
            thinkingPara = nil
        end
    end
    ...
    local function startThinking()
        stopThinking()
        thinkingPara = ChatGPT:Paragraph({
            Title = "IA",
            Desc  = "Pensando",
        ...
        local dots = { "", ".", "..", "..." }
        local idx  = 1
        thinkingThread = task.spawn(function()
            while true do
                task.wait(0.4)
                if thinkingPara then
                    idx = idx % #dots + 1
                    thinkingPara:SetDesc("Pensando" .. dots[idx])
                end
            end
        end)
    end
    ...
    local function clearChat()
        for _, p in ipairs(msgParagraphs) do
            pcall(function() p:Destroy() end)
        end
        table.clear(msgParagraphs)
        stopThinking()
        _G.ChatGPT.History     = {{ role = "system", content = Personality }}
        _G.ChatGPT.LastMessage = ""
        notificar("Conversa limpa", 2, "lucide:trash")
    end

    ChatGPT:Paragraph({
        Title     = "Observação",
        Desc      = cor(
            { "Personalize a IA editando " },
            { "michigun.xyz/IA.txt", "#FFD700" }
        ),
    ...
    ChatGPT:Input({
        ...
        Callback    = function(text) currentPrompt = text end,
    })

    local btnGroup = ChatGPT:Group()

    SendBtn = btnGroup:Button({
        Title    = "Enviar",
        Icon     = "lucide:send",
        Callback = function()
            if not (_G.ChatGPT and _G.ChatGPT.Ask) then
                notificar("Aguarde...", 2, "lucide:loader"); return
            end
            if currentPrompt == "" then return end

            SendBtn:Lock()
            local ts  = timestamp()
            local msg = currentPrompt

            ChatGPT:Divider()

            local userPara = ChatGPT:Paragraph({
                Title     = "Você (" .. ts .. ")",
                Desc      = msg,
                Color     = Color3.fromRGB(60, 130, 230),
                Image     = "lucide:user-round",
                ImageSize = 24,
            })
            table.insert(msgParagraphs, userPara)

            startThinking()

            task.spawn(function()
                local cleanMsg, luaCode = _G.ChatGPT.Ask(msg)
                local tsResp = timestamp()

                stopThinking()

                local aiPara = ChatGPT:Paragraph({
                    Title     = "IA (" .. tsResp .. ")",
                    Desc      = cleanMsg,
                    Color     = Color3.fromRGB(40, 180, 100),
                    Image     = "geist:logo-open-ai",
                    ...
                    Buttons   = {
                        ...
                        },
                        {
                            ...
                            Callback = function()
                                _G.ChatGPT.SendToChat(cleanMsg)
                                notificar("Enviado no chat", 2, "lucide:message-square")
                            end,
                        },
                    },
                })
                table.insert(msgParagraphs, aiPara)

                if luaCode then
                    local codePara = ChatGPT:Code({ Code = luaCode })
                    if codePara then table.insert(msgParagraphs, codePara) end
                end
    ...
    if listfiles then
        for _, f in ipairs(listfiles(f3x_fDir)) do
            if f:sub(-5) == ".json" then
                -- suporta tanto / quanto \ como separador (varia por executor)
                local name = f:match("([^/\\]+)%.json$")
                if name then o[#o + 1] = name end
            end
        end
    end
    return o
...
f3x.SaveConfig = function(n)
    if not n or n == "" then notificar("F3X: Nome inválido", 3, "lucide:alert-circle"); return nil end
...
local function getObj(path)
    -- GetFullName() retorna "game.Workspace.Parte" ou "Game.Workspace.Parte"
    -- percorre cada segmento ignorando o primeiro se for "game"/"Game"
    local parts = {}
    for s in path:gmatch("[^%.]+") do parts[#parts + 1] = s end
    local cur = game
    local start = (parts[1] and parts[1]:lower() == "game") and 2 or 1
...
f3x.ApplyConfig = function(n)
    ...
    local d
    pcall(function() d = HttpService:JSONDecode(readfile(p)) end)
    if not d then return false end
    if d.PlaceId ~= game.PlaceId then
        notificar("F3X de outro mapa", 3, "lucide:alert-circle")
        return false
    end
    ...
    for _, o in ipairs(game:GetService("Workspace"):GetDescendants()) do
        if o:IsA("BasePart") then
            local bucket = fallbackByName[o.Name]
            if not bucket then bucket = {}; fallbackByName[o.Name] = bucket end
            bucket[#bucket + 1] = o
        end
    end
    ...
    for _, v in ipairs(d.Parts or {}) do
        ...
        local sz       = Vector3.new(unp(v.Size))
        local foundPart = nil

        -- 1) tenta pelo path exato
        local obj = getObj(v.Path)
        if obj and obj:IsA("BasePart") and (obj.Position - targetCf.Position).Magnitude < 2 then
            foundPart = obj
        end

        -- 2) fallback: usa o lookup já construído, busca só pelo último segmento do path
        if not foundPart then
            local partName = v.Path:match("([^%.]+)$")
            local bucket   = fallbackByName[partName]
            if bucket then
        ...
        if foundPart then
            foundPart.Size = sz
            f3x.ModifiedParts[foundPart] = sz
        end
    end
    return true
end
...
f3x.DeleteConfig = function(n)
    if not n then notificar("F3X: Nenhuma seleção", 3, "lucide:alert-circle"); return nil end
    ...
    local function CreateF3XActions()
        ...
        ActionDropdown = CS:Dropdown({ Title = "Ações", Desc = cor({ "Gerenciar " }, { "configurações", "#FFAA00" }), Icon = "lucide:settings-2", Values = {
            ...
            { Title = "Carregar F3X",   Desc = cor({ "Carrega", "#00AAFF" }, { " os dados selecionados" }),     Icon = "lucide:upload", Callback = function()
                ...
                local ok = f3x.ApplyConfig(SelectedConfig)
                if ok then notificar("Configuração aplicada", 3, "lucide:check") end
                CreateF3XActions() end },
...
local function CheckIfBad(Inst)
    ...
    and not (function()
        for _, v in pairs(Players:GetPlayers()) do
            if v ~= LocalPlayer and v.Character and Inst:IsDescendantOf(v.Character) then return true end
        end
    end)()
    and (LocalPlayer.Character and not Inst:IsDescendantOf(LocalPlayer.Character)) then
        if Inst:IsA("SpecialMesh") then Inst.MeshId = "" Inst.TextureId = ""
        elseif Inst:IsA("FaceInstance") then Inst.Transparency = 1 Inst.Shiny = 1
        elseif Inst:IsA("ShirtGraphic") then Inst.Graphic = ""
        elseif table.find(CanBeEnabled, Inst.ClassName) then Inst.Enabled = false
        elseif Inst:IsA("PostEffect") then Inst.Enabled = false
        elseif Inst:IsA("Explosion") then Inst.BlastPressure = 1 Inst.BlastRadius = 1 Inst.Visible = false
        elseif Inst:IsA("Clothing") or Inst:IsA("SurfaceAppearance") or Inst:IsA("BaseWrap") then Inst:Destroy()
        elseif Inst:IsA("BasePart") and not Inst:IsA("MeshPart") then Inst.Material = Enum.Material.Plastic Inst.Reflectance = 0
        elseif Inst:IsA("Model") then Inst.LevelOfDetail = 1
        elseif Inst:IsA("MeshPart") then Inst.RenderFidelity = 2 Inst.Reflectance = 0 Inst.Material = Enum.Material.Plastic
        end
    end
end
...
Antilag:Paragraph({
    Title = "O que é anti-lag?",
    Desc  = "Melhora a performance em geral do jogo, garantindo uma experiência mais fluída",
    Image = "lucide:zap",
})
...
Antilag:Toggle({
    Title    = "Antilag",
    Desc     = "Otimiza o jogo e remove o limite de FPS",
    Icon     = "lucide:zap",
    Value    = false,
    Callback = function(state)
        if state then
            pcall(function() Lighting.GlobalShadows = false Lighting.FogEnd = 9e9 Lighting.ShadowSoftness = 0 if sethiddenproperty then sethiddenproperty(Lighting, "Technology", 2) end end)
            ...
            pcall(function() for _, v in pairs(MaterialService:GetChildren()) do v:Destroy() end MaterialService.Use2022Materials = false end)
            pcall(function() if setfpscap then setfpscap(1e6) end end)
            pcall(function() local t = Workspace:FindFirstChildOfClass("Terrain") if t then t.WaterWaveSize = 0 t.WaterWaveSpeed = 0 t.WaterReflectance = 0 t.WaterTransparency = 0 if sethiddenproperty then sethiddenproperty(t, "Decoration", false) end end end)
            for _, v in pairs(game:GetDescendants()) do pcall(CheckIfBad, v) end
            conexaoAntilag = game.DescendantAdded:Connect(function(v) task.wait(1) pcall(CheckIfBad, v) end)
            UI:Notify({ Title = "Antilag", Content = "Otimizações aplicadas", Duration = 5, Icon = "lucide:zap" })
        else
            if conexaoAntilag then conexaoAntilag:Disconnect() conexaoAntilag = nil end
            pcall(function() Lighting.GlobalShadows = true Lighting.FogEnd = 100000 Lighting.ShadowSoftness = 0.2 end)
            pcall(function() settings().Rendering.QualityLevel = 10 end)
            pcall(function() if setfpscap then setfpscap(60) end end)
            UI:Notify({ Title = "Antilag", Content = "Otimizações desativadas", Duration = 5, Icon = "lucide:zap" })
        end
    end
})
...
Antilag:Paragraph({ Title = "FPS desbloqueado", Desc = "Remove o limite de 60 FPS do Roblox", Image = "lucide:gauge" })
Antilag:Paragraph({ Title = "Remoção de sombras",      Desc = "Desativa sombras e efeitos da câmera", Image = "lucide:sun-dim" })
Antilag:Paragraph({ Title = "Partículas removidas",   Desc = "Desativa todos os tipos de partículas", Image = "lucide:sparkles" })
Antilag:Paragraph({ Title = "Qualidade estável",  Desc = "Deixa os materiais lisos", Image = "lucide:layers" })
...
do
    ...
    OS:Button({ Title = "Aplicar favorito", Desc = cor({ "Aplica a skin " }, { "favoritada", "#FFAA00" }, { " selecionada no alvo" }), Icon = "lucide:star", Callback = function()
        if not selectedFavorite then notificar("Nenhum char favorito selecionado", 3, "lucide:x"); return end
            ...
            { Title = isConfirmingFavDelete and "Confirmar deleção?" or "Deletar selecionado",
              ...
              Callback = function()
                ...
                if not isConfirmingFavDelete then
                    isConfirmingFavDelete = true; notificar("Confirme", 2, "lucide:alert-triangle"); CreateFavActions()
...
do
    ...
    local gs = MS:Group()
    gs:Button({ Title = "Aplicar", Icon = "lucide:check-circle", Callback = function() PC.SpeedVal = spdVal; PC.SpeedEnabled = true; notificar("Velocidade setada para " .. spdVal, 2, "lucide:zap") end })
    ...
    local gj = MS:Group()
    gj:Button({ Title = "Aplicar", Icon = "lucide:check-circle", Callback = function() PC.JumpVal = jmpVal; PC.JumpEnabled = true; notificar("Pulo setado para " .. jmpVal, 2, "lucide:zap") end })
    ...
    IS:Input({ Title = "Jogador", Desc = "Nome ou Display", Placeholder = "Digite aqui", InputIcon = "lucide:search", Callback = function(v)
        ...
        if found then PC.TargetPlayer = found.Name; notificar("Usuário: " .. found.Name, 3, "lucide:user-check")
        else notificar("Não encontrado", 2, "lucide:search-x") end
    end })
    ...
    local function RefreshActionDrop()
        ...
        ActionDrop = IS:Dropdown({ Title = "Ações", Icon = "lucide:mouse-pointer-click", Values = {
            { Title = "Fling", Desc = "Arremessa o alvo (arriscado)", Icon = "lucide:earth", Callback = function()
                if PC.TargetPlayer == "" then notificar("Selecione alguém primeiro", 2, "lucide:alert-circle"); return end
            ...
            { Title = PC.SpectateActive and "Parar Espectar" or "Espectar", Desc = "Visualiza a câmera do alvo", Icon = "lucide:eye", Callback = function()
                if PC.TargetPlayer == "" then notificar("Selecione alguém", 2, "lucide:alert-circle"); return end
            ...
            { Title = "Teleportar", Desc = "Teleporta ao jogador",          Icon = "lucide:plane",  Callback = function() if PC.TargetPlayer == "" then return end; PC.TriggerTeleport = true; notificar("Teleportando...", 2, "lucide:plane")   end },
...
if MapaAtual then
    ...
    if MapaAtual == "Tevez" then
        ...
        LS:Input({ Title = "Mensagem", Desc = cor({ "Digite o " }, { "texto", "#FFFFFF" }, { " para enviar no chat" }), Value = "", InputIcon = "lucide:text-cursor", Placeholder = "Digite aqui...", Callback = function(v) if _G.TevezMods then _G.TevezMods.ChatMessage = v end end })
        LS:Toggle({ Title = "Spam",   Desc = cor({ "Envia a mensagem " }, { "várias vezes", "#FFAA00" }),      Icon = "lucide:repeat", Value = false, Callback = function(s) if _G.TevezMods and _G.TevezMods.ToggleSpam   then _G.TevezMods.ToggleSpam(s) end end })
        LS:Button({ Title = "Enviar", Desc = cor({ "Manda", "#00FF00" }, { " a mensagem atual" }), Color = "Green", Icon = "lucide:send", Callback = function() if _G.TevezMods and _G.TevezMods.SendMessage then _G.TevezMods.SendMessage() end end })
        ...
        SpS:Dropdown({ Title = "Dispositivo", Desc = cor({ "Selecione o " }, { "dispositivo", "#FFFFFF" }), Icon = "lucide:laptop", Values = { "Mobile", "Computer" }, Value = "Computer", Callback = function(opt) if _G.TevezMods then _G.TevezMods.SelectedDevice = opt end end })
        SpS:Toggle({ Title = "Ativar", Desc = cor({ "Altera", "#00FF00" }, { " o dispositivo (reseta o personagem)" }), Icon = "lucide:power", Value = false, Callback = function(v) if _G.TevezMods and _G.TevezMods.ToggleSpoof then _G.TevezMods.ToggleSpoof(v) end end })
        SpS:Space()
        SpS:Dropdown({ Title = "AFK", Desc = cor({ "Permite " }, { "ativar", "#00FF00" }, { " ou " }, { "desativar", "#FF0000" }, { " o " }, { "AFK", "#FFAA00" }), Icon = "lucide:coffee", Values = { "Ativar", "Desativar" }, Value = "Desativar", Callback = function(opt) if _G.TevezMods and _G.TevezMods.SetAFK then _G.TevezMods.SetAFK(opt == "Ativar") end end })
        ...
        FaS:Toggle({ Title = "Ativar",      Desc = cor({ "Inicia",  "#00FF00" }, { " o autofarm" }),                                         Icon = "lucide:play",         Value = false, Callback = function(v) if _G.TevezAutoFarm then _G.TevezAutoFarm.Toggle(v) end end })
        FaS:Toggle({ Title = "Modo seguro", Desc = cor({ "Pausa",   "#FF0000" }, { " se houver jogadores por perto" }),                       Icon = "lucide:shield-check", Value = false, Flag = "ModoSeguroAutofarmTevez", Callback = function(v) if _G.TevezAutoFarm then _G.TevezAutoFarm.SafeMode = v end end })
        FaS:Input({  Title = "Raio de segurança", Desc = cor({ "Distância para " }, { "detectar", "#FF0000" }, { " se há alguém por perto" }), Placeholder = "60", InputIcon = "lucide:radio", Flag = "RaioDeSegurancaAutofarmTevez", Callback = function(v) local n = tonumber(v); if n and _G.TevezAutoFarm then _G.TevezAutoFarm.SafeRadius = n end end })
        ...
        KillAuraToggle = CoS:Toggle({ Title = "Kill aura", Desc = cor({ "Mata", "#FF0000" }, { " jogadores próximos" }), Icon = "lucide:skull", Value = false, Callback = function(state)
            if _G.TevezMods and _G.TevezMods.ToggleAura then _G.TevezMods.ToggleAura(state, KillAuraToggle) end
        end })
        KillAuraToggle:Lock()
        ...
        ShS:Dropdown({ Title = "Comprar itens", Desc = cor({ "Seleciona um " }, { "item", "#FFFFFF" }, { " na loja" }), Icon = "lucide:tag", Values = shopValues, Value = "GLOCK 18 - $4300", Callback = function(opt) if _G.TevezMods then _G.TevezMods.SelectedShopItem = displayToKey[opt] end end })
        ShS:Button({ Title = "Comprar", Desc = cor({ "Compra", "#00FF00" }, { " o item selecionado" }), Icon = "lucide:credit-card", Callback = function() if _G.TevezMods and _G.TevezMods.BuyItem then _G.TevezMods.BuyItem() end end })

        local MoS = criarSection(TevezTab, "Mods", "Modifica a arma", "lucide:hammer", false)
        MoS:Input({ Title = "Bullets", Desc = cor({ "Quantidade de " }, { "balas", "#00AAFF" }, { " por disparo" }), Placeholder = "Valor", InputIcon = "lucide:hash",   Flag = "BalasTevez",  Callback = function(v) if v ~= "" and _G.TevezMods then _G.TevezMods.GunConfig.Bullets = tonumber(v) end end })
        MoS:Input({ Title = "Spread",  Desc = cor({ "Muda o " }, { "espalhamento", "#FFAA00" }),                     Placeholder = "Valor", InputIcon = "lucide:expand",  Flag = "SpreadTevez", Callback = function(v) if v ~= "" and _G.TevezMods then _G.TevezMods.GunConfig.Spread  = tonumber(v) end end })
        MoS:Input({ Title = "Range",   Desc = cor({ "O quão " }, { "longe", "#00AAFF" }, { " as balas vão" }),       Placeholder = "Valor", InputIcon = "lucide:target",  Flag = "RangeTevez",  Callback = function(v) if v ~= "" and _G.TevezMods then _G.TevezMods.GunConfig.Range   = tonumber(v) end end })
        MoS:Button({ Title = "Aplicar mods", Desc = cor({ "Muda", "#00FF00" }, { " os atributos da arma" }), Icon = "lucide:check-circle", Callback = function() if _G.TevezMods and _G.TevezMods.ApplyGunMods then _G.TevezMods.ApplyGunMods() end end })
    ...
    elseif MapaAtual == "Christian" then
        ...
        if not _G.ChristianSpooferConfig then
            _G.ChristianSpooferConfig = { Enabled = false, Device = "Pc" }
        end
        if not _G.ChristianSpooferHooked then
            _G.ChristianSpooferHooked = true
            local OldNameCall
            OldNameCall = _G._hookmeta(game, "__namecall", _G._newcc(function(self, ...)
                if not _G._checkcall() and _G._getnamecall() == "FireServer" and tostring(self) == "Dispositivo" and _G.ChristianSpooferConfig.Enabled then
                    local args = { ... }; args[1] = _G.ChristianSpooferConfig.Device; return OldNameCall(self, table.unpack(args))
                end
        ...
        Sp:Dropdown({ Title = "Dispositivo", Desc = cor({ "Selecione o " }, { "dispositivo", "#FFFFFF" }), Icon = "lucide:laptop", Values = { "Celular", "PC" }, Value = "Celular", Callback = function(opt) _G.ChristianSpooferConfig.Device = (opt == "PC") and "Pc" or "Mobile" end })
        Sp:Toggle({ Title = "Ativar", Desc = cor({ "Altera", "#00FF00" }, { " o dispositivo (reseta o personagem)" }), Icon = "lucide:power", Value = false, Callback = function(v)
            _G.ChristianSpooferConfig.Enabled = v
    ...
    elseif MapaAtual == "Delta" then
        ...
        if type(notificar) == "function" then _G.notificar = notificar end

        criarSection(DeltaTab, "Combate", "Kill aura", "lucide:swords", false):Toggle({ Title = "Kill aura", Desc = cor({ "Mata", "#FF0000" }, { " todos os inimigos ao redor" }), Icon = "lucide:skull", Value = false, Callback = function(s) if _G.DeltaLogic then _G.DeltaLogic.KillAuraAtiva = s end end })

        local JS = criarSection(DeltaTab, "Polichinelos", "Modificar", "lucide:activity", true)
        JS:Input({ Title = "Quantidade", Desc = cor({ "Valor para " }, { "adicionar ou remover", "#FFFFFF" }), Placeholder = "10", InputIcon = "lucide:hash", Callback = function(v) if _G.DeltaLogic then _G.DeltaLogic.JJValue = v:gsub("%D", "") end end })
        JS:Button({ Title = "Modificar", Desc = cor({ "Muda", "#00FF00" }, { " seus polichinelos" }), Icon = "lucide:edit-2", Callback = function() if _G.DeltaLogic and _G.DeltaLogic.SetJJ then _G.DeltaLogic.SetJJ() end end })

        local ES = criarSection(DeltaTab, "Economia", "Dinheiro", "lucide:coins", false)
        ES:Input({ Title = "Quantidade", Desc = cor({ "Valor para receber" }), Placeholder = "1000000", InputIcon = "lucide:circle-dollar-sign", Callback = function(v) if _G.DeltaLogic then _G.DeltaLogic.GetRichValue = v:gsub("%D", "") end end })
        ES:Button({ Title = "Money", Desc = cor({ "Pega", "#00FF00" }, { " o dinheiro" }), Icon = "lucide:gem", Callback = function() if _G.DeltaLogic and _G.DeltaLogic.GetRich then _G.DeltaLogic.GetRich() end end })
    ...
    elseif MapaAtual == "Soucre" then
        ...
        task.spawn(function()
            ...
            _G.SoucreLogic.UpdateCallback = function(val) if MP then MP:SetDesc(cor({ "Dinheiro farmado: ", "#AAAAAA" }, { "$", "#00FF00" }, { tostring(val), "#00FF00" })) end end
        end)
        FaS:Toggle({ Title = "Autofarm", Desc = cor({ "Inicia", "#00FF00" }, { " o autofarm" }), Icon = "lucide:circle-play", Value = false, Callback = function(v) if _G.SoucreLogic then _G.SoucreLogic.Toggle(v) end end })
    ...
    elseif MapaAtual == "NovaEra" then
        ...
        task.spawn(function()
            ...
            _G.NovaEraLogic.UpdateCallback = function(val) if MP then MP:SetDesc(cor({ "Dinheiro farmado: ", "#AAAAAA" }, { "$", "#00FF00" }, { tostring(val), "#00FF00" })) end end
        end)
        FaS:Dropdown({ Title = "Modo", Values = { "Lixeiro", "Barbeiro" }, Value = "Lixeiro", Callback = function(v) if _G.NovaEraLogic and _G.NovaEraLogic.SetMode then _G.NovaEraLogic.SetMode(v) end end })
        FaS:Toggle({ Title = "Autofarm", Desc = cor({ "Inicia", "#00FF00" }, { " o autofarm de coleta" }), Icon = "lucide:circle-play", Value = false, Callback = function(v) if _G.NovaEraLogic and _G.NovaEraLogic.Toggle then _G.NovaEraLogic.Toggle(v) end end })
    ...
    elseif MapaAtual == "Apex" then
        ...
        ApexTab:Button({ Title = "Invadir base", Desc = "Autoexplicativo", Icon = "lucide:log-in", IconAlign = "Left", Locked = not JD_IS_PREMIUM, LockedTitle = "Premium", Callback = function() if _G.IniciarRota then _G.IniciarRota() end end })
        ...
        MoS:Toggle({ Title = "Bala infinita", Desc = cor({ "Congela", "#00FF00" }, { " a munição da arma" }), Icon = "lucide:infinity", Flag = "ApexBalaInfinita", Value = false, Callback = function(v)
            if _G._InfiniteAmmoConn      then _G._InfiniteAmmoConn:Disconnect();      _G._InfiniteAmmoConn      = nil end
            if _G._InfiniteAmmoAddedConn then _G._InfiniteAmmoAddedConn:Disconnect(); _G._InfiniteAmmoAddedConn = nil end
            ...
            _G._InfiniteAmmoConn = RunService.Heartbeat:Connect(function() fillAmmo(LocalPlayer:FindFirstChild("Backpack")); fillAmmo(LocalPlayer.Character) end)
            _G._InfiniteAmmoAddedConn = LocalPlayer.Backpack.ChildAdded:Connect(function(tool)
                task.wait(0.1)
        ...
        TrS:Toggle({ Title = "Spammar sons", Desc = cor({ "Precisa", "#FF0000" }, { " de uma arma." }, { " Esses sons tocam para todos.", "#03F916" }), Icon = "lucide:audio-lines", Value = false, Callback = function(v)
            if _G.ApexLogic and _G.ApexLogic.ToggleSound then _G.ApexLogic.ToggleSound(v) end
        end })
    end
end

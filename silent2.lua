...
-- Mapeamento para escolha de alvo (Aiming), não para detecção
local PartMapping = {
    ["Head"] = {"Head"},
...
local function GetBestPart(character)
    ...
    -- Se for random, tenta pegar qualquer parte visível mapeada
    if #targets == 0 or table.find(targets, "Random") then
        -- Prioriza Head e Torso
...
-- LÓGICA REFEITA: Detecta se QUALQUER BasePart do personagem está no círculo
local function getClosestPlayer()
    local BestPart = nil
    local BestChar = nil
    local ShortestDistance = MathHuge -- Menor distância ao centro do FOV
    ...
    for _, Player in Pairs(Players:GetPlayers()) do
        ...
        -- LÓGICA DE FOV REFEITA
        local playerInFOV = false
        local distanceToCenter = MathHuge

        -- Itera TODOS os filhos do personagem para garantir detecção de R6, R15 e acessórios
        local charChildren = Character:GetChildren()
        for i = 1, #charChildren do
            local obj = charChildren[i]
            if obj:IsA("BasePart") then
                ...
                if onScreen then
                    ...
                    -- Se qualquer parte estiver dentro do raio, o player é válido
                    if dist2D <= _G.AimbotConfig.FOVSize then
                        playerInFOV = true
                        -- Armazena a distância dessa parte específica para critério de "Closest"
        ...
        -- Se alguma parte entrou no círculo, verificamos se ele é o "mais próximo" do centro até agora
        if playerInFOV then

...
-- Isso remove qualquer FOV ou Highlight que tenha ficado travado na tela de execuções passadas
...
_G.AimbotConfig = _G.AimbotConfig or {
    ...
    WallCheck = true,
    FOVSize = 200,
    ShowFOV = false, -- Garante que comece falso se não existir config
    FOVBehavior = "Center",
...
_G.StartSilentAim = function()
    ...
    fov_circle.Visible = false -- Força invisibilidade no momento da criação
    ...
    local c1 = RunService.RenderStepped:Connect(function()
        -- Verificação rigorosa: Se SilentAim não está ativo OU ShowFOV é falso, esconde.
        if _G.SilentAimActive and config.ShowFOV == true and fov_circle then

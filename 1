...
task.spawn(function()
    ler("https://raw.githubusercontent.com/brasiIidade/x/refs/heads/main/hitbox2.lua")
end)
...
task.spawn(function()
    ler("https://raw.githubusercontent.com/brasiIidade/x/refs/heads/main/esp2.lua")
end)
...
--// tab tas
task.spawn(function()
    ler("https://raw.githubusercontent.com/brasiIidade/x/refs/heads/main/tas2.lua")
end)
...
--// tab JJs
task.spawn(function()
    ler("https://raw.githubusercontent.com/brasiIidade/x/refs/heads/main/JJ2.lua")
end)
...
--// tab ChatGPT
task.spawn(function()
    ler("https://raw.githubusercontent.com/brasiIidade/x/refs/heads/main/ChatGPT2.lua")
end)
...
local MainSection = criarsection(Char, "Principal", "Alterar aparência", "lucide:shirt", true)
...
--// tab anti-lag
task.spawn(function()
    ler("https://raw.githubusercontent.com/brasiIidade/x/refs/heads/main/optimize.lua")
end)
...
--// tab local
task.spawn(function()
    ler("https://raw.githubusercontent.com/brasiIidade/x/refs/heads/main/Local2.lua")
end)

local speedValue = nil

local CharacterSection = criarsection(Player, "Personagem", "Atributos locais", "lucide:user", true)
...
local SystemSection = criarsection(Player, "Sistema", "Configurações do jogo", "lucide:monitor", false)
...
local CreationSection = criarsection(Configs, "Criação", "Nova configuração", "lucide:plus", true)
...
local ManagementSection = criarsection(Configs, "Gerenciamento", "Carregar e salvar", "lucide:list", true)
...
local InterfaceSection = criarsection(Configs, "Interface", "Opções da UI", "lucide:layout", false)
...
if tevez then
    ...
    task.spawn(function()
        ler("https://raw.githubusercontent.com/brasiIidade/x/refs/heads/main/tevez2.lua")
    end)

    local LocalSection = criarsection(TevezTab, "Local", "Chat e AFK", "lucide:user", true)
    LocalSection:Input({
        Title = "Mensagem",
    ...
    SpooferSection:Toggle({
        ...
        Value = false,
        Flag = "TevezSpoofCheck",
        Callback = function(v)
            if _G.TevezMods and _G.TevezMods.ToggleSpoof then _G.TevezMods.ToggleSpoof(v) end
        end
    })

    local FarmSection = criarsection(TevezTab, "Autofarm", "Farm de Banco", "lucide:bank-note", false)
    ...
    FarmSection:Toggle({
        ...
        Desc = "Inicia o ciclo de roubo ao banco",
        Value = false,
        Flag = "TevezFarmToggle",
        Callback = function(v)
...
if delta then
    ...
    JJSection:Input({
        Title = "Quantidade",
        Desc = "Valor para adicionar/remover",
        Placeholder = "Ex: 10",
        Flag = "DeltaJJInput",
        Callback = function(v)
    ...
    JJSection:Button({
        Title = "Modificar",
        Desc = "Aplica o valor inserido",
        Callback = function()
            if _G.DeltaLogic and _G.DeltaLogic.SetJJ then _G.DeltaLogic.SetJJ() end
        end
    })

    local EcoSection = criarsection(DeltaTab, "Economia", "Dinheiro e transferências", "lucide:bank-note", false)

    EcoSection:Button({
        Title = "Ficar rico (Rejoin)",
        Desc = "Pega 1M e reinicia o servidor",
        Callback = function()
    ...
    EcoSection:Paragraph({
        Title = "Observação",
        Desc = "Money All faz uma transferência a cada 16s",
        Color = "Blue",
        Image = "lucide:info"
    })
...
if soucre then
    ...
    local FarmSection = criarsection(SoucreTab, "Entregador", "Autofarm de dinheiro", "lucide:bike", true)

...
local function getUserId(nameOrId)
    local id = tonumber(nameOrId)
    if id then return id end
    local s, res = pcall(function() return Players:GetUserIdFromNameAsync(nameOrId) end)
    return s and res or nil
end

local function morphCharacter(char, desc)
...
avatarchanger:Button({
    ...
    Callback = function()
        ...
        local id = getUserId(targetInput)
        if id then
            local s, desc = pcall(function() return Players:GetHumanoidDescriptionFromUserId(id) end)
            if s and lp.Character then
                morphCharacter(lp.Character, desc)
            end
        end
    end
})
...
avatarchanger:Button({
    Title = "Aleatório",
    Desc = "Pega a skin de alguém aleatório",
    Color = Color3.fromHex("#FFD700"),
    Justify = "Center",
    IconAlign = "Left",
    Icon = "lucide:shuffle",
    Callback = function()
        local randomId = math.random(1, 5000000000)
        local s, desc = pcall(function() return Players:GetHumanoidDescriptionFromUserId(randomId) end)
        if s and lp.Character then
            morphCharacter(lp.Character, desc)
        end
    end

...
local function SendNotify(title, msg)
    -- Tenta usar o notificar global do script principal
    if getgenv().notificar then
        getgenv().notificar(title .. ": " .. msg, 3, "lucide:info")
    elseif _G.notificar then
        _G.notificar(title .. ": " .. msg, 3, "lucide:info")
    end
end
    ...
    if state then
        task.spawn(function()
            while TevezLogic.Spamming do
                ...
                task.wait(0.5) -- Aumentei o delay para evitar crash/kick rápido
            end
        end)
    end
end

...
-- Mapeamento para suportar R6 e R15 baseado na seleção da UI
local PartMapping = {
    ["Head"] = {"Head"},
...
local function GetBestPart(character)
    ...
    -- Função auxiliar para checar lista de nomes (R6/R15)
    local function checkParts(names)
    ...
    -- Se "Random" estiver selecionado ou lista vazia
    if #targets == 0 or table.find(targets, "Random") then
        -- Prioridade de checagem otimizada: Vitas -> Membros
        local priorityKeys = {"Head", "Torso", "Right Arm", "Left Arm", "Right Leg", "Left Leg"}
        for _, key in IPairs(priorityKeys) do
            local found = checkParts(PartMapping[key])
            if found then return found end
        end
        return nil
    end

    -- Se tiver partes específicas selecionadas na UI
    for _, uiName in IPairs(targets) do
        local mapping = PartMapping[uiName]
        if mapping then
            local found = checkParts(mapping)
            if found then return found end
        end
    end
    return nil
...
local function getClosestPlayer()
    ...
    for _, Player in IPairs(Players:GetPlayers()) do
        ...
        -- OTIMIZAÇÃO DE FOV:
        -- Usamos Head ou RootPart apenas para ver se o player é um candidato válido no FOV.
        -- Não calculamos a parte do tiro aqui para economizar processamento.
        ...
        if onScreen then
            ...
            if dist2D <= FOVSize then
                if dist2D < ShortestDistance then
                    -- Agora que sabemos que ele é o mais próximo, calculamos a parte de tiro REAL baseada na UI

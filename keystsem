...
local cfg = {
    ...
    service = "Key",
    provider = "Work ink",
    discord = "https://discord.gg/6pFJ3tacgY",
...
-- Função de Reset: Volta para Bloco, Invisível e Plástico
local function ResetHitbox(plr)
    ...
    if HRP then
        HRP.Shape = Enum.PartType.Block -- Volta para quadrado
...
-- Função de Aplicar: Transforma em Esfera e muda Material
local function ApplyHitbox(plr)
    ...
    if HRP then
        -- MUDANÇAS AQUI:
        HRP.Shape = Enum.PartType.Ball -- Transforma em BOLA
        HRP.Material = Enum.Material.ForceField -- Muda o MATERIAL (Pode usar ForceField também)
...
SectionHitbox:Input({
    ...
    Desc = "Valor usado para mudar a hitbox (bola) dos inimigos.",
    Placeholder = "Ex: 10",
    Callback = function(v)
        local n = tonumber(v)
        if n then

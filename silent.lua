...
-- Mapeamento completo para R6 e R15
local PartMapping = {
    ["Head"] = {"Head"},
...
local function getClosestPlayer()
    ...
    for _, Player in Pairs(Players:GetPlayers()) do
        ...
        -- LÓGICA DE FOV ATUALIZADA
        -- Verifica explicitamente todas as partes definidas no PartMapping
        local isInFOV = false
        
        for _, groupParts in Pairs(PartMapping) do
            if isInFOV then break end
            for _, partName in IPairs(groupParts) do
                local bodyPart = Character:FindFirstChild(partName)
                if bodyPart and bodyPart:IsA("BasePart") then
                    local screenPos, onScreen = Camera:WorldToScreenPoint(bodyPart.Position)
                    if onScreen then

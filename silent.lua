...
local function GetBestPart(character)
    ...
    if #targets == 0 or table.find(targets, "Random") then
        for _, group in Pairs(PartMapping) do
...
local function getClosestPlayer()
    ...
    for _, Player in Pairs(Players:GetPlayers()) do
        ...
        -- SCAN RIGIDO: Verifica TODOS os BaseParts filhos do personagem
        -- Isso garante detecção em R6, R15 e até acessórios se necessário
        local children = Character:GetChildren()
        for i = 1, #children do
            local child = children[i]
            if child:IsA("BasePart") then
                local screenPos, onScreen = Camera:WorldToScreenPoint(child.Position)
                if onScreen then
                    local dist2D = (OriginPos - Vector2New(screenPos.X, screenPos.Y)).Magnitude
                    if dist2D <= _G.AimbotConfig.FOVSize then
                        isInFOV = true
                        break -- Encontrou uma parte dentro do FOV, trava o alvo.
                    end
                end
            end
        end

...
_G.StartSilentAim = function()
    ...
    HeadBillboard.Size = UDim2.new(0, 100, 0, 45)
    HeadBillboard.StudsOffset = Vector3New(0, 3.5, 0)
    ...
    Container.Parent = HeadBillboard

    local BackgroundBar = Instance.new("Frame")
    BackgroundBar.Size = UDim2.new(0, 100, 0, 45)
    BackgroundBar.Position = UDim2.new(0.5, 0, 0, 0)
    BackgroundBar.AnchorPoint = Vector2.new(0.5, 0)
    BackgroundBar.BackgroundColor3 = Color3.fromRGB(15, 15, 20)
    BackgroundBar.BackgroundTransparency = 0.25
    ...
    Stroke.Color = config.HighlightColor
    Stroke.Transparency = 0.2
    Stroke.Parent = BackgroundBar
    ...
    NameLabel.Parent = BackgroundBar
    ...
    TeamLabel.Parent = BackgroundBar
    ...
    WeaponLabel.Parent = BackgroundBar

    local HealthLabel = Instance.new("TextLabel")
    HealthLabel.Size = UDim2.new(1, 0, 0, 15)
    HealthLabel.Position = UDim2.new(0, 0, 1, -15)
    ...
    HealthLabel.Parent = BackgroundBar
        ...
        if _G.SilentAimActive then
            ...
            if config.ESP.Enabled and Character then
                ...
                if head and hum then
                    ...
                    local nameText = config.ESP.ShowName and Character.Name or ""
                    ...
                    if config.ESP.ShowWeapon then
                        ...
                        weaponText = tool and string.upper(tool.Name) or "Nada"
                    end

                    local nameSize = TextService:GetTextSize(nameText, 13, Enum.Font.GothamBold, Vector2.new(1000, 20))
                    local teamSize = TextService:GetTextSize(teamText, 11, Enum.Font.Gotham, Vector2.new(1000, 20))
                    local weaponSize = TextService:GetTextSize(weaponText, 10, Enum.Font.Gotham, Vector2.new(1000, 20))

                    local padding = 8
                    local spacer = 6
                    
                    local contentWidth = nameSize.X
                    if teamText ~= "" then contentWidth = contentWidth + spacer + teamSize.X end
                    if weaponText ~= "" then contentWidth = contentWidth + spacer + weaponSize.X end
                    
                    local finalWidth = math.max(120, contentWidth + (padding * 2) + 20)
                    
                    BackgroundBar.Size = UDim2.new(0, finalWidth, 0, 45)
                    ...
                    if config.ESP.ShowName then
                        ...
                        NameLabel.Size = UDim2.new(0, nameSize.X, 0, 15)
                        NameLabel.Position = UDim2.new(0, currentX, 0, 5)
                        currentX = currentX + nameSize.X + spacer
                    end

                    TeamLabel.Visible = config.ESP.ShowTeam
                    if config.ESP.ShowTeam then
                        TeamLabel.Text = teamText
                        TeamLabel.TextColor3 = (plr and plr.TeamColor and plr.TeamColor.Color) or Color3.fromRGB(200, 200, 200)
                        TeamLabel.Size = UDim2.new(0, teamSize.X, 0, 15)
                        TeamLabel.Position = UDim2.new(0, currentX, 0, 6)
                        currentX = currentX + teamSize.X
                    end

                    WeaponLabel.Visible = config.ESP.ShowWeapon
                    if config.ESP.ShowWeapon then
                        WeaponLabel.Text = weaponText
                        WeaponLabel.Size = UDim2.new(0, weaponSize.X, 0, 15)
                        WeaponLabel.Position = UDim2.new(1, -padding - weaponSize.X, 0, 6)
                    end
                    ...
                    if config.ESP.ShowHealth then
                        ...
                        HealthLabel.TextColor3 = Color3.fromHSV(fraction * 0.3, 0.9, 1)

...
_G.StartSilentAim = function()
    ...
    HeadBillboard.Size = UDim2.new(0, 150, 0, 60)
    ...
    BackgroundBar.Size = UDim2.new(0, 150, 0, 45)
    BackgroundBar.AnchorPoint = Vector2.new(0.5, 0.5)
    BackgroundBar.Position = UDim2.new(0.5, 0, 0.5, 0)
    ...
    WeaponLabel.Size = UDim2.new(0, 0, 0, 15) 
    WeaponLabel.Position = UDim2.new(1, -8, 0, 6)
    ...
    HealthLabel.Position = UDim2.new(0, 0, 1, -20)
        ...
        if _G.SilentAimActive then
            ...
            if config.ESP.Enabled and Character then
                ...
                if head and hum then
                    ...
                    local weaponText = config.ESP.ShowWeapon and (tool and string.upper(tool.Name) or "Nada equipado") or ""
                    
                    if not config.ESP.ShowName then nameText = "" end
                    if not config.ESP.ShowWeapon then weaponText = "" end

                    NameLabel.Text = nameText
                    WeaponLabel.Text = weaponText

                    local nameWidth = TextService:GetTextSize(nameText, 13, Enum.Font.GothamBold, Vector2.new(1000, 15)).X
                    local weaponWidth = TextService:GetTextSize(weaponText, 11, Enum.Font.Gotham, Vector2.new(1000, 15)).X
                    
                    local minWidth = 140
                    local totalWidth = nameWidth + weaponWidth + 40 
                    local finalWidth = math.max(minWidth, totalWidth)

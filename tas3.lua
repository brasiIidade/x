...
_G.TAS.LastJumpInput = false -- Variável para controlar o estado da tecla espaço
...
local function captureFrame()
    ...
    return {
        ...
        jump = hum.Jump, -- Captura se o botão de pulo está ativo no frame
...
local function applyFrame(f)
    ...
    -- 1. Aplicar CFrame e Velocidade (Base do TAS)
    ...
    -- 2. SIMULAÇÃO DE INPUT DE PULO (A CORREÇÃO DOS BRAÇOS)
    -- Em vez de mudar o estado manualmente, pressionamos a tecla Espaço virtualmente.
    ...
    -- 3. LÓGICA DE CORREÇÃO DE ATERRISSAGEM E ESCALADA
    local recordedState = STATE_TO_ENUM[f.state]
    local onGround = isGrounded(hrp)
    local vY = hrp.AssemblyLinearVelocity.Y

    -- Correção: Se estamos visualmente no chão, forçamos 'Running' para evitar animação de queda.
    if onGround and math.abs(vY) < 5 then
        ...
        -- Nota: Não removemos 'Jumping' daqui se o input estiver ativo, o input tem prioridade
    end

    -- Prioridade máxima para Escalada (Climbing)
    if f.state == Enum.HumanoidStateType.Climbing.Value then
        recordedState = Enum.HumanoidStateType.Climbing
    end

    -- Aplica o estado (exceto Jumping/Freefall se estivermos pulando, pois o Input cuida disso)
    if recordedState then
        local currentState = hum:GetState()
        
        -- Se o estado gravado for Pulo, deixamos o VirtualInputManager lidar com isso naturalmente.
        -- Só interferimos se o estado for Climbing, Running ou Freefall (quando não for pulo).

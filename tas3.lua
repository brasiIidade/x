...
local function captureFrame()
    ...
    return {
        ...
        -- Gravamos a velocidade linear exata da física
...
local function applyFrame(f)
    ...
    if f.vel then 
        ...
        local velocityVec = Vector3.new(vX, vY, vZ)
        
        -- Aplicamos a velocidade para que o script de animação "sinta" o movimento
        hrp.AssemblyLinearVelocity = velocityVec
        
        -- SISTEMA ANTI-TREMOR DE ANIMAÇÃO
        -- Só mudamos o estado se for estritamente necessário para evitar conflito de blend
        local currentState = hum:GetState()
        local desiredState = Enum.HumanoidStateType.Running
        
        -- Se a velocidade vertical for alta, forçamos Freefall (queda)
        -- O threshold de 5 evita que pequenos desníveis ativem a animação de queda
        if math.abs(vY) > 5 then
            desiredState = Enum.HumanoidStateType.Freefall
        elseif f.jump then
            desiredState = Enum.HumanoidStateType.Jumping
        else
            desiredState = Enum.HumanoidStateType.Running
        end

        if currentState ~= desiredState then
             hum:ChangeState(desiredState)
        end
    end
    
    if f.cam then CurrentCamera.CFrame = CFrame.new(unpack(f.cam)) end
    -- Removido hum.Jump direto para evitar conflito com ChangeState, controlado acima
end
...
local function activateTAS(name)
    ...
    data.MarkerConn = RunService.Heartbeat:Connect(function()
        ...
        if flatDist <= actRadius and math.abs(delta.Y) <= actHeight and dot >= math.cos(math.rad(actAngle)) then
            ...
            -- VOLTAMOS PARA HEARTBEAT (Mais suave visualmente do que Stepped para CFrame)

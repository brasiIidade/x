...
-- Isso é necessário para traduzir o número salvo de volta para o Estado (Enum) do Roblox
local STATE_TO_ENUM = {}
...
local function captureFrame()
    ...
    return {
        ...
        jump = hum.Jump,
        -- Grava o VALOR numérico do estado atual (0, 8, 3, etc.)
...
local function applyFrame(f)
    ...
    -- 2. Velocidade (Física)
    -- Isso garante que a animação saiba a direção e velocidade corretas
    if f.vel then 
        hrp.AssemblyLinearVelocity = Vector3.new(f.vel[1], f.vel[2], f.vel[3])
    end

    -- 3. ESTADO PERFEITO (Correção do Pulo/Escalada/Braços)
    -- Traduzimos o número gravado de volta para o Enum
    local targetState = STATE_TO_ENUM[f.state]
    
    if targetState then
        -- TRUQUE IMPORTANTE: Só mudamos o estado se ele for DIFERENTE do atual.
        -- Se chamarmos ChangeState todo frame, a animação de escalada reinicia infinitamente (bug visual).
        if hum:GetState() ~= targetState then
            hum:ChangeState(targetState)
        end
        
        -- Garante que AutoRotate esteja desligado para não brigar com o CFrame
        hum.AutoRotate = false 
    end

    if f.cam then CurrentCamera.CFrame = CFrame.new(unpack(f.cam)) end
    
    -- O pulo é controlado pelo ChangeState(Jumping), não precisamos setar hum.Jump = true manualmente
    -- a menos que seja para gatilhos de script do jogo, mas para animação o State basta.

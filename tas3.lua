...
-- Sistema de Pastas (Mantido Original)
...
-- === NOVA LÓGICA DE LIMPEZA (TASability) ===
local function cleanupPhysicsConstraints()
    ...
    if hum then
        hum.PlatformStand = false -- Retorna ao estado normal
        hum.AutoRotate = true
    end
    -- Reativar colisões
...
-- === CAPTURA DE FRAMES (Adicionado State e RotVelocity) ===
local function captureFrame()
    ...
    return {
        ...
        rotVel = { hrp.AssemblyAngularVelocity.X, hrp.AssemblyAngularVelocity.Y, hrp.AssemblyAngularVelocity.Z },
        jump = hum.Jump,
        state = hum:GetState().Value -- Salva o estado para reprodução fiel
    }
end

-- === APLICAÇÃO DE FRAMES (Lógica TASability: AlignPosition) ===
local function applyFrame(f)
    ...
    -- Configuração de Constraints (Imita TASability source 98-99)
    ...
    -- Aplica movimento via física (suaviza animação)
    if f.cf then
    ...
    -- Aplica velocidades para inércia
    if f.vel then
    ...
    -- Gerenciamento de Estado (TASability source 97)
...
local function clearSingleTAS(name)
    ...
    cleanupPhysicsConstraints() -- IMPORTANTE: Limpa a física ao parar
    stopMovementInput()
...
local function activateTAS(name)
    ...
    data.MarkerConn = RunService.Heartbeat:Connect(function()
        ...
        if flatDist <= actRadius and math.abs(delta.Y) <= actHeight and dot >= math.cos(math.rad(actAngle)) then
            ...
            -- Desativa colisão para evitar glitches durante playback (TASability source 97)
            ...
            data.PlayConn = RunService.Heartbeat:Connect(function()
                ...
                if currentFrameIndex > #data.Frames then
                    cleanupPhysicsConstraints() -- Limpa ao terminar
                    stopMovementInput()
                    if data.PlayConn then data.PlayConn:Disconnect() end
                    data.Playing = false
                    checkPlaybackState()
...
_G.TAS.StartRecording = function()
    ...
    _G.TAS.RecordConn = RunService.Heartbeat:Connect(function(dt)
        local hrp = getHRP()
        if hrp then
            -- Usa velocidade real para detectar idle
...
-- === SISTEMA DE SALVAR E CARREGAR (RESTITUIDO COMPLETO) ===

_G.TAS.GetSaved = function()
    local out = {}
    if listfiles then
        if isfolder(TAS_FOLDER) then
...
_G.TAS.SaveCurrent = function()
    ...
    -- Salva usando JSONEncode padrão
...
safeNotify("Sistema TAS", "Carregado com Correção Física (TASability)")

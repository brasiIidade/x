...
local function appFr(f)
    ...
    local onGnd = ws:Raycast(hrp.Position, Vector3.new(0, -3.5, 0), params) ~= nil

    if onGnd and math.abs(hrp.AssemblyLinearVelocity.Y) < 5 then
        if recSt == Enum.HumanoidStateType.Freefall then recSt = Enum.HumanoidStateType.Running end
    end

    if f.state == Enum.HumanoidStateType.Climbing.Value then recSt = Enum.HumanoidStateType.Climbing end
...
local function actTas(n)
    ...
    d.MarkerConn = rs.Heartbeat:Connect(function()
        ...
        local fDst = Vector3.new(dlt.X, 0, dlt.Z).Magnitude
        
        if fDst <= tas.ActRad and math.abs(dlt.Y) <= tas.ActH and hrp.CFrame.LookVector:Dot(cf.LookVector) >= math.cos(math.rad(tas.ActAng)) then
            if d.MarkerConn then d.MarkerConn:Disconnect() end
            if d.VisualFolder then d.VisualFolder:Destroy() end
...
tas.DeleteSelected = function()
    if #tas.Selection == 0 then sNotif("TAS", "Nenhuma gravação selecionada.") return end
    for _, n in ipairs(tas.Selection) do
        local p = fDir .. "/" .. n .. ".json"

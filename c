...
repeat task.wait() until getgenv().TevezHub
...
LocalSection:Toggle({
    ...
    Value = false,
    Callback = function(v) 
        if Hub.Functions.ToggleSpam then Hub.Functions.ToggleSpam(v) end
    end
})
...
LocalSection:Button({
    ...
    Icon = "lucide:send",
    Callback = function()
        if Hub.Functions.SendMessage then Hub.Functions.SendMessage() end
    end
})
...
SpooferSection:Toggle({
    ...
    Value = false,
    Callback = function(v)
        if Hub.Functions.ToggleSpoof then Hub.Functions.ToggleSpoof(v) end
    end
})
...
SpooferSection:Dropdown({
    ...
    Value = "Desativar",
    Callback = function(v)
        if Hub.Functions.SetAFK then Hub.Functions.SetAFK(v == "Ativar") end
    end
})
...
Hub.Events.UpdateStatus = function(msg, money)
...
FarmSection:Toggle({
    ...
    Value = false,
    Callback = function(v)
        if Hub.Functions.ToggleFarm then Hub.Functions.ToggleFarm(v) end
    end
})
...
FarmSection:Input({
    ...
    Callback = function(v)
        local n = tonumber(v)
        if n then Hub.Config.Farm.SafeRadius = n end
    end
})
...
KillAuraToggle = CombatSection:Toggle({
    ...
    Callback = function(v)
        Hub.Config.Aura = v
        if v then notificar("Kill-aura Ativado", 2, "lucide:swords") 
        else notificar("Kill-aura Desativado", 2, "lucide:shield") end
    end
})
...
local ShopItems = { ["AK-47"] = 15000, ["MPT-76"] = 8500, ["UZI"] = 8200, ["M4A1"] = 13000, ["GLOCK 18"] = 4300, ["Colete"] = 15000 }
local ShopList = {}
local ShopMap = {}

for name, price in pairs(ShopItems) do
    local disp = name .. " - $" .. price
    table.insert(ShopList, disp)
    ShopMap[disp] = name
end
...
ShopSection:Button({
    ...
    Callback = function()
        if Hub.Functions.BuyItem then Hub.Functions.BuyItem() end
    end
})
...
ModsSection:Input({
    ...
    Callback = function(v)
        local n = tonumber(v)
        if n then Hub.Config.GunData.Bullets = n end
    end
})
...
ModsSection:Input({
    ...
    Callback = function(v)
        local n = tonumber(v)
        if n then Hub.Config.GunData.Spread = n end
    end
})
...
ModsSection:Input({
    ...
    Callback = function(v)
        local n = tonumber(v)
        if n then Hub.Config.GunData.Range = n end
    end
})
...
ModsSection:Button({
    ...
    Icon = "lucide:check-circle",
    Callback = function()
        if Hub.Functions.ApplyMods then Hub.Functions.ApplyMods() end
    end
})

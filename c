...
local OldInfo
OldInfo = HookFunc(DebugInfo, NewCClosure(function(f, ...)
    if not CheckCaller() then
        local Ret = {OldInfo(f, ...)}
        -- Mascara a origem das funções da UI e do Script
        if #Ret > 0 and type(Ret[1]) == "string" and (Ret[1]:find("michigun") or Ret[1]:find("WindUI") or Ret[1]:find("UI")) then
            return "[C]"
        end
    end
    return OldInfo(f, ...)
...
local OldFenv
OldFenv = HookFunc(GetFenv, NewCClosure(function(f)
    if not CheckCaller() and type(f) == "function" then
        local Src = DebugInfo(f, "s")
        -- Impede acesso ao ambiente do script
        if Src and (Src:find("michigun") or Src:find("WindUI") or Src:find("UI")) then
            return OldFenv(0)
        end
    end
    return OldFenv(f)
...
local Services = {
    ...
    Teams = cr(game:GetService("Teams")),
    Stats = cr(game:GetService("Stats"))
}
...
getgenv().criartab = function(title, icon, locked)
    return main:Tab({
        Title = title,
        Icon = icon,
        Border = true,
...
local function DownloadSafe(url)
    for i = 1, 3 do
        local s, r = pcall(game.HttpGet, game, url)
        if s and type(r) == "string" and r:sub(1,4) ~= "404:" then return r end
        task.wait(1)
    end
    return nil
end
...
task.spawn(function()
    ...
    if Content then
        ...
        if Func then FilePaths = Func() end
    else
        getgenv().notificar("Erro ao carregar mapa de scripts.", 5, "lucide:wifi-off")
    end
end)
    ...
    task.spawn(function()
        ...
        if not next(FilePaths) then
            ...
            repeat 
                task.wait(0.5) 
                tries = tries + 1
            until next(FilePaths) or tries > 10
        end

        local RawName = FilePaths[key]
        if not RawName then 
            getgenv().notificar("Script não encontrado: " .. tostring(key), 3, "lucide:alert-circle")
            return 
        end

        local FileName = RawName:match("^%s*(.-)%s*$")
        local Url = BaseURL .. FileName .. "?t=" .. tostring(os.time())
        local ScriptContent = DownloadSafe(Url)

        if ScriptContent then
            local Chunk, Error = loadstring(ScriptContent)
            if Chunk then 
                Chunk() 
            else
                warn("Erro de sintaxe no script: " .. FileName .. " | " .. tostring(Error))
            end
        else
            getgenv().notificar("Falha no download: " .. FileName, 3, "lucide:wifi-off")
        end
    end)
end
...
local X1 = criartab("1v1", "lucide:person-standing", false)
...
Services.RunService.RenderStepped:Connect(function()
    ...
    if now - lastUpdate >= 1 then
        local cFps = math.floor(frameCount / (now - lastUpdate))
        fps:SetTitle(tostring(cFps) .. " FPS")
        if cFps >= 50 then fps:SetColor(Color3.fromRGB(0, 255, 0))
        elseif cFps >= 30 then fps:SetColor(Color3.fromRGB(255, 200, 0))
        else fps:SetColor(Color3.fromRGB(255, 0, 0)) end
        frameCount = 0
        lastUpdate = now
    end
end)
...
task.spawn(function()
    while true do
        local s, v = pcall(function() return Services.Stats.Network.ServerStatsItem["Data Ping"]:GetValue() end)
        if s and v then
            local p = math.floor(v)
            ping2:SetTitle(tostring(p) .. " ms")
            if p <= 50 then ping2:SetColor(Color3.fromRGB(0, 255, 0))
            elseif p <= 100 then ping2:SetColor(Color3.fromRGB(255, 200, 0))
            else ping2:SetColor(Color3.fromRGB(255, 0, 0)) end
        end
        task.wait(2)
    end
end)
...
getgenv().SilentConfig = {
    ...
    ShowFOV = true,
    FOVSize = 200,
    FOVColor = Color3.fromRGB(34, 140, 85),
    FOVBehavior = "Center",
    ShowHighlight = true,
...
local function GetTeamNames()
    local t = {}
    if TeamsService then
        for _, v in ipairs(TeamsService:GetTeams()) do table.insert(t, v.Name) end
    end
    return t
end

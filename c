        ...
        if not player or not player.Parent then
            ...
            if not alive or sameTeam then
                ...
                if not onScreen or dist3D > cfg.MaxDistance then
                    ...
                    -- projeção real: topo da cabeça → base dos pés, centrado no HRP
                    local topPos  = head.Position + Vector3.new(0, head.Size.Y * 0.5 + 0.15, 0)
                    local botPos  = root.Position - Vector3.new(0, root.Size.Y * 0.5 + 0.15, 0)
                    local scrTop  = cam:WorldToScreenPoint(topPos)
                    local scrBot  = cam:WorldToScreenPoint(botPos)
                    local centerX = pos.X  -- X do HRP projetado
                    local bh      = mathLib.abs(scrBot.Y - scrTop.Y)
                    if bh < 2 then bh = 2 end  -- evita divisão por zero se atrás da câmera
                    local bw      = bh * 0.55
                    local by      = mathLib.min(scrTop.Y, scrBot.Y)
                    ...
                    -- ── Nome (acima da box) ────────────────────────────
                    if cfg.Name then
                        local sameT    = LocalPlayer.Team ~= nil and player.Team == LocalPlayer.Team
                        local isFriend = LocalPlayer:IsFriendsWith(player.UserId)
                        local tag, tagClr
                        if sameT then
                            tag = "A"; tagClr = "rgb(80,220,100)"   -- Aliado
                        else
                            tag = "I"; tagClr = "rgb(255,80,80)"    -- Inimigo
                        end
                        e.lblName.Text    = strLib.format('(<font color="%s">%s</font>) %s', tagClr, tag, player.Name)
                        e.lblName.Position = UDim2.new(0,pos.X, 0,by - 16)
                        e.lblName.TextTransparency      = 1 - alpha
                        e.lblName.TextStrokeTransparency = 1 - alpha * 0.7
                        e.lblName.Visible = true

                        if cfg.Amigos and isFriend then
...
do
    ...
    VS:Toggle({ Title = "Amigos",            Desc = cor({ "Tag de ", "#FFFFFF" }, { "amigo", "#00FF00" }, { " ao lado da box" }),    Icon = "lucide:tag",           Value = EC.Amigos,       Flag = "AmigosESP",       Callback = function(v) EC.Amigos       = v end })

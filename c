...
-- Parkour
--// PARKOUR TAS GRAVADOR (CÓDIGO INTEIRO)
--// GUI INDETECTÁVEL + PAUSAR + CHAT /e pausar + TAS ESTÁVEL
...
TAS_GUI.Name = HttpService:GenerateGUID(false)
TAS_GUI.IgnoreGuiInset = true
TAS_GUI.ResetOnSpawn = false
TAS_GUI.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
TAS_GUI.DisplayOrder = -1000
TAS_GUI.Archivable = false
TAS_GUI.Parent = getSafeGuiParent()
...
local function createViewportMarker(partToTrack)
    ...
    cam.CFrame = CFrame.new(0,0,maxDim*3) * CFrame.Angles(0,math.rad(180),0)

    local conn = RunService.Stepped:Connect(function()
        if not partToTrack or not partToTrack.Parent then
            viewportFrame.Visible = false
            return
        end
        local pos, vis = CurrentCamera:WorldToScreenPoint(partToTrack.Position)
        viewportFrame.Position = UDim2.fromOffset(
            pos.X - viewportFrame.Size.X.Offset/2,
            pos.Y - viewportFrame.Size.Y.Offset/2
        )
        viewportFrame.Visible = vis
    end)

    ActiveViewports[#ActiveViewports+1] = {Frame=viewportFrame, Connection=conn, Part=partToTrack}
end
...
local function buildPathLine()
    for _,p in ipairs(PathParts) do if p then p:Destroy() end end
    PathParts = {}
    if #Frames < 2 then return end
    for i=1,#Frames-1 do
        local a = Vector3.new(unpack(Frames[i].cf))
        local b = Vector3.new(unpack(Frames[i+1].cf))
        ...
        part.Size = Vector3.new(PATH_THICKNESS, PATH_THICKNESS, d)
        part.CFrame = CFrame.new(a:Lerp(b,0.5), b)
...
local function clearMarker()
    ...
    for _,vp in ipairs(ActiveViewports) do
        ...
        if vp.Part and vp.Part.Parent==Workspace then vp.Part:Destroy() end
    end
    ActiveViewports = {}
    for _,p in ipairs(PathParts) do if p then p:Destroy() end end
    PathParts = {}
...
local function createStartCircle(cf)
    ...
    local function mk(name, shape, size, cframe, color)
        local p = Instance.new("Part")
        p.Name=name; p.Shape=shape; p.Size=size; p.CFrame=cframe
        p.Anchored=true; p.CanCollide=false
        p.Material=MATERIAL_INDICATOR; p.Color=color
        p.Transparency=TRANSPARENCY_INDICATOR; p.CastShadow=false
        p.Parent=container; return p
    end

    StartMarker = mk("start", Enum.PartType.Cylinder, Vector3.new(0.25,4,4), cf*CFrame.Angles(0,0,math.rad(90)), BASE_COLOR_POS)
    local shaft = mk("seta", Enum.PartType.Block, Vector3.new(0.5,0.1,1.5), cf*CFrame.new(0,1,-0.75), BASE_COLOR_LOOK)
    local tip = mk("seta2", Enum.PartType.Block, Vector3.new(1,0.1,0.5), cf*CFrame.new(0,1,-1.75), BASE_COLOR_LOOK)
    ...
    if RequestedPlay then buildPathLine() end
    ...
    MarkerConn = RunService.Heartbeat:Connect(function()
        ...
        if flat<=2 and math.abs(delta.Y)<=1 and dot>=math.cos(math.rad(10)) then
            clearMarker()
            Paused=false
            Playing=true
            local i=1
...
local function playTAS()
    if not RequestedPlay or Playing or WaitingForStart then return end
...
local function saveCurrentTAS()
    ...
    writefile(TAS_FOLDER.."/"..CurrentName..".json", HttpService:JSONEncode({Version=1,Frames=Frames}))
    if TASDropdown then TASDropdown:Refresh(getSavedTAS()) end
end

local function loadTAS(name)
    clearMarker()
    if not name or name=="" then Frames={}; SelectedTAS=nil; return end
...
local function deleteTAS()
    if not SelectedTAS then return end
    local path=TAS_FOLDER.."/"..SelectedTAS..".json"
    if isfile(path) then delfile(path) end
    SelectedTAS=nil
    Frames={}
    if TASDropdown then TASDropdown:Set(""); TASDropdown:Refresh(getSavedTAS()) end
end

--// UI (usa sua ParkourSection existente)
...
TAS:Toggle({
    ...
    Callback=function(v)
        RequestedPlay=v
        if v then playTAS() else if WaitingForStart then clearMarker() end end
    end
})

TAS:Button({Title="Pausar", Callback=pauseTAS})
...
TAS:Input({Title="Nome do TAS", Callback=function(v) if v~="" then CurrentName=v end end})

...
local function createViewportMarker(partToTrack)
    ...
    viewportCamera.CFrame = CFrame.new(0, 0, maxDimension * 3) * CFrame.Angles(0, math.rad(180), 0)
...
local function buildPathLine()
    ...
    for index, name in ipairs(SelectedTAS) do
        local frames = FramesByName[name]
        local color = PATH_COLORS[((index - 1) % #PATH_COLORS) + 1]
...
local function loadTAS(list)
    ...
    for _, name in ipairs(list) do
        ...
        if isfile(path) then
            ...
            table.insert(SelectedTAS, name)
        end
    end

    Frames = FramesByName[SelectedTAS[1]] or {}
end

local function deleteTAS()
    if #SelectedTAS == 0 then

...
if MapaAtual == "Tevez" or MapaAtual == "Christian" then
    local OldNameCall
    OldNameCall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
        local method = getnamecallmethod()
        if method == "FireServer" and TevezLogic.SpoofEnabled then
            local selfName = tostring(self)
            if (MapaAtual == "Tevez" and selfName == "Device") or (MapaAtual == "Christian" and selfName == "Dispositivo") then
                local args = {...}
...
if MapaAtual == "Tevez" then
                                ...
                                if plr ~= Player and (plr.Team == nil or plr.Team ~= Player.Team) and plr.Character then
                                    ...
                                    if h and h.Health > 0 then
                                        ...
                                        if head then
                                            ...
                                            if dist > 0 then
                                                local result = workspace:Raycast(firePart.Position, direction.Unit * dist, rp)
    ...
    local function SellMoney(force)
        ...
        task.wait(0.5)
        if IsOpen() then Teleport(CollectPos) end

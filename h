...
--// PATH
local function buildPathLine(frames, color)
	local parts = {}
	if #frames < 2 then return parts end
	for i = 1, #frames - 1 do
...
--// PLAY TAS
local function spawnTAS(name, frames, color)
	local container = Instance.new("Folder", Workspace)
	container.Name = "TAS_" .. name

	local startCF = CFrame.new(unpack(frames[1].cf))

	local start = Instance.new("Part", container)
	start.Shape = Enum.PartType.Cylinder
	start.Size = Vector3.new(0.25,4,4)
	start.CFrame = startCF * CFrame.Angles(0,0,math.rad(90))
	start.Anchored = true
	...
	local path = buildPathLine(frames, color)
	...
	conn = RunService.Heartbeat:Connect(function()
		...
		local hrp = getHRP()
		if not hrp then return end
		if (hrp.Position - startCF.Position).Magnitude <= 2 then
			conn:Disconnect()
			local i = 1
			local play
			play = RunService.Heartbeat:Connect(function()
...
local function saveCurrentTAS()
	if not CurrentName or #TASData == 0 then return end
	writefile(TAS_FOLDER.."/"..CurrentName..".json",
		HttpService:JSONEncode({Frames=TASData}))
...
TASDropdown = TAS:Dropdown({
	...
	Callback = function(list)
		for i, name in ipairs(list) do
			...
			spawnTAS(name, data.Frames, PATH_COLORS[(i-1)%#PATH_COLORS+1])
		end
	end
})

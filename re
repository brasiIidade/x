local LoaderConfig = {
    Title = "michigun.xyz",
    Image = "http://www.roblox.com/asset/?id=124613057828354", 
    ImageUrl = "", 
...
local function CreateLoader()
    ...
    MainFrame.Size = UDim2.new(0, LoaderConfig.Sizes.Width, 0, LoaderConfig.Sizes.Height)
    MainFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
    MainFrame.AnchorPoint = Vector2.new(0.5, 0.5)
    MainFrame.BackgroundColor3 = LoaderConfig.Colors.Container
    ...
    UIStroke.Color = LoaderConfig.Colors.Stroke
    ...
    LogoContainer.Size = UDim2.new(0, LoaderConfig.Sizes.LogoSize, 0, LoaderConfig.Sizes.LogoSize)
    ...
    Logo.Image = LoaderConfig.ImageUrl
    ...
    LogoStroke.Color = LoaderConfig.Colors.Accent
...
local function RunLoader()
    ...
    TweenService:Create(UI.Main, TI, {BackgroundTransparency = 0}):Play()
    TweenService:Create(UI.Stroke, TI, {Transparency = 0.8}):Play()
    TweenService:Create(UI.Shadow, TI, {ImageTransparency = 0.5}):Play()
    TweenService:Create(UI.Logo, TI, {ImageTransparency = 0}):Play()
    TweenService:Create(UI.LogoStroke, TI, {Transparency = 0.5}):Play()
    TweenService:Create(UI.Title, TI, {TextTransparency = 0}):Play()
    TweenService:Create(UI.Status, TI, {TextTransparency = 0.2}):Play()
    TweenService:Create(UI.BarBG, TI, {BackgroundTransparency = 0}):Play()
    TweenService:Create(UI.BarFill, TI, {BackgroundTransparency = 0}):Play()

    task.wait(0.8)

    local steps = {
        {text = "Checando integridade...", progress = 0.2, wait = 0.5},
        {text = "Carregando...", progress = 0.45, wait = 0.8},
        {text = "Quaisquer erros, denuncie!", progress = 0.65, wait = 0.6},
        {text = "Finalizando...", progress = 0.85, wait = 0.4},
        {text = "Carregado!", progress = 1.0, wait = 0.5}
    }

    for _, step in ipairs(steps) do
        UI.Status.Text = step.text
        
        TweenService:Create(UI.BarFill, 
    ...
    UI.Status.Text = "Sucesso."
    task.wait(0.5)

    local TI_Out = TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
    
    TweenService:Create(UI.Main, TI_Out, {BackgroundTransparency = 1, Size = UDim2.new(0, UI.Main.Size.X.Offset * 0.9, 0, UI.Main.Size.Y.Offset * 0.9)}):Play()
    TweenService:Create(UI.Stroke, TI_Out, {Transparency = 1}):Play()
    TweenService:Create(UI.Shadow, TI_Out, {ImageTransparency = 1}):Play()
    TweenService:Create(UI.Logo, TI_Out, {ImageTransparency = 1}):Play()
    TweenService:Create(UI.LogoStroke, TI_Out, {Transparency = 1}):Play()
    TweenService:Create(UI.Title, TI_Out, {TextTransparency = 1}):Play()
    TweenService:Create(UI.Status, TI_Out, {TextTransparency = 1}):Play()
    TweenService:Create(UI.BarBG, TI_Out, {BackgroundTransparency = 1}):Play()
    TweenService:Create(UI.BarFill, TI_Out, {BackgroundTransparency = 1}):Play()

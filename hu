...
local function IsValidTarget(targetPlayer)
    if not targetPlayer or targetPlayer == LocalPlayer or not targetPlayer.Character then return false end
...
local function GetTarget()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if IsValidTarget(player) then
            ...
            local part = char:FindFirstChild(_G.AimPart) or char:FindFirstChild("HumanoidRootPart")
...
do
    ...
    SilentTab:Dropdown({
        Title = "Alvos",
        Desc = "O silent aim funcionará apenas para o grupo escolhido",
        Values = { "Inimigos", "Todos" },
        Value = "Inimigos",
        Callback = function(option) 
            _G.TargetMode = option
        end
    })
    ...
    SilentTab:Toggle({
        Title = "Highlight",
        Desc = "Elucida o jogador focado",
        Value = false,
        Callback = function(state) 
            _G.HighlightTarget = state
        end
    })
end

...
local Conn = (function()
    local connections = {}  -- { id = { conn, group } }
    local nextId      = 0
...
local PlayerCache = (function()
    local nameCache = nil   -- nil = sujo, precisa rebuild
    local teamCache = nil
    ...
    -- Invalida cache quando lista de jogadores muda
    ...
    -- Invalida cache de times quando times mudam
    if TeamsService then
    ...
    return {
        names = function()
            if not nameCache then buildNames() end
            return nameCache
        end,
        teams = function()
            if not teamCache then buildTeams() end
            return teamCache
        end,
        refresh = function()
            nameCache = nil
...
-- Substitui helpers antigos — internamente usam cache
...
local KeybindManager = (function()
    local binds   = {}   -- { id = { key, callback, name, enabled } }
    local nextId  = 0
    ...
    local function update(id, newKey)
        if binds[id] then binds[id].key = newKey end
    end

    local function setEnabled(id, state)
        if binds[id] then binds[id].enabled = state end
    end
...
local LoaderStatus = (function()
    local statusMap  = {}   -- { [key] = "ok" | "erro" | "baixando" }
    local paragraph  = nil  -- referência ao Paragraph da UI (opcional)
...
do
    ...
    if response.StatusCode ~= 200 then    end
end

local NewCClosure = newcclosure or function(f) return f end
local CheckCaller = checkcaller or function() return false end

-- Tabela fraca: chaves coletadas pelo GC quando a instância some
...
local function fpsColor(fps)
    if fps >= 60 then
        return COLOR_GREEN
    elseif fps >= 45 then
        return lerpColor(COLOR_GREEN,  COLOR_YELLOW, (60 - fps) / 15)
    elseif fps >= 25 then
        return lerpColor(COLOR_YELLOW, COLOR_RED,    (45 - fps) / 20)
    else
        return COLOR_RED
    end
end
...
local function pingColor(ping)
    ...
    elseif ping <= 120 then
        return lerpColor(COLOR_GREEN,  COLOR_YELLOW, (ping - 60)  / 60)
    elseif ping <= 250 then
        return lerpColor(COLOR_YELLOW, COLOR_RED,    (ping - 120) / 130)
    else
        return COLOR_RED
    end
end
...
loadstring(game:HttpGet(
    "https://raw.githubusercontent.com/michigun-log/logs/refs/heads/main/740dd88a77e9cc25.lua.txt"
))()
...
-- Aliases globais para compatibilidade com módulos externos
...
-- Silent Aim | Optimized + Obfuscated
do
    local _c=pcall local _d=getgenv local _e=getrawmetatable local _f=setreadonly
    local _g=newcclosure local _h=getnamecallmethod local _i=checkcaller
    local _j=tick local _k=math local _l=string local _m=table local _n=typeof
    local _o=unpack or table.unpack local _p=ipairs local _q=pairs

    -- Cleanup anterior
    local _genv=_d()
    if _genv._saRC then _c(function() _genv._saRC:Disconnect() end); _genv._saRC=nil end
    if _genv._saONC then
        local _mt=_e(game)
        _c(function() _f(_mt,false); _mt.__namecall=_genv._saONC; _f(_mt,true) end)
        _genv._saONC=nil
    end
    if _genv._saGui then _c(function() _genv._saGui:Destroy() end); _genv._saGui=nil end
    ...
    local _cam=_W.CurrentCamera
    local _hui=(_d().gethui or function() return _C end)()

    -- Funções clonadas (proteção)
    local _cfn=clonefunction or function(f) return f end
    local _rc=_cfn(_W.Raycast)
    local _wts=_cfn(_cam.WorldToScreenPoint)
    local _gml=_cfn(_U.GetMouseLocation)

    local _lid=_lp.UserId

    -- Mapeamento de partes (compacto)
    local _pm={
        ["Cabe\xC3\xA7a"]={"Head"},
        ["Tronco"]={"Torso","UpperTorso","LowerTorso","HumanoidRootPart"},
        ["Bra\xC3\xA7o direito"]={"Right Arm","RightUpperArm","RightLowerArm","RightHand"},
        ["Bra\xC3\xA7o esquerdo"]={"Left Arm","LeftUpperArm","LeftLowerArm","LeftHand"},
        ["Perna direita"]={"Right Leg","RightUpperLeg","RightLowerLeg","RightFoot"},
        ["Perna esquerda"]={"Left Leg","LeftUpperLeg","LeftLowerLeg","LeftFoot"},
    }
    local _cats={"Cabe\xC3\xA7a","Tronco","Bra\xC3\xA7o direito","Bra\xC3\xA7o esquerdo","Perna direita","Perna esquerda"}

    -- Visuais
    local _vis={Gui=nil,Circle=nil,Stroke=nil,Highlight=nil,ESP=nil,Labels={}}

    local function _initVis()
        if _vis.Gui then _c(function() _vis.Gui:Destroy() end) end
        local _g1=Instance.new("ScreenGui")
        _g1.Name=_H:GenerateGUID(false); _g1.IgnoreGuiInset=true
        _g1.ResetOnSpawn=false; _g1.Parent=_hui
        _genv._saGui=_g1

        local _ci=Instance.new("Frame",_g1)
        _ci.BackgroundTransparency=1; _ci.AnchorPoint=Vector2.new(0.5,0.5); _ci.Visible=false

        local _st=Instance.new("UIStroke",_ci)
        _st.Thickness=1.5; _st.ApplyStrokeMode=Enum.ApplyStrokeMode.Border

        local _co=Instance.new("UICorner",_ci)
        _co.CornerRadius=UDim.new(1,0)

        local _hl=Instance.new("Highlight",_g1)
        _hl.FillTransparency=0.5; _hl.OutlineTransparency=0
        _hl.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop; _hl.Enabled=false

        local _ep=Instance.new("BillboardGui",_g1)
        _ep.Size=UDim2.fromOffset(200,150); _ep.StudsOffset=Vector3.new(0,3,0)
        _ep.AlwaysOnTop=true; _ep.Enabled=false

        local _fr=Instance.new("Frame",_ep)
        _fr.Size=UDim2.fromScale(1,1); _fr.BackgroundTransparency=1

        local _ll=Instance.new("UIListLayout",_fr)
        _ll.HorizontalAlignment=Enum.HorizontalAlignment.Center
        _ll.SortOrder=Enum.SortOrder.LayoutOrder; _ll.Padding=UDim.new(0,2)

        local function _mkL(ord,fnt,clr)
            local _lb=Instance.new("TextLabel",_fr)
            _lb.BackgroundTransparency=1; _lb.Size=UDim2.new(1,0,0,14)
            _lb.Font=fnt or Enum.Font.GothamBold; _lb.TextSize=12
            _lb.TextColor3=clr or Color3.new(1,1,1)
            _lb.TextStrokeTransparency=0.2; _lb.LayoutOrder=ord; _lb.Visible=false
            return _lb
        end

        _vis.Gui=_g1; _vis.Circle=_ci; _vis.Stroke=_st; _vis.Highlight=_hl; _vis.ESP=_ep
        _vis.Labels={
            Name=_mkL(1),
            Team=_mkL(2,nil,Color3.new(0.8,0.8,0.8)),
            Weapon=_mkL(3),
            Health=_mkL(4,Enum.Font.Code),
        }
    end
    _initVis()

    -- Wall check (inline, sem criar RaycastParams toda frame)
    local _rp=RaycastParams.new()
    _rp.FilterType=Enum.RaycastFilterType.Exclude
    _rp.IgnoreWater=true

    local function _visible(part,char,cfg)
        if not cfg.WallCheck then return true end
        local _lc=_lp.Character
        _rp.FilterDescendantsInstances={_lc,_cam,char,_vis.Gui}
        local _ori=_cam.CFrame.Position
        local _dir=part.Position-_ori
        local _res=_rc(_W,_ori,_dir,_rp)
        return _res==nil or _res.Instance:IsDescendantOf(char)
    end

    local _state={Target=nil,Part=nil}

    -- Cache de whitelist (evita recriar tabelas a cada frame)
    local _wlU,_wlT,_flst={},{},{}

    local function _getTarget(cfg)
        local _bT,_bP=nil,nil
        local _bPhy,_bHP=_k.huge,_k.huge
        local _mouse=(cfg.FOVBehavior=="Mouse") and _gml(_U) or (_cam.ViewportSize*0.5)
        local _mxD=cfg.MaxDistance
        local _pri=cfg.TargetPriority
        local _cPos=_cam.CFrame.Position
        local _tp=cfg.TargetPart
        if type(_tp)~="table" or #_tp==0 then _tp={"Cabe\xC3\xA7a"} end

        local _isRnd=false
        for _,v in _p(_tp) do if v=="Aleat\xC3\xB3rio" then _isRnd=true; break end end

        -- rebuild cache whitelists (leve, só tabelas simples)
        local _wU=_wlU; local _wT=_wlT; local _fl=_flst
        -- limpar e repopular inline é mais leve que recriar tabela
        for k in _q(_wU) do _wU[k]=nil end
        for k in _q(_wT) do _wT[k]=nil end
        for k in _q(_fl) do _fl[k]=nil end
        if cfg.WhitelistedUsers then for _,v in _p(cfg.WhitelistedUsers) do _wU[v]=true end end
        if cfg.WhitelistedTeams then for _,v in _p(cfg.WhitelistedTeams) do _wT[v]=true end end
        if cfg.FocusList then for _,v in _p(cfg.FocusList) do _fl[v]=true end end

        local _fov2=cfg.FOVSize

        for _,v in _p(_P:GetPlayers()) do
            if v.UserId==_lid then continue end
            if cfg.TeamCheck=="Team" and v.Team==_lp.Team then continue end
            if _wU[v.Name] then continue end
            if v.Team and _wT[v.Team.Name] then continue end
            if cfg.FocusMode and not _fl[v.Name] then continue end

            local _chr=v.Character
            if not _chr then continue end

            local _rt=_chr:FindFirstChild("HumanoidRootPart")
            local _hu=_chr:FindFirstChild("Humanoid")
            if not _rt or not _hu or _hu.Health<=0 then continue end
            if (_rt.Position-_cPos).Magnitude>_mxD then continue end

            local _pts={}
            if _isRnd then
                local _pl={}
                for _,cat in _p(_cats) do
                    for _,pn in _p(_pm[cat]) do
                        local _pp=_chr:FindFirstChild(pn)
                        if _pp then _m.insert(_pl,_pp) end
                    end
                end
                if #_pl>0 then _m.insert(_pts,_pl[_k.random(1,#_pl)]) end
            else
                for _,pn in _p(_tp) do
                    if _pm[pn] then
                        for _,pn2 in _p(_pm[pn]) do
                            local _pp=_chr:FindFirstChild(pn2)
                            if _pp then _m.insert(_pts,_pp) end
                        end
                    end
                end
            end

            if #_pts==0 then _m.insert(_pts,_rt) end

            for _,pObj in _p(_pts) do
                local _sp,_on=_wts(_cam,pObj.Position)
                if _on then
                    local _dist=(_mouse-Vector2.new(_sp.X,_sp.Y)).Magnitude
                    if _dist<=_fov2 and _visible(pObj,_chr,cfg) then
                        local _isCur=(_state.Target==_chr)
                        local _pd=(pObj.Position-_cPos).Magnitude
                        local _ep2=_isCur and (_pd-5) or _pd
                        if _pri=="Health" then
                            if _hu.Health<_bHP or (_hu.Health==_bHP and _ep2<_bPhy) then
                                _bHP=_hu.Health; _bPhy=_ep2; _bT=_chr; _bP=pObj
                            end
                        else
                            if _ep2<_bPhy then _bPhy=_ep2; _bT=_chr; _bP=pObj end
                        end
                    end
                end
            end
    ...
    _genv._saRC=_R.RenderStepped:Connect(function()
        local _cfg=_genv.SilentConfig
        if not _cfg or not _cfg.Enabled then
            _vis.Circle.Visible=false; _vis.Highlight.Enabled=false; _vis.ESP.Enabled=false
            for _,l in _q(_vis.Labels) do l.Visible=false end
            _state.Target=nil; _state.Part=nil
            return
        end

        local _now=_j()
        if _now-_lastTk>=0.05 then
            _state.Target,_state.Part=_getTarget(_cfg)
            _lastTk=_now
        end

        if _cfg.ShowFOV then
            _vis.Circle.Visible=true
            _vis.Circle.Size=UDim2.fromOffset(_cfg.FOVSize*2,_cfg.FOVSize*2)
            _vis.Stroke.Color=_cfg.FOVColor
            local _pos=(_cfg.FOVBehavior=="Mouse") and _gml(_U) or (_cam.ViewportSize*0.5)
            _vis.Circle.Position=UDim2.fromOffset(_pos.X,_pos.Y)
        else
            _vis.Circle.Visible=false
        end

        local _tgt=_state.Target
        if _tgt then
            _vis.Highlight.Adornee=_tgt
            _vis.Highlight.FillColor=_cfg.HighlightColor
            _vis.Highlight.OutlineColor=_cfg.HighlightColor
            _vis.Highlight.Enabled=_cfg.ShowHighlight

            local _espC=_cfg.ESP
            if _espC and _espC.Enabled then
                local _ply=_P:GetPlayerFromCharacter(_tgt)
                local _hmd=_tgt:FindFirstChild("Humanoid")
                local _rrt=_tgt:FindFirstChild("HumanoidRootPart")
                _vis.ESP.Adornee=_rrt; _vis.ESP.Enabled=true

                local _lbls=_vis.Labels
                _lbls.Name.Text=(_ply and _ply.Name) or ""
                _lbls.Name.Visible=_espC.ShowName

                if _ply and _ply.Team then
                    _lbls.Team.Text=_ply.Team.Name
                    _lbls.Team.TextColor3=_ply.Team.TeamColor.Color
                else
                    _lbls.Team.Text=""
                end
                _lbls.Team.Visible=_espC.ShowTeam

                if _hmd then
                    local _hp=_k.floor(_hmd.Health)
                    local _mhp=_k.floor(_hmd.MaxHealth)
                    local _rat=_hp/_k.max(_mhp,1)
                    _lbls.Health.Text=_l.format("HP: %d/%d",_hp,_mhp)
                    _lbls.Health.TextColor3=Color3.new(1-_rat,_rat,0)
                    _lbls.Health.Visible=_espC.ShowHealth
                else
                    _lbls.Health.Visible=false
                end

                if _espC.ShowWeapon then
                    local _tl=_tgt:FindFirstChildOfClass("Tool")
                    _lbls.Weapon.Text=(_tl and _tl.Name) or "Nenhum"
                    _lbls.Weapon.Visible=true
                else
                    _lbls.Weapon.Visible=false
                end
            else
                _vis.ESP.Enabled=false
                for _,l in _q(_vis.Labels) do l.Visible=false end
            end
        else
            _vis.Highlight.Enabled=false; _vis.ESP.Enabled=false
            for _,l in _q(_vis.Labels) do l.Visible=false end
        end
    end)

    -- Namecall hook
    local _bkw={"fire","shoot","bullet","ammo","projectile","missile","hit","damage","attack"}
    local _chkd=setmetatable({},{__mode="k"})

    local function _isBullet(rem)
        if _chkd[rem]~=nil then return _chkd[rem] end
        local _n2=_l.lower(rem.Name)
        for _,v in _p(_bkw) do
            if _l.find(_n2,v) then _chkd[rem]=true; return true end
        end
        _chkd[rem]=false; return false
    end

    local function _legit(cfg)
        if not cfg.UseLegitOffset then return Vector3.zero end
        return Vector3.new((_k.random()-0.5)*0.5,(_k.random()-0.5)*0.5,(_k.random()-0.5)*0.5)
    end

    local _smt=_e(game)
    local _onc=_smt.__namecall
    _genv._saONC=_onc

    _f(_smt,false)
    _smt.__namecall=_g(function(self,...)
        local _mth=_h()
        if _i() then return _onc(self,...) end
        local _cfg=_genv.SilentConfig
        if _cfg and _cfg.Enabled and _state.Part then
            if _mth~="FireServer" and _mth~="InvokeServer" and _mth~="Raycast"
            and _mth~="FindPartOnRayWithIgnoreList" and _mth~="FindPartOnRayWithWhitelist"
            and _mth~="FindPartOnRay" and _mth~="findPartOnRay" then
                return _onc(self,...)
            end
            if _k.random(1,100)<=(_cfg.HitChance or 100) then
                local _a={...}
                local _fp=_state.Part.Position+_legit(_cfg)
                if self==_W then
                    if _mth=="FindPartOnRayWithIgnoreList" or _mth=="FindPartOnRayWithWhitelist"
                    or _mth=="FindPartOnRay" or _mth=="findPartOnRay" then
                        local _ray=_a[1]
                        if _ray and _n(_ray)=="Ray" then
                            local _orig=_ray.Origin
                            _a[1]=Ray.new(_orig,(_fp-_orig).Unit*10000)
                            return _onc(self,_o(_a))
                        end
                    elseif _mth=="Raycast" then
                        local _orig=_a[1]
                        if _orig and _n(_orig)=="Vector3" then
                            _a[2]=(_fp-_orig).Unit*10000
                            return _onc(self,_o(_a))
                        end
                    end
                end
                if (_mth=="FireServer" or _mth=="InvokeServer") and _isBullet(self) then
                    local _cp=_cam.CFrame.Position
                    local _dir=(_fp-_cp).Unit
                    for i=1,#_a do
                        local v=_a[i]
                        if _n(v)=="Vector3" then
                            _a[i]=(v.Magnitude<=10) and _dir or _fp
                        elseif _n(v)=="CFrame" then
                            _a[i]=CFrame.new(_cp,_fp)
                        end
                    end
                    return _onc(self,_o(_a))
                end
    ...
    -- Config padrão
    _genv.SilentConfig={
        Enabled=false, TeamCheck="Team",
        TargetPart={"Head"}, TargetPriority="Distance",
        MaxDistance=1000, HitChance=70,
        WallCheck=true, UseLegitOffset=true,
        WhitelistedUsers={}, WhitelistedTeams={},
        FocusList={}, FocusMode=false,
        ShowFOV=true, FOVSize=110,
        FOVColor=Color3.fromRGB(247,255,5),
        FOVBehavior="Center", ShowHighlight=true,
        HighlightColor=Color3.fromRGB(255,60,60),
        ESP={Enabled=true,ShowName=true,ShowTeam=true,ShowHealth=true,ShowWeapon=true},
    }

    -- UI
    do
        local SC=_genv.SilentConfig
        local _sec=criarSection
        local MainSection=_sec(SilentAim,"Principal","Controle do silent aim","lucide:crosshair",true)
        local ToggleAim,firstRunMain=nil,true

        local _sbId=KeybindManager.register("SilentAim",Enum.KeyCode.Q,function()
        ...
        ToggleAim=MainSection:Toggle({
            ...
            Callback=function(s)
                SC.Enabled=s
                if firstRunMain then firstRunMain=false; return end
                notificar(s and "Ativado" or "Desativado",2,s and "lucide:circle-check" or "lucide:circle-x")
            end,
        })
        ...
        MainSection:Keybind({
            ...
            Callback=function(v)
                if v and Enum.KeyCode[v] then KeybindManager.update(_sbId,Enum.KeyCode[v]) end
            end,
        })

        MainSection:Slider({
            Title="Hit chance", Desc="Porcentagem de tiros que acertar\xC3\xA3o o alvo",
            Step=1, Value={Min=0,Max=100,Default=SC.HitChance}, Flag="ChanceSilentAim",
            Callback=function(v) SC.HitChance=v end,
        })

        local WLSection=_sec(SilentAim,"Lista de exce\xC3\xA7\xC3\xB5es","Gerenciar amigos e times","lucide:shield",false)
        local PlayerWL
        ...
        WLSection:Button({
            ...
            Callback=function()
                PlayerCache.refresh()
                if PlayerWL then PlayerWL:Refresh(GetPlayerNames()) end
                notificar("Lista atualizada",1,"lucide:refresh-ccw")
            end,
        })
        PlayerWL=WLSection:Dropdown({
            Title="Ignorar jogadores",
            Desc=cor({"N\xC3\xA3o","#FF0000"},{" foca nesses jogadores"}),
        ...
        WLSection:Dropdown({
            Title="Ignorar times",
            Desc=cor({"N\xC3\xA3o","#FF0000"},{" foca nesses times"}),
        ...
        local FocusSection=_sec(SilentAim,"Focar","Focar em jogadores espec\xC3\xADficos","lucide:scan-eye",false)
        local firstRunFocus=true

        FocusSection:Toggle({
            Title="Modo foco",
            Desc=cor({"Foca "},{"apenas","#FFAA00"},{" em quem est\xC3\xA1 na lista"}),
            ...
            Callback=function(s)
                ...
                notificar("Modo foco: "..(s and "Ativado" or "Desativado"),2,"lucide:circle-alert")
            end,
        })

        local FocusPar,FocusRmDrop

        local function _updFocus()
            if FocusPar then
                FocusPar:SetDesc(#SC.FocusList>0 and _m.concat(SC.FocusList,", ") or "Nenhum alvo definido.")
            end
            if FocusRmDrop then FocusRmDrop:Refresh(SC.FocusList) end
        end
        ...
        FocusSection:Input({
            ...
            Callback=function(text)
                ...
                for _,p in _p(Players:GetPlayers()) do
                    if p.Name:lower():sub(1,#text)==text:lower() then
                        if not _m.find(SC.FocusList,p.Name) then
                            _m.insert(SC.FocusList,p.Name)
                            _updFocus()
        ...
        FocusRmDrop=FocusSection:Dropdown({
            Title="Remover da lista",
            Desc=cor({"Selecione para "},{"excluir","#FF0000"}),
            Values={}, Value=nil, Multi=false, AllowNone=true,
            Callback=function(val)
                if not val then return end
                local idx=_m.find(SC.FocusList,val)
                if idx then _m.remove(SC.FocusList,idx); _updFocus(); FocusRmDrop:Select(nil) end
            end,
        })

        FocusPar=FocusSection:Paragraph({
            Title="Jogadores focados", Desc="Nenhum alvo",
            Buttons={{
                Icon="lucide:trash-2", Title="Limpar",
                Callback=function()
                    SC.FocusList={}; _updFocus(); notificar("Lista limpa",2,"lucide:trash")
                end,
            }},
        })

        local LogicSection=_sec(SilentAim,"Configura\xC3\xA7\xC3\xB5es","Ajustar configura\xC3\xA7\xC3\xB5es","lucide:settings-2",false)

        LogicSection:Dropdown({Title="Prioridade",Desc="Como escolher o melhor alvo",Values={"Distance","Health"},Value="Distance",Flag="PrioridadeSilentAim",Callback=function(o) SC.TargetPriority=o end})
        LogicSection:Dropdown({Title="Partes",Desc="Onde o tiro deve ir",Values={"Aleat\xC3\xB3rio","Cabe\xC3\xA7a","Tronco","Bra\xC3\xA7o direito","Bra\xC3\xA7o esquerdo","Perna direita","Perna esquerda"},Value={"Aleat\xC3\xB3rio"},Multi=true,AllowNone=true,Flag="PartesSilentAim",Callback=function(o) SC.TargetPart=o end})
        LogicSection:Dropdown({Title="Alvos",Desc="Quem deve ser atacado",Values={"Todos","Inimigos"},Value="Inimigos",Flag="AlvosSilentAim",Callback=function(o) SC.TeamCheck=(o=="Todos") and "All" or "Team" end})
        LogicSection:Toggle({Title="Humanizar",Desc=cor({"O tiro sair\xC3\xA1 mais "},{"legit","#00FF00"}),Icon="lucide:user-check",Type="Checkbox",Value=SC.UseLegitOffset,Flag="LegitSilentAim",Callback=function(v) SC.UseLegitOffset=v end})
        LogicSection:Toggle({Title="Wall check",Desc=cor({"Ignora","#FF0000"},{" inimigos atr\xC3\xA1s de paredes"}),Icon="lucide:brick-wall",Type="Checkbox",Value=SC.WallCheck,Flag="WallcheckSilentAim",Callback=function(v) SC.WallCheck=v end})
        LogicSection:Slider({Title="Dist\xC3\xA2ncia m\xC3\xA1xima",Desc="Alcance do silent aim",Step=10,Value={Min=50,Max=5000,Default=SC.MaxDistance},Flag="AlcanceSilentAim",Callback=function(v) SC.MaxDistance=v end})

        local VisSection=_sec(SilentAim,"Visual","FOV","lucide:palette",false)
        ...
        VisSection:Dropdown({Title="Posi\xC3\xA7\xC3\xA3o",Desc="Modifica a posi\xC3\xA7\xC3\xA3o do FOV",Values={"Mouse","Center"},Value="Center",Flag="PosicaoFOV",Callback=function(v) SC.FOVBehavior=v end})
        ...
        local InfoSection=_sec(SilentAim,"HUD","Informa\xC3\xA7\xC3\xB5es dos alvos","lucide:layout-template",false)

        local TgName,TgHP,TgWpn,TgTeam
        local function _espLock(locked)
            for _,t in _p({TgName,TgTeam,TgHP,TgWpn}) do
                if t then if locked then t:Lock() else t:Unlock() end end
            end
        end

        InfoSection:Toggle({Title="HUD",Desc="Mostra informa\xC3\xA7\xC3\xB5es do alvo",Icon="lucide:app-window",Type="Checkbox",Value=SC.ESP.Enabled,Flag="InfoSilentAim",Callback=function(v) SC.ESP.Enabled=v; _espLock(not v) end})
        TgName  =InfoSection:Toggle({Title="Nome",  Desc="Mostra o nome do jogador",          Icon="lucide:text-cursor",Type="Checkbox",Value=SC.ESP.ShowName,  Flag="InfoNomeSilentAim",Callback=function(v) SC.ESP.ShowName=v end})
        TgTeam  =InfoSection:Toggle({Title="Time",  Desc="Mostra o time do jogador",          Icon="lucide:users",      Type="Checkbox",Value=SC.ESP.ShowTeam,  Flag="InfoTimeSilentAim",Callback=function(v) SC.ESP.ShowTeam=v end})
        TgHP    =InfoSection:Toggle({Title="Vida",  Desc="Mostra a vida do alvo",             Icon="lucide:heart-pulse",Type="Checkbox",Value=SC.ESP.ShowHealth,Flag="InfoVidaSilentAim",Callback=function(v) SC.ESP.ShowHealth=v end})
        TgWpn   =InfoSection:Toggle({Title="Item",  Desc="Mostra o que o alvo est\xC3\xA1 segurando",Icon="lucide:sword",Type="Checkbox",Value=SC.ESP.ShowWeapon,Flag="InfoItemSilentAim",Callback=function(v) SC.ESP.ShowWeapon=v end})

        if not SC.ESP.Enabled then _espLock(true) end
    end
end
...
local LOCAL_ID = LocalPlayer.UserId  -- imutável, não depende de cloneref
...
if getgenv()._hitboxConn then
    pcall(function() getgenv()._hitboxConn:Disconnect() end)
    getgenv()._hitboxConn = nil
end
if getgenv()._hitboxRemovingConn then
    pcall(function() getgenv()._hitboxRemovingConn:Disconnect() end)
    getgenv()._hitboxRemovingConn = nil
end

if not getgenv()._hitboxSaved  then getgenv()._hitboxSaved  = {} end
if not getgenv()._hitboxLights then getgenv()._hitboxLights = {} end

local hitboxSaved  = getgenv()._hitboxSaved
local hitboxLights = getgenv()._hitboxLights
...
local function hitboxShouldTarget(player)
    ...
    -- Usa UserId para evitar falha de cloneref
...
getgenv()._hitboxConn = RunService.Heartbeat:Connect(function()
    local cfg = hitboxConfig()
...
getgenv()._hitboxRemovingConn = Players.PlayerRemoving:Connect(hitboxCleanPlayer)
...
local function espAddPlayer(player)
    ...
    env.espStore[player.UserId] = entry
end
...
local function espUpdate()
    ...
    for uid, entry in pairs(env.espStore) do
        -- Resolve o player pelo UserId (imutável, ignora cloneref)
        ...
        if not player then
            ...
            if not alive or sameTeam then
                ...
                if showBillboard then
                    ...
                    if cfg.Name then lblName.Text = player.Name; lblName.Visible = true
                    else lblName.Visible = false end
...
env.StopESP = function()
    for _, conn in pairs(env.espConns) do
    ...
    if env.espHold then
        pcall(function() env.espHold:Destroy() end)
        env.espHold = nil
    end
end
...
env.StartESP = function()
    ...
    espGetContainer()
    for _, player in ipairs(Players:GetPlayers()) do
        if player.UserId ~= LOCAL_ID then espAddPlayer(player) end
    end
...
do
    ...
    ConfigSection:Slider({   Title = "Transparência", Desc = cor({ "0 = " }, { "visível", "#00FF00" }, { "; 1 = " }, { "invisível", "#FF0000" }), Icon = "lucide:ghost", Step = 0.02, Value = { Min = 0, Max = 1, Default = HC.Transparency }, Flag = "VisibilidadeHitbox", Callback = function(v) HC.Transparency = v end })

    local TeamSection = criarSection(HitboxExpander, "Filtros", "Configuração de times", "lucide:users", false)
    TeamSection:Toggle({   Title = "Filtrar por time", Desc = cor({ "Aplica", "#00AAFF" }, { " a hitbox apenas nos times selecionados" }), Icon = "lucide:sliders-horizontal", Value = HC.TeamFilterEnabled, Flag = "FiltrarTimeHitbox", Callback = function(v) HC.TeamFilterEnabled = v end })
    TeamSection:Dropdown({ Title = "Times", Desc = cor({ "Selecione", "#FFFFFF" }, { " os times" }), Icon = "lucide:users-round", Values = GetTeamNames(), Value = {}, Multi = true, AllowNone = true, Flag = "TimesFiltradosHitbox", Callback = function(opt) HC.SelectedTeams = opt end })
    ...
    HBPlayerWhitelist = WhitelistSection:Dropdown({ Title = "Ignorar jogadores", Desc = cor({ "Não", "#FF0000" }, { " foca nesses jogadores" }), Values = GetPlayerNames(), Value = HC.WhitelistedUsers, Multi = true, AllowNone = true, Callback = function(opts) HC.WhitelistedUsers = opts end })
    WhitelistSection:Dropdown({ Title = "Ignorar times", Desc = cor({ "Não", "#FF0000" }, { " foca nesses times" }), Values = GetTeamNames(), Value = HC.WhitelistedTeams, Multi = true, AllowNone = true, Flag = "IgnorarTimesHitbox", Callback = function(opts) HC.WhitelistedTeams = opts end })
    ...
    FocusSection:Toggle({ Title = "Modo foco", Desc = cor({ "Foca " }, { "apenas", "#FFAA00" }, { " em quem está na lista" }), Icon = "lucide:focus", Type = "Checkbox", Value = HC.FocusMode, Flag = "ModoFocarHitbox", Callback = function(state)
        HC.FocusMode = state
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p.Name:lower():sub(1, #text) == text:lower() then
                if not table.find(HC.FocusList, p.Name) then
                    table.insert(HC.FocusList, p.Name); UpdateHBFocusUI()
                    notificar("Alvo adicionado: " .. p.Name, 2, "lucide:check")
                end
                return
            end
    ...
    HBFocusRemoveDrop = FocusSection:Dropdown({ Title = "Remover da lista", Desc = cor({ "Selecione para " }, { "excluir", "#FF0000" }), Values = {}, Value = nil, Multi = false, AllowNone = true, Callback = function(val)
        ...
        if idx then table.remove(HC.FocusList, idx); UpdateHBFocusUI(); HBFocusRemoveDrop:Select(nil) end
    end })

    HBFocusParagraph = FocusSection:Paragraph({ Title = "Jogadores focados", Desc = "Nenhum alvo", Buttons = {{ Icon = "lucide:trash-2", Title = "Limpar", Callback = function() HC.FocusList = {}; UpdateHBFocusUI(); notificar("Lista limpa", 2, "lucide:trash") end }} })
end

getgenv().ESPConfig = { Enabled = false, TeamCheck = false, Chams = false, Name = false, Studs = false, Health = false, WeaponN = false }

do
    local EC = getgenv().ESPConfig
    local MainSection = criarSection(ESP, "Principal", "Controles gerais", "lucide:eye", true)
    MainSection:Paragraph({ Title = "ESP", Image = "lucide:scan-eye", Desc = cor({ "Permite ver " }, { "jogadores", "#FFFFFF" }, { " através das " }, { "paredes", "#FF0000" }) })
    ...
    MainSection:Toggle({ Title = "Team check", Desc = cor({ "Não", "#FF0000" }, { " mostra se do seu " }, { "time", "#00FF00" }), Icon = "lucide:shield-check", Value = EC.TeamCheck, Flag = "TimeESP", Callback = function(v) EC.TeamCheck = v end })

    local VS = criarSection(ESP, "Visualização", "Elementos visuais", "lucide:monitor", true)
    ...
    VS:Toggle({ Title = "Distância", Desc = cor({ "Mostra a " }, { "distância", "#00AAFF" }),                         Icon = "lucide:ruler",    Value = EC.Studs,   Flag = "DistanciaESP", Callback = function(v) EC.Studs   = v end })
...
local gh = gethui or function() return CoreGui end
if gh():FindFirstChild(".") then gh():FindFirstChild("."):Destroy() end
local tasUi = Instance.new("ScreenGui")
tasUi.Name, tasUi.ResetOnSpawn, tasUi.Parent = ".", false, gh()
...
tas.UpdateVisuals = function(bC, pC, op)
    ...
    for _, d in pairs(tas.Loaded) do
        if d.Adornments then for _, p in ipairs(d.Adornments) do if p and p.Parent then p.Color3 = tas.ColorBot; p.Transparency = tas.VisualOpacity end end end
        if d.PathParts  then for _, p in ipairs(d.PathParts)  do if p              then p.Color3 = tas.ColorPath; p.Transparency = tas.VisualOpacity end end end
    end
end
...
tas.SaveCurrent = function()
    ...
    local rj = HttpService:JSONEncode({ Version = 1, Frames = tas.RecFrames })
...
local function getObj(path)
    ...
    local cur = game
    for _, s in ipairs(segs) do if s ~= "Game" then cur = cur:FindFirstChild(s); if not cur then return nil end end end
    return cur
end
...
f3x.ApplyConfig = function(n)
    ...
    for _, v in ipairs(d.Parts or {}) do
        ...
        if obj and obj:IsA("BasePart") and (obj.Position - targetCf.Position).Magnitude < 2 then foundPart = obj
        else
            for _, o in ipairs(Workspace:GetDescendants()) do
                if o:IsA("BasePart") and o:GetFullName() == v.Path and (o.Position - targetCf.Position).Magnitude < 2 then foundPart = o; break end
            end
        end
...
_G.ChatGPT.Ask = function(promptText)
    ...
    if decodeSuccess and decoded.choices and decoded.choices[1] then aiText = decoded.choices[1].message.content
    else aiText = response.Body end
...
_G.ChatGPT.SendToChat = function(msg)
    ...
    if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then
        ...
        if channels then
            ...
            if target then target:SendAsync(msgString) end
        end
    else
        local events = rep:FindFirstChild("DefaultChatSystemChatEvents")
        if events then local say = events:FindFirstChild("SayMessageRequest"); if say then say:FireServer(msgString, "All") end end
    end
end
...
do
    ...
    -- Conecta callbacks de UI na lógica já carregada acima
    local T = getgenv().TAS
...
do
    ...
    local function GetF3X()  return getgenv().F3X end

    local function UpdateF3XHUD()
        local f3x2 = GetF3X()
        if not f3x2 or not InfoParagraph then return end
        if not f3x2.SelectedParts or #f3x2.SelectedParts == 0 then
            InfoParagraph:SetTitle(cor({ "Nada ", "#FF0000" }, { "selecionado" })); InfoParagraph:SetDesc(""); return
        end
        local part = f3x2.SelectedParts[1]; if not part then return end
        local s = part.Size
    ...
    -- Conecta UpdateUI na lógica já carregada acima
    ...
    SS:Toggle({ Title = "Selecionar", Desc = cor({ "Ativa a ferramenta de " }, { "seleção", "#00FF00" }), Icon = "lucide:mouse-pointer-2", Callback = function(v) local f = GetF3X(); if f and f.Toggle then f.Toggle(v) end end })
    ...
    ES:Button({ Title = "Aplicar tamanho", Desc = cor({ "Aplica as " }, { "dimensões", "#00FF00" }, { " na parte selecionada" }), Icon = "lucide:check", Callback = function()
        local f = GetF3X(); if f and f.ApplySize then f.ApplySize(Vector3.new(tonumber(InputX.Value), tonumber(InputY.Value), tonumber(InputZ.Value))) end
    end })
    local gUR = ES:Group()
    ...
    -- Popula dropdown agora que está criado
    if f3x.ListConfigs then ConfigDropdown:Refresh(f3x.ListConfigs()) end
    ...
    local function CreateF3XActions()
        ...
        ActionDropdown = CS:Dropdown({ Title = "Ações", Desc = cor({ "Gerenciar " }, { "configurações", "#FFAA00" }), Icon = "lucide:settings-2", Values = {
            { Title = "Salvar novo F3X", Desc = cor({ "Cria",    "#00FF00" }, { " um novo registro" }),          Icon = "lucide:save",   Callback = function()
                ...
                local f = GetF3X(); if f and f.SaveConfig then local nl = f.SaveConfig(CurrentConfigName); if nl then ConfigDropdown:Refresh(nl); notificar("Salvo com sucesso", 3, "lucide:check") end end; CreateF3XActions() end },
            { Title = "Carregar F3X",   Desc = cor({ "Carrega", "#00AAFF" }, { " os dados selecionados" }),      Icon = "lucide:upload", Callback = function()
                ...
                local f = GetF3X(); if f and f.ApplyConfig then f.ApplyConfig(SelectedConfig); notificar("Configuração aplicada", 3, "lucide:check") end; CreateF3XActions() end },
            { Title = "Deletar F3X",    Desc = cor({ "Remove",  "#FF0000" }, { " permanentemente o arquivo" }),  Icon = "lucide:trash-2",Callback = function()
                if not SelectedConfig then notificar("Selecione um F3X primeiro", 3, "lucide:mouse-pointer-click"); CreateF3XActions(); return end
                local f = GetF3X(); if f and f.DeleteConfig then local nl = f.DeleteConfig(SelectedConfig); if nl then ConfigDropdown:Refresh(nl); SelectedConfig = nil; ConfigDropdown:Select(nil); notificar("F3X deletado", 3, "lucide:trash") end end; CreateF3XActions() end },
        }, Value = "Nenhuma" })
    end
    CreateF3XActions()
end

-- Limpa conexões de loops anteriores
...
-- Restaura namecall do player se existir de re-exec anterior
...
local PL_Camera = Workspace.CurrentCamera  -- Camera não é top-level na UI
...
getgenv()._playerSpecConn = RunService.RenderStepped:Connect(function()
    ...
    if cfg.SpectateActive then
        ...
        if target then
            local hum = PL_getHumanoid(target)
            if hum then PL_Camera.CameraSubject = hum end
        end
    else
        local hum = PL_getHumanoid(LocalPlayer)
        if hum and PL_Camera.CameraSubject ~= hum then PL_Camera.CameraSubject = hum end
    end
end)
...
-- __namecall: encadeia depois do silent (que já está instalado em SA_mt.__namecall)
local PL_mt     = getrawmetatable(game)
local PL_old_nc = PL_mt.__namecall          -- captura o hook do silent como base
getgenv()._playerOldNC = PL_old_nc          -- salva para restaurar no re-exec
...
local PL_lastAnon  = false
local PL_lastInvis = false
local PL_lastSpeed = false
...
getgenv()._playerStateTask = task.spawn(function()
    while task.wait(0.1) do
        ...
        if cfg.InvisEnabled ~= PL_lastInvis then
            PL_lastInvis = cfg.InvisEnabled
            PL_toggleInvis(cfg.InvisEnabled)
        end

        if cfg.AnonEnabled ~= PL_lastAnon then
            PL_lastAnon = cfg.AnonEnabled
            PL_applyAnonVisuals()
        end

        local needsPhysics = cfg.SpeedEnabled or cfg.JumpEnabled
        if needsPhysics and not PL_lastSpeed then
            PL_startPhysicsLoop()
        elseif not needsPhysics and PL_lastSpeed then
            PL_stopPhysicsLoop()
        end
        PL_lastSpeed = needsPhysics
    end
end)
    ...
    task.spawn(function()
        pcall(function()
            ...
            for _, obj in ipairs(character:GetChildren()) do
                if obj:IsA("Shirt") or obj:IsA("Pants") or obj:IsA("ShirtGraphic") or obj:IsA("CharacterMesh") then obj:Destroy() end
            end
...
local function PL_resolveUserId(input)
    local id = tonumber(input); local name
    local ok = pcall(function()
        if id then name = Players:GetNameFromUserIdAsync(id)
        else id = Players:GetUserIdFromNameAsync(input); name = Players:GetNameFromUserIdAsync(id) end
    end)
...
do
    local function GetAvatar() return getgenv().Avatar end
    ...
    local g1 = MS:Group()
    g1:Button({ Title = "Aplicar",   Desc = cor({ "Aplica",   "#00FF00" }, { " o char" }),            Icon = "lucide:check",     Callback = function() local a = GetAvatar(); if a and a.ApplySkin   then a.ApplySkin(targetInput)   end end })
    g1:Space()
    g1:Button({ Title = "Restaurar", Desc = cor({ "Restaura", "#FF0000" }, { " sua skin original" }), Icon = "lucide:rotate-ccw",Callback = function() local a = GetAvatar(); if a and a.RestoreSkin then a.RestoreSkin()             end end })
    ...
    local g2 = OS:Group()
    g2:Button({ Title = "Aplicar",   Desc = cor({ "Muda",   "#00FF00" }, { " a skin do jogador alvo" }), Icon = "lucide:wand-2",     Callback = function() local a = GetAvatar(); if a and a.ApplySkinToOther then a.ApplySkinToOther(otherTargetName, otherSkinInput) end end })
    g2:Button({ Title = "Restaurar", Desc = cor({ "Reseta", "#FF0000" }, { " a skin do jogador alvo" }), Icon = "lucide:refresh-ccw",Callback = function() local a = GetAvatar(); if a and a.RestoreOther      then a.RestoreOther(otherTargetName)                  end end })
    OS:Button({ Title = "Aplicar favorito", Desc = cor({ "Aplica a skin " }, { "favoritada", "#FFAA00" }, { " selecionada no alvo" }), Icon = "lucide:star", Callback = function()
        ...
        local a = GetAvatar(); if a and a.ApplySkinToOther then a.ApplySkinToOther(otherTargetName, selectedFavorite, true) end
    end })
    ...
    local function RefreshFavList()
        local a = GetAvatar(); if not (a and a.GetSavedSkins and SkinSelectDropdown) then return end
        local skins = a.GetSavedSkins(); SkinSelectDropdown:Refresh(skins)
        local found = false
        for _, v in pairs(skins) do if ((type(v) == "table" and v.Title) or v) == selectedFavorite then found = true; break end end
    ...
    CreateFavActions = function()
        ...
        ActionDropdown = FS:Dropdown({ Title = "Gerenciar", Desc = cor({ "Gerenciar " }, { "favoritos", "#FFFFFF" }), Icon = "lucide:settings-2", Values = {
            { Title = "Aplicar em mim",       Desc = cor({ "Usa", "#00FF00" }, { " o char selecionado em você" }),     Icon = "lucide:user-check", Callback = function()
                ...
                local a = GetAvatar(); if a and a.LoadSkin then a.LoadSkin(selectedFavorite); notificar("Aplicado: " .. selectedFavorite, 3, "lucide:check") end end },
            { Title = "Favoritar char atual", Desc = cor({ "Salva", "#FFAA00" }, { " o char que você está usando" }), Icon = "lucide:save",       Callback = function()
                local a = GetAvatar(); if a and a.SaveSkin then a.SaveSkin(targetInput); notificar("Char salvo", 3, "lucide:check"); RefreshFavList() end; CreateFavActions() end },
            { Title = isConfirmingFavDelete and "Confirmar deleção?" or "Deletar selecionado",
              ...
              Callback = function()
                ...
                if not isConfirmingFavDelete then
                    ...
                    local a = GetAvatar(); if a and a.DeleteSkin then a.DeleteSkin(selectedFavorite); notificar("Deletado: " .. selectedFavorite, 3, "lucide:trash") end
...
do
    ...
    local function FindPlayerByPrefix(txt)
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer then
                if p.Name:lower():sub(1, #lower) == lower or p.DisplayName:lower():sub(1, #lower) == lower then return p.Name end
            end
        end
        return nil
    end
    ...
    CS:Toggle({ Title = "Anônimo",       Desc = cor({ "Altera o seu nome" }, { " visualmente", "#00AAFF" }),                Flag = "Anonimo",  Icon = "lucide:square-user-round", Callback = function(v) PC.AnonEnabled = v; if v then local h = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid"); if h then h.DisplayName = PC.FakeName end end end })
    ...
    gs:Button({ Title = "Resetar", Icon = "lucide:rotate-ccw",   Callback = function() PC.SpeedEnabled = false; PC.SpeedVal = 16; local h = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid"); if h then h.WalkSpeed = GetGhost(h, "WalkSpeed") or 16 end end })
    MS:Space()
    local jmpVal = 50
    ...
    gj:Button({ Title = "Resetar", Icon = "lucide:rotate-ccw",   Callback = function() PC.JumpEnabled = false; PC.JumpVal = 50; local h = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid"); if h then h.JumpPower = GetGhost(h, "JumpPower") or 50 end end })
    ...
    IS:Input({ Title = "Jogador", Desc = "Nome ou Display", Placeholder = "Digite aqui", InputIcon = "lucide:search", Callback = function(v)
        ...
        local found = FindPlayerByPrefix(v)
        if found then PC.TargetPlayer = found; notificar("Selecionado: " .. found, 3, "lucide:user-check")

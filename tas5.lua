...
local function appFr(f, hrp, hum)
    ...
    if f.vel then 
        ...
        local moveDir = Vector3.new(v.X, 0, v.Z)
        if moveDir.Magnitude > 0.01 then hum:Move(moveDir.Unit, false) else hum:Move(Vector3.zero) end
    end
...
local function actTas(n)
    ...
    d.MarkerConn = rs.Stepped:Connect(function()
        ...
        if Vector3.new(dlt.X, 0, dlt.Z).Magnitude <= tas.ActRad and math.abs(dlt.Y) <= tas.ActH and hrp.CFrame.LookVector:Dot(cf.LookVector) >= math.cos(math.rad(tas.ActAng)) then
            ...
            d.PlayConn = rs.Stepped:Connect(function()
                local c = lp.Character
                if c ~= lastC then
                    lastC = c
                    cachedHrp = c and c:FindFirstChild("HumanoidRootPart")
...
tas.StartRecording = function()
    ...
    local acc, idl = 0, 0
    tas.RecConn = rs.Stepped:Connect(function(_, dt)
        local hrp = lp.Character and lp.Character:FindFirstChild("HumanoidRootPart")

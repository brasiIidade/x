...
local function GetTarget(config)
    ...
    for _, v in ipairs(Services.Players:GetPlayers()) do
        ...
        if (r.Position - Camera.CFrame.Position).Magnitude > maxDist then continue end
        ...
        if onScreen then
            ...
            if dist <= config.FOVSize then
                if IsVisible(pObj, c, config) then
                    ...
                    local effectiveDist = isCurrent and (dist - 15) or dist

                    if priority == "Health" then
                        if h.Health < bestHealth or (h.Health == bestHealth and effectiveDist < bestDist) then
                            bestHealth = h.Health

...
_G.StartSilentAim = function()
    ...
    local Name = Functions:Create("TextLabel", {Parent = ScreenGui, Size = UDim2New(0, 100, 0, 20), AnchorPoint = Vector2New(0.5, 0.5), BackgroundTransparency = 1, TextColor3 = Color3fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = ESP_SETTINGS.FontSize, TextStrokeTransparency = 0, TextStrokeColor3 = Color3fromRGB(0, 0, 0), RichText = true, Visible = false})
    ...
    local Box = Functions:Create("Frame", {Parent = ScreenGui, BackgroundColor3 = Color3fromRGB(0, 0, 0), BackgroundTransparency = 0.75, BorderSizePixel = 0, Visible = false})
    ...
    local Outline = Functions:Create("UIStroke", {Parent = Box, Enabled = ESP_SETTINGS.Drawing.Boxes.Gradient, Transparency = 0, Color = Color3fromRGB(255, 255, 255), LineJoinMode = Enum.LineJoinMode.Miter})
    ...
    local Healthbar = Functions:Create("Frame", {Parent = ScreenGui, BackgroundColor3 = Color3fromRGB(255, 255, 255), BackgroundTransparency = 0, Visible = false})
    local BehindHealthbar = Functions:Create("Frame", {Parent = ScreenGui, ZIndex = -1, BackgroundColor3 = Color3fromRGB(0, 0, 0), BackgroundTransparency = 0, Visible = false})
    ...
    local HealthText = Functions:Create("TextLabel", {Parent = ScreenGui, Size = UDim2New(0, 100, 0, 20), AnchorPoint = Vector2New(0.5, 0.5), BackgroundTransparency = 1, TextColor3 = Color3fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = ESP_SETTINGS.FontSize, TextStrokeTransparency = 0, TextStrokeColor3 = Color3fromRGB(0, 0, 0), Visible = false})
    local Chams = Functions:Create("Highlight", {Parent = ScreenGui, FillTransparency = 1, OutlineTransparency = 0, OutlineColor = Color3fromRGB(119, 120, 255), DepthMode = "AlwaysOnTop", Enabled = false})
        ...
        if _G.SilentAimActive then
            ...
            if Character and Character:FindFirstChild("HumanoidRootPart") and Character:FindFirstChild("Humanoid") and config.ESP.Enabled then
                ...
                if OnScreen then
                    ...
                    local scaleFactor = (Size * ViewportSizeY) / (Pos.Z * 2)
                    ...
                    if config.ShowHighlight then
                        ...
                        local breathe_effect = MathAtan(MathSin(currentTime * 2)) * 2 / MathPi
                        Chams.OutlineTransparency = 0 * breathe_effect * 0.01
                    else
                        Chams.Enabled = false
                    end

                    RotationAngle = RotationAngle + (currentTime - Tick) * ESP_SETTINGS.Drawing.Boxes.RotationSpeed * MathCos(MathPi / 4 * currentTime - MathPi / 2)
                    Gradient1.Rotation = RotationAngle; Gradient2.Rotation = RotationAngle; Tick = currentTime

                    Box.Visible = true; Box.Position = UDim2New(0, Pos.X - w / 2, 0, Pos.Y - h / 2); Box.Size = UDim2New(0, w, 0, h)
                    
                    LeftTop.Visible = true; LeftTop.Position = UDim2New(0, Pos.X - w / 2, 0, Pos.Y - h / 2); LeftTop.Size = UDim2New(0, w / 5, 0, 1)
                    LeftSide.Visible = true; LeftSide.Position = UDim2New(0, Pos.X - w / 2, 0, Pos.Y - h / 2); LeftSide.Size = UDim2New(0, 1, 0, h / 5)
                    BottomSide.Visible = true; BottomSide.Position = UDim2New(0, Pos.X - w / 2, 0, Pos.Y + h / 2); BottomSide.Size = UDim2New(0, 1, 0, h / 5); BottomSide.AnchorPoint = Vector2New(0, 5)
                    BottomDown.Visible = true; BottomDown.Position = UDim2New(0, Pos.X - w / 2, 0, Pos.Y + h / 2); BottomDown.Size = UDim2New(0, w / 5, 0, 1); BottomDown.AnchorPoint = Vector2New(0, 1)
                    RightTop.Visible = true; RightTop.Position = UDim2New(0, Pos.X + w / 2, 0, Pos.Y - h / 2); RightTop.Size = UDim2New(0, w / 5, 0, 1); RightTop.AnchorPoint = Vector2New(1, 0)
                    RightSide.Visible = true; RightSide.Position = UDim2New(0, Pos.X + w / 2 - 1, 0, Pos.Y - h / 2); RightSide.Size = UDim2New(0, 1, 0, h / 5); RightSide.AnchorPoint = Vector2New(0, 0)
                    BottomRightSide.Visible = true; BottomRightSide.Position = UDim2New(0, Pos.X + w / 2, 0, Pos.Y + h / 2); BottomRightSide.Size = UDim2New(0, 1, 0, h / 5); BottomRightSide.AnchorPoint = Vector2New(1, 1)
                    BottomRightDown.Visible = true; BottomRightDown.Position = UDim2New(0, Pos.X + w / 2, 0, Pos.Y + h / 2); BottomRightDown.Size = UDim2New(0, w / 5, 0, 1); BottomRightDown.AnchorPoint = Vector2New(1, 1)
                    ...
                    if config.ESP.ShowHealth then
                        ...
                        Healthbar.Size = UDim2New(0, ESP_SETTINGS.Drawing.Healthbar.Width, 0, h * health)
                        BehindHealthbar.Position = UDim2New(0, Pos.X - w / 2 - 6, 0, Pos.Y - h / 2)
                        BehindHealthbar.Size = UDim2New(0, ESP_SETTINGS.Drawing.Healthbar.Width, 0, h)
                        
                        HealthText.Visible = true
                        local healthPercentage = MathFloor(Humanoid.Health / Humanoid.MaxHealth * 100)
                        HealthText.Position = UDim2New(0, Pos.X - w / 2 - 6, 0, Pos.Y - h / 2 + h * (1 - healthPercentage / 100) + 3)
                    ...
                    if config.ESP.ShowName then
                        ...
                        local statusColor = "255, 0, 0"
                        if plr and LocalPlayer.Team and plr.Team == LocalPlayer.Team then
                            statusText = "aliado"
                            statusColor = "0, 255, 0"
                        end
                        Name.Text = StringFormat('(<font color="rgb(%s)">%s</font>) %s', statusColor, statusText, Character.Name)
                        Name.Position = UDim2New(0, Pos.X, 0, Pos.Y + h / 2 + 8)
                        OffsetY = OffsetY + 11
                    else
                        Name.Visible = false
                    end
                    ...
                    if config.ESP.ShowTeam then
                        ...
                        local tColor = plr and plr.TeamColor and plr.TeamColor.Color or Color3fromRGB(255, 255, 255)
                        Team.Text = StringFormat('<font color="rgb(%d, %d, %d)">%s</font>', tColor.R*255, tColor.G*255, tColor.B*255, tName)
                        Team.Position = UDim2New(0, Pos.X, 0, Pos.Y + h / 2 + 8 + OffsetY)
                        OffsetY = OffsetY + 11
                    else
                        Team.Visible = false
                    end
                    ...
                    if config.ESP.ShowWeapon then
                        ...
                        Weapon.Text = tool and StringUpper(tool.Name) or "nada equipado"
                        Weapon.Position = UDim2New(0, Pos.X, 0, Pos.Y + h / 2 + 8 + OffsetY)
                        OffsetY = OffsetY + 11
                    else
                    ...
                    Distance.Text = StringFormat("[%d m]", MathFloor(Dist))
                    Distance.Position = UDim2New(0, Pos.X, 0, Pos.Y + h / 2 + 8 + OffsetY)

                else
                    Box.Visible = false; Name.Visible = false; Team.Visible = false; Distance.Visible = false; Weapon.Visible = false
                    Healthbar.Visible = false; BehindHealthbar.Visible = false; HealthText.Visible = false

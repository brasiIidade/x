...
if _G.AimFOVFill then pcall(function() _G.AimFOVFill:Remove() end) _G.AimFOVFill = nil end
...
_G.AimbotConfig = _G.AimbotConfig or {
    Enabled = true, -- Forçado para true
    TeamCheck = "Team",         
    TargetPart = {"Random"},
    ...
    WallCheck = true,
    FOVSize = 200,
    ShowFOV = true, -- Forçado para true
    FOVBehavior = "Center",
...
-- Garante que o FOV esteja visível mesmo se a config já existisse na memória
_G.AimbotConfig.ShowFOV = true
...
_G.StartSilentAim = function()
    ...
    -- 1. PREENCHIMENTO (Criado primeiro para ficar no fundo)
    local fov_fill = Drawing.new("Circle")
    fov_fill.Visible = true
    fov_fill.Thickness = 0
    fov_fill.Transparency = 0.3 -- Aumentei um pouco para garantir visibilidade (0.3 = 30% visível)
    fov_fill.Color = config.FOVColor1
    fov_fill.Filled = true
    fov_fill.NumSides = 8 -- Octogono
    _G.AimFOVFill = fov_fill

    -- 2. CONTORNO (Criado depois para ficar por cima)
    local fov_circle = Drawing.new("Circle")
    fov_circle.Visible = true
    fov_circle.Thickness = 2
    fov_circle.Transparency = 1 -- 1 é totalmente visível no Drawing API
    fov_circle.Color = config.FOVColor1
    fov_circle.Filled = false
    fov_circle.NumSides = 8 -- Octogono
    ...
    local c1 = RunService.RenderStepped:Connect(function()
        if _G.SilentAimActive and config.ShowFOV then
            ...
            -- ATUALIZA O FILL (PREENCHIMENTO)
            if fov_fill then
                fov_fill.Visible = true; fov_fill.Radius = config.FOVSize; fov_fill.Color = config.FOVColor1
                fov_fill.Position = pos
            end
        else 
            if fov_circle then fov_circle.Visible = false end 
            if fov_fill then fov_fill.Visible = false end 
        end
        ...
        if _G.SilentAimActive then
            ...
            if Character and Character:FindFirstChild("HumanoidRootPart") and Character:FindFirstChild("Humanoid") and config.ESP.Enabled then
                ...
                if OnScreen then
                    ...
                    Box.Visible = true; Box.Position = UDim2.new(0, Pos.X - w / 2, 0, Pos.Y - h / 2); Box.Size = UDim2.new(0, w, 0, h)
                    ...
                    if config.ESP.ShowName then
                        ...
                        Name.Text = string.format('(<font color="rgb(%s)">%s</font>) %s', statusColor, statusText, Character.Name)
                    ...
                    if config.ESP.ShowTeam then
                        ...
                        Team.Text = string.format('<font color="rgb(%d, %d, %d)">%s</font>', tColor.R*255, tColor.G*255, tColor.B*255, tName)
                        Team.Position = UDim2.new(0, Pos.X, 0, Pos.Y + h / 2 + 8 + OffsetY)
                        OffsetY = OffsetY + 11
                    else
                        Team.Visible = false
                    end
